#!/usr/bin/env node
/**
 * Engångskörning: tar bort tjänsten «Moms» från utvalda kunder.
 * Påverkar inte framtida val i tjänstlistan.
 *
 * Kör: node scripts/remove-moms-tjanst-kunder.js --dry-run
 *      node scripts/remove-moms-tjanst-kunder.js
 */
...
function isTargetKund(namn) {
  const folded = foldName(namn);
  return TARGET_KUNDER.some((target) => foldName(target) === folded);
}
...
async function main() {
  ...
  for (const targetNamn of TARGET_KUNDER) {
    ...
    for (const rec of records) {
      ...
      const cleaned = removeMomsTjanster(existing, catalog);

      if (cleaned.length === existing.length) {
        console.log(`Hoppar ${namn}: Moms fanns inte bland valda tjänster`);
        skipped += 1;
        continue;
      }

      console.log(`${dryRun ? '[dry-run] ' : ''}Tar bort Moms från ${namn} (${existing.length} → ${cleaned.length} tjänster)`);
      // eslint-disable-next-line no-await-in-loop
