#!/usr/bin/env node
/**
 * Engångskörning: tar bort «Utföra betalningsuppdrag» från alla kunder
 * utom Höglanders och ENA Operations. Påverkar inte framtida val i tjänstlistan.
 *
 * Kör: node scripts/remove-betalningsuppdrag-tjanst.js --dry-run
 *      node scripts/remove-betalningsuppdrag-tjanst.js
 */
...
async function main() {
  ...
  for (const rec of records) {
    ...
    if (customerMayKeepBetalningsuppdrag(namn)) {
      kept += 1;
      skipped += 1;
      continue;
    }
    ...
    const normalized = catalog.length
      ...
      : existing;
    const cleaned = removeBetalningsuppdragTjanster(normalized, catalog);
    ...
    console.log(`${dryRun ? '[dry-run] ' : ''}Tar bort betalningsuppdrag från ${namn || rec.id} (${existing.length} → ${cleaned.length} tjänster)`);
    // eslint-disable-next-line no-await-in-loop
  ...
  console.log(`Klart. ${updated} kunder rensade, ${kept} Höglanders/ENA Operations oförändrade, ${skipped - kept} övriga utan ändring.`);
}
