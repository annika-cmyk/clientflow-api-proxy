...
Optional Gmail connect per user (sidebar *Mejl*). Inbox shows messages under the Gmail label **KUNDER** (child labels matched to customer names). Compose/send uses the user’s Gmail so mail appears from their address. Env (canonical): `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_GMAIL_REDIRECT_URI` (prod redirect `https://www.app.clientflow.se/api/gmail/oauth/callback`); aliases such as `GOOGLE_GMAIL_CLIENT_ID` / `GOOGLE_OAUTH_CLIENT_ID` / `GMAIL_CLIENT_ID` (and matching secret/redirect names) are accepted. `/api/gmail/status` exposes `envPresent` booleans and `envResolvedFrom` without leaking secrets. When unset, Mejl shows status *Ej konfigurerad* and the connect button toasts the missing variables.
