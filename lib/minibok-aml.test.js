...
describe('mapOverallRisk', () => {
  it('mappar svenska nivåer', () => {
    ...
    assert.equal(mapOverallRisk('Låg'), 'low');
    assert.equal(mapOverallRisk('Medel'), 'normal');
    assert.equal(mapOverallRisk('Normal'), 'normal');
    assert.equal(mapOverallRisk('Förhöjd'), 'elevated');
    assert.equal(mapOverallRisk('Hog'), 'high');
    assert.equal(mapOverallRisk('Hög'), 'high');
    assert.equal(mapOverallRisk('Oacceptabel'), 'unacceptable');
    assert.equal(mapOverallRisk('Sammantagen risknivå: Förhöjd\n\nMotivering.'), 'elevated');
  });
});
...
describe('mapCustomerAmlRisk', () => {
  it('mappar Riskniva, Omsättning, UBO och riskåtgärder', () => {
    ...
    assert.equal(payload.overallRisk, 'normal');
  ...
  it('första saknade risknivå → overallRisk null (okänt ≠ normal)', () => {
...
describe('mapAgencyAmlRisk / policy', () => {
  ...
  it('använder byråns sparade riskaptittext', () => {
    const record = {
      id: 'recByra',
      fields: { '9. Riskaptit': 'Vår egen riskaptit.' }
    };
    const policy = mapAgencyAmlPolicy(record);
    assert.equal(policy.sections['9. Riskaptit'], 'Vår egen riskaptit.');
    const risk = mapAgencyAmlRisk(record);
    assert.equal(risk.sections['9. Riskaptit'], 'Vår egen riskaptit.');
  });
