...
describe('identifierade-risker-view', () => {
  it('renderar tjänst som expanderbart kort med S×K och hot-tagg', () => {
    const html = View.render({
      tjanster: [{
        ...
        sannolikhet: 2,
        konsekvens: 2,
        motivering_inneboende_risk: 'Sannolikheten är låg eftersom felaktiga avskrivningar sällan förekommer i praktiken.',
        hot: [
    ...
    assert.doesNotMatch(html, /<span class="tag tag-pt">/);
    ...
    assert.doesNotMatch(html, /vuln-kategori|Bekräftad byråspecifik|Tjänstetypisk risk/);
    assert.doesNotMatch(html, />Verksamhet</);
    assert.match(html, /Avstämning/);
    assert.match(html, /Motivering av inneboende risk/);
    assert.match(html, /felaktiga avskrivningar sällan förekommer/);
  ...
  it('renderar övrig riskfaktor med åtgärd och residualbricka', () => {
    const html = View.render({
      ...
      ovriga: [{
        ...
        sannolikhetEfter: 2,
        konsekvensEfter: 3,
        motivering_inneboende_risk: 'Sannolikheten är hög eftersom kontanter hanteras ofta utan full spårbarhet.',
        motivering_residual_risk: 'Residualrisken sänks genom frågor om kassahantering och uppföljning.',
        ptTfRelevans: 'TF'
      }]
    });
    ...
    assert.match(html, /Motivering av inneboende risk/);
    assert.match(html, /Motivering av residualrisk/);
  ...
  it('kan visa bara tjänster eller bara distributionskanaler', () => {
    ...
    assert.doesNotMatch(verk, /identifierade-group-title/);
    assert.doesNotMatch(verk, /Verksamhetsspecifika riskfaktorer:/);
  });
