...
describe('allmän riskbedömning HTML', () => {
    ...
    it('sektion 4 är analys av tjänster och sektion 5 hänvisar till kartläggning i avsnitt 2', () => {
        const start4 = html.indexOf('data-field-id="fld-identifierade-risker"');
        const end4 = html.indexOf('data-card-id="fld-identifierade-nya"');
        const card4 = html.slice(start4, end4);
        assert.match(card4, /4\. Analys av våra produkter och tjänster/);
        assert.doesNotMatch(card4, /id="ar-tjanster-chart"/);
        assert.match(card4, /id="identifierade-risker-live"/);

        const start5 = html.indexOf('data-card-id="fld-identifierade-nya"');
        const end5 = html.indexOf('data-field-id="fld-riskreducerande"');
        const card5 = html.slice(start5, end5);
        assert.match(card5, /5\. Identifierade risker och sårbarheter/);
        assert.match(card5, /2\.1\.2–2\.1\.5/);
        assert.doesNotMatch(card5, /id="ar-kundtyper-body"/);
        assert.match(html, /2\.1\.1 Tjänster/);
