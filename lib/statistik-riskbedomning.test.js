...
const {
  ...
  recordMatchesBolagsform,
  recordMatchesRiskniva,
  drilldownKundBransch,
  drilldownHogriskBransch,
  recordMatchesKundBranschBucket,
  recordMatchesHogriskBransch
...
describe('statistik-riskbedomning', () => {
  it('räknar kundantal per id och namn', () => {
    ...
    assert.equal(maps.hogriskbransch.Kontanthandel, 1);
    assert.ok(maps.hogriskbranschAny >= 1);
  });
  ...
  it('räknar kundexponering per tjänst och markerar saknad kunddata', () => {
    ...
    // Kund1+Kund2 via recTjanst000001; Kund3 har fri text «Löpande bokföring» och räknas inte in
    ...
    assert.match(missing.fel, /kunde inte hämtas/i);
  });

  it('matchar kundtjänster på exact namn eller kopplat id — inte alias', () => {
    const names = { recTjanst000001: 'Bokföring', recTjanst000002: 'ROT/RUT' };
    const viaId = buildTjanstExponering(records, {
      tjanstId: 'recTjanst000001',
      tjanstNamn: 'Bokföring',
      tjanstIdToName: names
    });
    assert.equal(viaId.antal_kunder, 2);
    assert.equal(viaId.kopplad, true);
    assert.ok(viaId.matchade_namn.includes('Bokföring'));
    assert.equal(viaId.matchade_namn.includes('Löpande bokföring'), false);

    const exactText = buildTjanstExponering(records, {
      tjanstNamn: 'Löpande bokföring',
      tjanstIdToName: names
    });
    assert.equal(exactText.antal_kunder, 1);
    assert.equal(exactText.kopplad, true);
    assert.ok(exactText.matchade_namn.includes('Löpande bokföring'));
    assert.equal(exactText.matchade_namn.includes('Bokföring'), false);

    const aliasOnly = buildTjanstExponering(records, {
      tjanstNamn: 'ROT-/RUT-administration',
      tjanstIdToName: names
    });
    assert.equal(aliasOnly.antal_kunder, 0);
    assert.equal(aliasOnly.kopplad, false);

    const momsAlias = buildTjanstExponering(records, {
      tjanstNamn: 'Momsredovisning',
      tjanstIdToName: { recTjanst000001: 'Moms' }
    });
    assert.equal(momsAlias.antal_kunder, 0);

    const foldEqual = buildTjanstExponering(records, {
      tjanstNamn: 'bokforing',
      tjanstIdToName: names
    });
    assert.equal(foldEqual.antal_kunder, 2);
    assert.ok(foldEqual.matchade_namn.includes('Bokföring'));

    const unmatched = buildTjanstExponering(records, {
      tjanstNamn: 'Hållbarhetsrapport',
      tjanstIdToName: names
    });
    assert.equal(unmatched.antal_kunder, 0);
    assert.equal(unmatched.ok, true);
    assert.equal(unmatched.kopplad, false);
    assert.ok(unmatched.saknade.includes('tjänsten kunde inte kopplas till kundernas tjänster'));
  });
  ...
  it('räknar kunder även när intern HTTP-fallback tidigare nollställde allt', () => {
    ...
    assert.ok(Array.isArray(stat.kundBranschBuckets));
    assert.ok(stat.omsattningAnstalldaProfil.length >= 1);
  });

  it('bygger kundBranschBuckets och drilldown till underbransch + kunder', () => {
    const branchRecords = [
      {
        id: 'recBygg1',
        fields: { Namn: 'Bygg Ett AB', Bransch: '41200 - Byggande av bostadshus' }
      },
      {
        id: 'recBygg2',
        fields: { Namn: 'El Två AB', 'SNI kod': '43210 - Elinstallationer' }
      },
      {
        id: 'recFast',
        fields: { Namn: 'Fast AB', Bransch: '68201 - Uthyrning av bostäder' }
      },
      {
        id: 'recIt',
        fields: { Namn: 'IT AB', Bransch: '62100 - Dataprogrammering' }
      }
    ];
    const stat = aggregateStatistik(branchRecords, {});
    const byNamn = Object.fromEntries(stat.kundBranschBuckets.map((r) => [r.namn, r.antal]));
    assert.equal(byNamn['Bygg och anläggning'], 2);
    assert.equal(byNamn.Fastighet, 1);
    assert.equal(byNamn['IT och konsultverksamhet'], 1);

    assert.equal(recordMatchesKundBranschBucket(branchRecords[0].fields, 'Bygg och anläggning'), true);
    assert.equal(recordMatchesKundBranschBucket(branchRecords[2].fields, 'Bygg och anläggning'), false);

    const drill = drilldownKundBransch(branchRecords, 'Bygg och anläggning');
    assert.equal(drill.antalKunder, 2);
    assert.equal(drill.kunder.length, 2);
    assert.ok(drill.undersni.length >= 2);
    assert.ok(drill.undersni.some((u) => /41200/.test(u.namn)));
    assert.ok(drill.undersni.some((u) => /43210/.test(u.namn)));

    const under = drilldownKundBransch(branchRecords, 'Bygg och anläggning', {
      sniFilter: '43210 - Elinstallationer'
    });
    assert.equal(under.antalKunder, 1);
    assert.equal(under.kunder[0].namn, 'El Två AB');
  });

  it('drilldown för högriskbransch listar kunder och underetiketter', () => {
    const drill = drilldownHogriskBransch(records, 'Kontanthandel');
    assert.equal(drill.antalKunder, 1);
    assert.equal(drill.kunder[0].namn, 'Alfa AB');
    assert.equal(recordMatchesHogriskBransch(records[0].fields, 'Kontanthandel'), true);
    assert.equal(recordMatchesHogriskBransch(records[1].fields, 'Kontanthandel'), false);
  });

  it('UI: branschstatistik är klickbar med drilldown till underbransch och kundlista', () => {
    ...
    const modalJs = fs.readFileSync(path.join(__dirname, '../public/js/statistik-kunder-modal.js'), 'utf8');
    assert.match(html, /statistik-bransch-lista/);
    assert.match(html, /statistik-bolagsform-lista/);
    assert.match(html, /statistik-bransch-drilldown/);
    assert.match(html, /statistik-kunder-modal\.js/);
    assert.match(js, /kundBranschBuckets/);
    assert.match(js, /typ:\s*'kund-bransch'/);
    assert.match(js, /typ:\s*'bolagsform'/);
    assert.match(js, /statistik-bolagsform-lista/);
    assert.match(js, /statistik-stat-chip/);
    assert.match(html, /statistik-source-badge/);
    assert.match(html, /Clientflow/);
    assert.match(html, /data-typ="riskniva"/);
    assert.match(html, /data-typ="alla"/);
    assert.match(html, /Bolagsformer/);
    assert.match(modalJs, /bransch-drilldown/);
    assert.match(modalJs, /fetchBranschDrilldown/);
    assert.match(modalJs, /Underbranscher/);
    assert.match(modalJs, /typ === 'bolagsform'/);
    assert.match(modalJs, /typ === 'riskniva'/);
    assert.match(modalJs, /bindStatChips/);
  });

  it('matchar bolagsform och risknivå för kundlista', () => {
    assert.equal(recordMatchesBolagsform({ Bolagsform: 'AB' }, 'Aktiebolag'), true);
    assert.equal(recordMatchesBolagsform({ Bolagsform: 'Aktiebolag' }, 'AB'), true);
    assert.equal(recordMatchesBolagsform({ Bolagsform: 'EF' }, 'Enskild firma'), true);
    assert.equal(recordMatchesBolagsform({ Bolagsform: 'Handelsbolag' }, 'Aktiebolag'), false);
    assert.equal(recordMatchesRiskniva({ Riskniva: 'Låg' }, 'Låg'), true);
    assert.equal(recordMatchesRiskniva({ 'sammanlagd risk': 'Hög' }, 'Hög'), true);
    assert.equal(recordMatchesRiskniva({ Riskniva: 'Låg' }, 'Hög'), false);
  });

  it('API-index stödjer bolagsform, risknivå och alla-kunder', () => {
    const index = fs.readFileSync(path.join(__dirname, '../index.js'), 'utf8');
    assert.match(index, /typ === 'bolagsform'/);
    assert.match(index, /typ === 'riskniva'/);
    assert.match(index, /typ === 'alla'/);
    assert.match(index, /recordMatchesBolagsform/);
    assert.match(index, /recordMatchesRiskniva/);
  });
  ...
  it('tolkar flera tjänst-id:n och exact normaliserat namn', () => {
    assert.deepEqual(parseTjanstRecordIds('recTjanst000001, recTjanst000002'), [
      'recTjanst000001',
      'recTjanst000002'
    ]);
    assert.equal(tjanstNamesMatch('Bokföring', 'bokforing'), true);
    assert.equal(tjanstNamesMatch('ROT/RUT', 'rot rut'), true);
    assert.equal(tjanstNamesMatch('Bokföring', 'Löpande bokföring'), false);
    assert.equal(tjanstNamesMatch('ROT/RUT', 'ROT-/RUT-administration'), false);
  });
  ...
  it('aggregateStatistik räknar bara pågående kunder (inte lead/avslutad/dold)', () => {
    const mixed = [
      { id: 'recPag1', fields: { Namn: 'Pågående AB', Kundstatus: 'Pågående kund', Bolagsform: 'Aktiebolag' } },
      { id: 'recPag2', fields: { Namn: 'Legacy AB', Bolagsform: 'Aktiebolag' } },
      { id: 'recLead', fields: { Namn: 'Lead AB', Kundstatus: 'Lead', Bolagsform: 'Aktiebolag' } },
      { id: 'recAv', fields: { Namn: 'Avslutad AB', Kundstatus: 'Avslutad', Bolagsform: 'Aktiebolag' } },
      { id: 'recAv2', fields: { Namn: 'Avslutade AB', Kundstatus: 'Avslutade', Bolagsform: 'Enskild firma' } },
      { id: 'recDold', fields: { Namn: 'Dold AB', Kundstatus: 'Pågående kund', Dold: true, Bolagsform: 'Aktiebolag' } }
    ];
    const stat = aggregateStatistik(mixed, {});
    assert.equal(stat.antalKunder, 2);
    const byForm = Object.fromEntries(stat.bolagsform.map((r) => [r.namn, r.antal]));
    assert.equal(byForm.Aktiebolag, 2);
    assert.equal(byForm['Enskild firma'], undefined);
  });
});
