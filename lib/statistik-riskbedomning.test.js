...
describe('statistik-riskbedomning', () => {
  ...
  it('räknar kundexponering per tjänst och markerar saknad kunddata', () => {
    ...
    assert.equal(expo.antal_kunder, 3);
  ...
  it('matchar katalognamn mot vanliga alias hos kunderna', () => {
    const names = { recTjanst000001: 'Bokföring', recTjanst000002: 'ROT/RUT' };
    const bokforing = buildTjanstExponering(records, {
      tjanstNamn: 'Löpande bokföring',
      tjanstIdToName: names
    });
    assert.equal(bokforing.antal_kunder, 3);
    assert.equal(bokforing.kopplad, true);
    assert.ok(bokforing.matchade_namn.includes('Bokföring'));
    assert.ok(bokforing.matchade_namn.includes('Löpande bokföring'));

    const rot = buildTjanstExponering(records, {
      tjanstNamn: 'ROT-/RUT-administration',
      tjanstIdToName: names
    });
    assert.equal(rot.antal_kunder, 1);
    assert.ok(rot.matchade_namn.includes('ROT/RUT'));

    const moms = buildTjanstExponering(records, {
      tjanstNamn: 'Momsredovisning',
      tjanstIdToName: { recTjanst000001: 'Moms' }
    });
    assert.equal(moms.antal_kunder, 2);
  ...
  it('tolkar flera tjänst-id:n och aliasnamn', () => {
    assert.deepEqual(parseTjanstRecordIds('recTjanst000001, recTjanst000002'), [
      'recTjanst000001',
      'recTjanst000002'
    ]);
    assert.equal(tjanstNamesMatch('Bokföring', 'Löpande bokföring'), true);
    assert.equal(tjanstNamesMatch('ROT/RUT', 'ROT-/RUT-administration'), true);
  });
