...
describe('ovriga-risk-fields', () => {
  ...
  it('gör PT/TF obligatoriskt i skapa- och redigeraformuläret', () => {
    ...
    const html = AnalyskortMall.renderPageModals('ovriga');
    const page = fs.readFileSync(path.join(__dirname, '../public/ovriga-riskfaktorer.html'), 'utf8');
    const js = fs.readFileSync(path.join(__dirname, '../public/js/ovriga-riskfaktorer.js'), 'utf8');
  ...
  it('listar Riskpoäng, PT/TF-relevans och Aktuell som extra schemafält', () => {
    assert.deepEqual(SCHEMA_FIELDS.map((f) => f.name), ['Riskpoäng', 'PT/TF-relevans', 'Aktuell', 'Hot', 'Sårbarheter', 'AI-extra underlag']);
  });
