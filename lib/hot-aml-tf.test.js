...
describe('hot-aml-tf', () => {
  ...
  it('skärper mot skatte-/kvalitetsrisk och kräver konkret PT/TF-mekanism', () => {
    ...
    assert.match(Hot.AI_RULES, /Avfärda inte terrorismfinansiering/);
    assert.match(Hot.AI_RULES, /ideell organisation eller utlandsbetalning/);
    assert.match(Hot.AI_RULES, /kan användas för att tvätta pengar/);
    assert.match(Hot.AI_RULES, /Falska ROT\/RUT-tjänster/);
    assert.match(Hot.AI_RULES, /felaktiga utbetalningar från Skatteverket/);
    assert.doesNotMatch(Hot.AI_RULES, /spekulativt huvudhot/);
    assert.equal(Hot.assessHot({
      titel: 'Felaktiga ROT-underlag',
      beskrivning: 'Oriktiga ROT-/RUT-ansökningar med överdriven arbetskostnad kan ge felaktiga utbetalningar från Skatteverket. Offentliga medel kan därefter tas ut eller föras vidare.'
    }).ok, true);
  });
