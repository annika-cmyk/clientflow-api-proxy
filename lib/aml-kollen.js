 ...
 * men är anpassade till engångsunderlag (kontoutdrag + SIE) och implementerade separat i denna kodbas.
 */
...
function detectSignals({ bankTxs, ledgerTxs, matchResult, customerBusinessText, relatedPartyNames, grossMargin, scbGrossMargin }) {
  const signals = [];
  const bankUnmatched = matchResult?.bankUnmatched || [];
  const ledgerUnmatched = matchResult?.ledgerUnmatched || [];

  if (ledgerUnmatched.length) {
    signals.push({
...
async function analyzeAmlKollenOnce({
  ...
  const bankParsed = await parseBankStatement(bankStatement.buffer, bankStatement.filename);
  if (!bankParsed.ok) return { ok: false, error: bankParsed.error };

  const sieParsed = parseSie(sieFile.buffer, sieFile.filename);
  ...
  const matchResult = buildMatches(bankTxs, ledgerTxs, { maxDayDiff: 3 });
  ...
  const grossMargin = computeGrossMarginFromSie(sieParsed.allTransactions);
  ...
  return {
    ...
    run: {
      ...
      createdAt,
      inputs: {
        bankFilename: bankStatement.filename,
        sieFilename: sieFile.filename,
      ...
      data: {
        ...
        warnings: [...(bankParsed.warnings || []), ...(rangeCheck.warnings || [])],
