...
describe('kundrisker-profil-analysforslag', () => {
  ...
  it('är inkopplat på kundrisker-sidan', () => {
    ...
    const clientflowJs = fs.readFileSync(path.join(__dirname, '../public/js/kundrisker-clientflow-statistik.js'), 'utf8');
    ...
    assert.match(clientflowJs, /\/api\/statistik-riskbedomning/);
    assert.match(clientflowJs, /statistik-stat-chip/);
    assert.match(clientflowJs, /typ:\s*'bolagsform'|typ: 'bolagsform'/);
    assert.match(clientflowJs, /kund-bransch/);
    assert.match(clientflowJs, /hogriskbransch/);
    assert.match(clientflowJs, /bindStatChips/);
    assert.match(css, /kundrisker-analys-panel/);
    assert.match(css, /kundrisker-analys-btn/);
    assert.match(css, /kundrisker-analys-status/);
    assert.match(css, /kundrisker-clientflow-statistik/);
    assert.match(css, /kundrisker-bransch-chip/);
