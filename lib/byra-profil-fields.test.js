...
describe('byra-profil-fields', () => {
  ...
  it('mappar hela byråprofilen inkl. befintliga fält och flerval', () => {
    ...
    assert.equal(built.fields['Auktoriserade konsulter'], 'Delvis');
  ...
  it('skapar select som singleSelect och flerval som text', () => {
    ...
    assert.deepEqual(byName['Auktoriserade konsulter'].options.choices.map((c) => c.name), ['Ja', 'Nej', 'Delvis']);
