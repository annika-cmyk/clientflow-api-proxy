...
function sampleValue(field) {
  ...
  if (field.type === 'risk-fordelning' || field.key === 'kundResidualriskFordelning') {
    return 'Låg: 2, Normal: 5';
  }
...
describe('byra-profil-fields', () => {
  ...
  it('har sju sektioner med fieldKeys (beroende/outsourcing/högrisk ihopslagna)', () => {
    assert.equal(BYRA_PROFIL_SECTIONS.length, 7);
    ...
    assert.ok(BYRA_PROFIL_FIELDS.some((f) => f.key === 'storaKundberoenden' && f.section === 'intern'));
    assert.ok(BYRA_PROFIL_FIELDS.some((f) => f.key === 'outsourcingUnderleverantorer' && f.section === 'intern'));
    assert.ok(BYRA_PROFIL_FIELDS.some((f) => f.key === 'bolagsbildningAtKund' && f.section === 'tjanster'));
  });
  ...
  it('skapar select som singleSelect och flerval som text', () => {
    ...
    assert.equal(airtableTypeForField(BYRA_PROFIL_FIELDS.find((f) => f.key === 'kundResidualriskFordelning')), 'multilineText');
...
describe('kunder enkät-sammanfattning', () => {
  ...
  it('visar ifyllda kundstock-svar och slår ihop antal', () => {
    const summary = buildKunderEnkatSummary({
      antalKunder: 42,
      kundResidualriskFordelning: 'Låg: 10, Normal: 25, Förhöjd: 5, Hög: 2',
    ...
    assert.ok(kundstock);
    const risk = kundstock.items.find((i) => i.key === 'kundResidualriskFordelning');
    assert.ok(risk);
    assert.match(risk.display, /Låg · 10/);
    assert.match(risk.display, /Normal · 25/);
    assert.equal(risk.label, 'Kundernas residualrisk');
  ...
  it('har residualrisk-fördelning i kundstock efter antal kunder', () => {
    const field = BYRA_PROFIL_FIELDS.find((f) => f.key === 'kundResidualriskFordelning');
    assert.ok(field);
    assert.equal(field.type, 'risk-fordelning');
    assert.equal(field.section, 'kundstock');
    assert.deepEqual(field.choices, ['Låg', 'Normal', 'Förhöjd', 'Hög', 'Oacceptabel']);
    assert.match(field.question, /låg \/ normal \/ förhöjd \/ hög \/ oacceptabel/i);
    const keys = BYRA_PROFIL_SECTIONS.find((s) => s.id === 'kundstock').fieldKeys;
    const antalIdx = keys.indexOf('antalKunder');
    const riskIdx = keys.indexOf('kundResidualriskFordelning');
    assert.ok(antalIdx >= 0 && riskIdx === antalIdx + 1);
    assert.ok(textStoredAirtableNames().includes('Kund residualrisk fördelning'));
  });
