...
5. Om ptTfRelevans = «Båda» måste minst ETT hot beskriva TF-mekanismen (terrorismfinansiering/värdebärare), även om risken är lägre för just denna kundtyp.
...
function validateOvrigAiPayload(payload, opts = {}) {
  ...
  if (thinHot.length) {
    violations.push({
      id: 'hot_for_tunna',
      message:
        'Minst ett hot är för tunt. Varje hot ska beskriva modus steg för steg (pengar in → bokföring/export → hur byrån kan upptäcka det), inte bara en mening.'
    });
  }
  ...
  if (product >= 10) {
    if (hot.filter((h) => h && (h.titel || h.beskrivning)).length < 3) {
      violations.push({
        id: 'for_fa_hot',
        message:
          'Inneboende risk är Förhöjd/Hög (S×K ≥ 10) — minst 3 hot med modus krävs.'
      });
    }
