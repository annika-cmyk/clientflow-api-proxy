...
function planDokumentEdit({ fields, sourceField, sourceIndex, displayName, category, customCategory, createdDate, subcategory }) {
  ...
  if (!hasDisplayName && !hasCategory && !hasCreatedDate && !hasSubcategory) {
  ...
  if (nextCategory === 'kyc') nextCategory = dokumentRiskSubkategori.AKTUELL_RISK_CATEGORY;
  ...
  if (hasCreatedDate) {
    ...
    if (!parsed) return { error: 'Ogiltigt skapat datum (t.ex. 2026-01-01, 20260101 eller 260101)', status: 400 };
  }
  ...
  const isRiskCat = nextCategory === dokumentRiskSubkategori.AKTUELL_RISK_CATEGORY
    || nextCategory === dokumentRiskSubkategori.HISTORIK_RISK_CATEGORY;
  if (hasSubcategory && isRiskCat) {
    const sub = dokumentRiskSubkategori.normalizeSubcategory(subcategory);
    updates.subcategory = sub || 'ovrigt_risk';
  } else if (isRiskCat && existing?.subcategory) {
    updates.subcategory = dokumentRiskSubkategori.normalizeSubcategory(existing.subcategory) || existing.subcategory;
  } else if (nextCategory === dokumentRiskSubkategori.AKTUELL_RISK_CATEGORY && hasCategory && !existing?.subcategory) {
    updates.subcategory = 'ovrigt_risk';
  }

  const nextKategorier = dokumentKategori.upsertDokumentKategori(kategorier, att, updates);
  // Kategori styrs via metadata – flytta inte bilagor mellan Airtable-fält (ger ofta 422).
  const patchFields = {
