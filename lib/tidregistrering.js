...
const { extractMejlLink, stripMejlLinkLine } = require('./mejl-link');

const MEJL_LINK_FIELD = 'Mejl-länk';
...
function mejlUrlFromRecord(fields) {
  const f = fields || {};
  const stored = String(f[MEJL_LINK_FIELD] || '').trim();
  if (stored) return stored;
  return extractMejlLink(f.Beskrivning);
}

/** Ämne/beskrivning utan inbäddad mejllänk när länken bara låg i fritext. */
function descriptionFromRecord(fields) {
  const f = fields || {};
  const raw = String(f.Beskrivning || '').trim();
  const stored = String(f[MEJL_LINK_FIELD] || '').trim();
  if (stored) return raw;
  return stripMejlLinkLine(raw, extractMejlLink(raw));
}
...
function recordToEntry(rec) {
  ...
  return {
    ...
    description: descriptionFromRecord(f),
    mejlUrl: mejlUrlFromRecord(f),
...
function buildEntryFields(input, { forCreate = false, user } = {}) {
  ...
  const mejlUrl = String(
    input.mejlUrl != null ? input.mejlUrl : input[MEJL_LINK_FIELD] != null ? input[MEJL_LINK_FIELD] : ''
  ).trim();
  if (mejlUrl) fields[MEJL_LINK_FIELD] = mejlUrl;
...
const REQUIRED_FIELDS = [
  ...
  { name: MEJL_LINK_FIELD, type: 'singleLineText' },
...
function isUnknownMejlLinkField(err) {
  const msg = String(err?.response?.data?.error?.message || err?.message || '');
  return /Mejl-länk/i.test(msg);
}

/**
 * Skriv tidpost med Mejl-länk. Skapar fältet om det saknas.
 * Om schemat inte går att utöka sparas länken på egen rad i Beskrivning
 * (ämnet ligger kvar ovanför) så recordToEntry kan lyfta ut den.
 */
async function writeEntrySavingMejlLink(ctx, fields, { id } = {}) {
  const link = String(fields[MEJL_LINK_FIELD] || '').trim();
  const write = (next) =>
    id ? updateEntry({ ...ctx, id, fields: next }) : createEntry({ ...ctx, fields: next });
  if (link) {
    try {
      await ensureTableAndFields({ token: ctx.token, baseId: ctx.baseId });
    } catch (err) {
      console.warn('ensure Mejl-länk:', err.message);
    }
  }
  try {
    return await write(fields);
  } catch (err) {
    if (!link || !isUnknownMejlLinkField(err)) throw err;
    const fallback = { ...fields };
    delete fallback[MEJL_LINK_FIELD];
    const desc = String(fallback.Beskrivning || '').trim();
    if (!desc.includes(link)) fallback.Beskrivning = desc ? `${desc}\n${link}` : link;
  ...
  function registerRoutes(app) {
    ...
    app.post('/api/tidregistrering', authenticateToken, async (req, res) => {
      try {
        ...
        const entry = await writeEntrySavingMejlLink(ctx, fields);
        res.status(201).json({
          entry,
    ...
    app.put('/api/tidregistrering/:id', authenticateToken, async (req, res) => {
      try {
        ...
        const merged = {
          ...
          mejlUrl: body.mejlUrl != null ? body.mejlUrl : existing.mejlUrl,
        ...
        const entry = await writeEntrySavingMejlLink(ctx, fields, { id });
        res.json({
          entry,
          invoiceLine: toInvoiceLine(entry),
          rateResolved: resolved || undefined
