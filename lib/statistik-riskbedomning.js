...
function recordMatchesBolagsform(fields, paramNamn) {
  const sok = String(paramNamn == null ? '' : paramNamn).trim();
  if (!sok) return false;
  const f = fields || {};
  const kyc = parseKycJson(f);
  const label = normalizeBolagsform(f['Bolagsform'] || kyc.bolagsform || '');
  if (!label) return false;
  if (label === sok || foldLabel(label) === foldLabel(sok)) return true;
  const sokNorm = normalizeBolagsform(sok);
  return !!(sokNorm && (label === sokNorm || foldLabel(label) === foldLabel(sokNorm)));
}

function recordMatchesRiskniva(fields, paramNamn) {
  const sok = String(paramNamn == null ? '' : paramNamn).trim();
  if (!sok) return false;
  const rn = Riskaptit.resolveResidualNiva(fields || {});
  if (sok === 'Övrigt' || foldLabel(sok) === 'övrigt') {
    if (!rn) return false;
    const bag = RiskSkala.emptyCounts();
    return !Object.prototype.hasOwnProperty.call(bag, rn);
  }
  if (!rn) return false;
  if (RiskSkala.sameLevel) return RiskSkala.sameLevel(rn, sok);
  return rn === sok || foldLabel(rn) === foldLabel(sok);
}
...
function broadCategoryFromLabel(label) {
  return KundBranschAggregat.mapToCommonKundBransch(label);
}
...
function industryLabelsForBucket(fields) {
  return KundBranschAggregat.industryLabelsFromFields(fields || {}, {
    asValues,
    parseKycJson
  });
}

function customerNameFromRecord(rec) {
  const f = (rec && rec.fields) || {};
  return String(f.Namn || f.Kundnamn || '').trim() || 'Namn saknas';
}

function labelsInKundBranschBucket(fields, bucket) {
  const target = String(bucket || '').trim();
  if (!target) return [];
  return industryLabelsForBucket(fields).filter(
    (label) => KundBranschAggregat.mapToCommonKundBransch(label) === target
  );
}

function recordMatchesKundBranschBucket(fields, bucket) {
  return labelsInKundBranschBucket(fields, bucket).length > 0;
}

function recordMatchesUnderbranschLabel(fields, label) {
  const target = foldLabel(label);
  if (!target) return false;
  return industryLabelsForBucket(fields).some((l) => foldLabel(l) === target);
}

function recordMatchesHogriskBransch(fields, namn) {
  const target = String(namn || '').trim();
  if (!target) return false;
  return asValues((fields || {})['Kunden verkar i en högriskbransch']).some((b) => String(b || '').trim() === target)
    || HogriskSni.matchSni(
      HogriskSni.sniRawFromFields ? HogriskSni.sniRawFromFields(fields || {}) : ''
    ).branscher.some((b) => String(b || '').trim() === target);
}

function toNamedListFromMap(map) {
...
/**
 * Drill-down: översiktsbransch → underbranscher (SNI/etikett) + kundlista.
 * opts.sniFilter / opts.underbransch begränsar kundlistan till en underbransch.
 */
function drilldownKundBransch(records, bucket, opts) {
  const options = opts || {};
  const underFilter = String(options.sniFilter || options.underbransch || '').trim();
  const underFold = foldLabel(underFilter);
  const underAntal = {};
  const kunder = [];
  const list = Array.isArray(records) ? records : [];

  for (const rec of list) {
    const matching = labelsInKundBranschBucket((rec && rec.fields) || {}, bucket);
    if (!matching.length) continue;
    matching.forEach((label) => pushCount(underAntal, label));
    if (underFold && !matching.some((label) => foldLabel(label) === underFold)) continue;
    kunder.push({ id: rec.id, namn: customerNameFromRecord(rec) });
  }

  return {
    typ: 'kund-bransch',
    bucket: String(bucket || '').trim(),
    underbransch: underFilter || null,
    undersni: toNamedListFromMap(underAntal),
    kunder,
...
/**
 * Drill-down för högriskbransch: underliggande SNI-/branschetiketter + kunder.
 */
function drilldownHogriskBransch(records, namn, opts) {
  const options = opts || {};
  const underFilter = String(options.sniFilter || options.underbransch || '').trim();
  const underFold = foldLabel(underFilter);
  const underAntal = {};
  ...
  for (const rec of list) {
    const f = (rec && rec.fields) || {};
    if (!recordMatchesHogriskBransch(f, target)) continue;
    const detailLabels = industryLabelsForBucket(f);
    if (detailLabels.length) {
      detailLabels.forEach((label) => pushCount(underAntal, label));
    } else {
      pushCount(underAntal, target);
    }
    if (underFold) {
      const ok = detailLabels.length
        ? detailLabels.some((label) => foldLabel(label) === underFold)
        : foldLabel(target) === underFold;
      if (!ok) continue;
    }
    kunder.push({ id: rec.id, namn: customerNameFromRecord(rec) });
  }
...
function aggregateStatistik(records, lookups) {
  // Endast pågående kunder — leads, avslutade och dolda ska aldrig räknas i Clientflow-statistik.
  ...
  const kundBranschBuckets = KundBranschAggregat.countKundBranschBuckets(list, {
    asValues,
    parseKycJson
  });
...
function buildKundantalMaps(records) {
  ...
  const risksankande = {};
  const hogriskbransch = {};
  const tjanstCustomerSets = {};
  const riskfaktorCustomerSets = {};
  const hogriskCustomerSets = {};
  const hogriskAnySet = new Set();
  ...
  for (const rec of list) {
    ...
    const hogLabels = new Set();
    const fromField = KundRiskprofil && KundRiskprofil.hogriskBranschVal
      ? KundRiskprofil.hogriskBranschVal(f)
      : asValues(f['Kunden verkar i en högriskbransch']);
    fromField.forEach((label) => {
      const t = String(label || '').trim();
      if (t) hogLabels.add(t);
    });
    const sniRaw = HogriskSni.sniRawFromFields ? HogriskSni.sniRawFromFields(f) : '';
    HogriskSni.matchSni(sniRaw).branscher.forEach((b) => {
      const t = String(b || '').trim();
      if (t) hogLabels.add(t);
    });
    for (const label of hogLabels) {
      if (!hogriskCustomerSets[label]) hogriskCustomerSets[label] = new Set();
      hogriskCustomerSets[label].add(rec.id);
      hogriskAnySet.add(rec.id);
    }
  }
  ...
  for (const [label, set] of Object.entries(hogriskCustomerSets)) {
    hogriskbransch[label] = set.size;
  }
  ...
  return {
    ...
    risksankande,
    hogriskbransch,
    hogriskbranschAny: hogriskAnySet.size
  };
}
...
/**
 * Exact (normalized) name equality for Clientflow statistik/exponering.
 * Deliberately does NOT use mall-alias tables — byrån owns duplicate service names.
 * Mall lookup / opening risk records may still use broader helpers elsewhere.
 */
...
module.exports = {
  ...
  industryLabelsForBucket,
  labelsInKundBranschBucket,
  recordMatchesKundBranschBucket,
  recordMatchesUnderbranschLabel,
