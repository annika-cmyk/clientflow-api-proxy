...
function customerUsesTjanst(fields, opts) {
  ...
  const wantedNamn = foldTjanstNamn(opts && opts.tjanstNamn);
  ...
  for (const raw of ids) {
    ...
    const namn = tjanstNamnForRawId(raw, tjanstIdToName);
    if (wantedNamn && namn && foldTjanstNamn(namn) === wantedNamn) return true;
  }
  return false;
}
...
function buildTjanstExponering(records, opts) {
  ...
  const saknade = [];
  if (!opts || (!opts.tjanstId && !opts.tjanstNamn)) {
