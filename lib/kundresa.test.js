...
describe('kundresa', () => {
  ...
  it('omits VH step for enskild firma and renumbers progress', () => {
    const summary = Kundresa.buildKundresaSummary({
      fields: {
        Namn: 'Anna Andersson',
        Orgnr: '198001011234',
        ...
        'Entity screening datum': '2026-08-01'
      },
      form: Kundformular.emptyForm(),
      kyc: {}
    });
    assert.equal(summary.vhGate.relevant, false);
    assert.equal(summary.vhGate.blocked, false);
    assert.equal(summary.total, 5);
    assert.equal(summary.steps.length, 5);
    assert.ok(!summary.steps.some((s) => s.key === 'vh_ombud'));
    ...
    assert.deepEqual(summary.steps.map((s) => s.number), [1, 2, 3, 4, 5]);
    assert.equal(summary.steps[0].status, 'done');
    assert.equal(summary.steps[1].status, 'done');
    assert.equal(summary.steps[1].key, 'screening');
    assert.equal(summary.steps[2].status, 'next');
    assert.equal(summary.steps[2].key, 'kundformular');
    assert.match(summary.progressLabel, /2 av 5/);
  });

  it('omits VH step for fysisk person', () => {
    const summary = Kundresa.buildKundresaSummary({
      fields: { Bolagsform: 'Fysisk person' },
      form: Kundformular.emptyForm()
    });
    assert.equal(summary.total, 5);
    assert.ok(!summary.steps.some((s) => s.key === 'vh_ombud'));
  ...
  it('keeps VH step for aktiebolag', () => {
    const summary = Kundresa.buildKundresaSummary({
      fields: { Bolagsform: 'Aktiebolag' },
      form: Kundformular.emptyForm()
    });
    assert.equal(summary.total, 6);
    assert.equal(summary.steps[1].key, 'vh_ombud');
    assert.equal(summary.steps[1].number, 2);
    assert.equal(summary.steps[1].title, 'Verifiera VH');
    assert.equal(summary.vhGate.relevant, true);
  });
