...
const AiTjanstAnalys = require('./ai-tjanst-analys');
...
function mapByraTjanstRecord(record) {
    ...
    const sarbarheter = parseJsonList(f['Sårbarheter']).map((s) => Object.assign({}, s, {
        beskrivning: AiTjanstAnalys.stripEvidensLeakFromText(s.beskrivning ?? s.description)
    }));
