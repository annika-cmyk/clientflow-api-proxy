...
describe('openai-assistant-run (Responses API)', () => {
  ...
  it('sätter max_output_tokens och prompt_cache_key när de anges', () => {
    const body = buildResponsesPayload({
      input: 'x',
      temperature: 0,
      maxOutputTokens: 8192,
      promptCacheKey: 'cf-byra-tjanst-gen'
    });
    assert.equal(body.max_output_tokens, 8192);
    assert.equal(body.prompt_cache_key, 'cf-byra-tjanst-gen');
  });

  it('resolveByraAnalysModel föredrar OPENAI_BYRA_ANALYS_MODEL', () => {
    const prev = {
      BYRA: process.env.OPENAI_BYRA_ANALYS_MODEL,
      MODEL: process.env.OPENAI_MODEL
    };
    try {
      delete process.env.OPENAI_BYRA_ANALYS_MODEL;
      delete process.env.OPENAI_MODEL;
      assert.equal(resolveByraAnalysModel(), 'gpt-4o');
      process.env.OPENAI_MODEL = 'gpt-4.1';
      assert.equal(resolveByraAnalysModel(), 'gpt-4.1');
      process.env.OPENAI_BYRA_ANALYS_MODEL = 'gpt-4.1-mini';
      assert.equal(resolveByraAnalysModel(), 'gpt-4.1-mini');
      assert.equal(resolveByraAnalysModel('explicit-model'), 'explicit-model');
    } finally {
      if (prev.BYRA == null) delete process.env.OPENAI_BYRA_ANALYS_MODEL;
      else process.env.OPENAI_BYRA_ANALYS_MODEL = prev.BYRA;
      if (prev.MODEL == null) delete process.env.OPENAI_MODEL;
      else process.env.OPENAI_MODEL = prev.MODEL;
    }
  });
  ...
  it('läser token-usage från Responses API', () => {
    assert.equal(extractResponsesUsage(null), null);
    assert.deepEqual(extractResponsesUsage({
      usage: {
        input_tokens: 1200,
        output_tokens: 80,
        total_tokens: 1280,
  ...
  it('väljer kunskapsbas för tjänst- och övrig-analys i rätt ordning', () => {
    ...
    assert.match(BYRA_ANALYS_KUNSKAPSBAS_RULES, /irrelevanta träffar/);
    assert.match(BYRA_ANALYS_KUNSKAPSBAS_RULES, /exakta/);
  });

  it('fyller debugOut med prompt och råsvar för chatt-vyn', () => {
    const body = buildResponsesPayload({
      model: 'gpt-4o-mini',
      instructions: 'Var tydlig.',
      input: 'Hej',
      vectorStoreId: 'vs_x',
      temperature: 0.4
    });
    const out = {};
    assignAiRunDebugOut(out, {
      body,
      userContent: 'Hej användare',
      conversationId: 'conv_abc',
      status: 'completed',
      rawResponse: 'Svarstext',
      rawResponseJson: { id: 'resp_1', output_text: 'Svarstext' }
    });
    assert.equal(out.model, 'gpt-4o-mini');
    assert.equal(out.temperature, 0.4);
    assert.equal(out.hasFileSearch, true);
    assert.equal(out.instructions, 'Var tydlig.');
    assert.equal(out.prompt, 'Hej användare');
    assert.equal(out.conversationId, 'conv_abc');
    assert.equal(out.status, 'completed');
    assert.equal(out.rawResponse, 'Svarstext');
    assert.match(out.rawResponseJsonText, /resp_1/);
    assert.equal(assignAiRunDebugOut(null, { body }), null);
  });

  it('avkortar mycket stora råsvar i debug-text', () => {
    const big = { blob: 'x'.repeat(5000) };
    const text = stringifyAiRunDebugRaw(big, 200);
    assert.ok(text.length < 250);
    assert.match(text, /avkortad/);
  });

  it('toAiRunDebugPayload bygger klientvänlig debug utan råobjekt', () => {
    const out = {};
    assignAiRunDebugOut(out, {
      body: buildResponsesPayload({ model: 'gpt-4o', instructions: 'Sys', input: 'x', temperature: 0.2 }),
      userContent: 'Hej',
      conversationId: 'conv_1',
      status: 'completed',
      rawResponse: 'svar',
      rawResponseJson: { id: 'resp_x' }
    });
    const payload = toAiRunDebugPayload(out, { route: '/api/ai-ovriga-riskfaktor', label: 'Riskfaktoranalys' });
    assert.equal(payload.label, 'Riskfaktoranalys');
    assert.equal(payload.route, '/api/ai-ovriga-riskfaktor');
    assert.equal(payload.prompt, 'Hej');
    assert.match(payload.rawResponseJsonText, /resp_x/);
    assert.equal(payload.rawResponseJson, undefined);
  });
