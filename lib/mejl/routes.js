  ...
  function registerRoutes(app) {
    ...
    app.post('/api/mejl/compose-prepare', authenticateToken, async (req, res) => {
      try {
        ...
        if (!user) return;
        const body = req.body || {};
        const publicText = String(body.publicText || body.text || '');
        const quotedText = body.quotedText != null ? String(body.quotedText) : '';
        const quoteFrom = String(body.quoteFrom || '').trim();
        const quoteDate = body.quoteDate != null ? body.quoteDate : '';
        ...
        const built = composeHtml.buildOutgoingEmail(composeOpts);
        const text = composeHtml.buildOutgoingText(composeOpts);
        ...
        res.json({
          ...
          signatureAttached: built.signatureAttached,
