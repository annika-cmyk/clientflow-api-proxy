...
function contactLinesHtml(settings, accent) {
  const rows = [];
  if (settings.name) {
    rows.push(
      `<div style="font-size:18px;font-weight:700;color:${accent};line-height:1.3;margin:0 0 2px 0;">${escapeHtml(settings.name)}</div>`
    );
  }
  if (settings.title) {
    rows.push(
      `<div style="font-size:13px;font-weight:700;color:#111827;margin:0 0 8px 0;">${escapeHtml(settings.title)}</div>`
    );
  }
  if (settings.phone) {
    rows.push(
      `<div style="font-size:13px;color:#111827;margin:0 0 2px 0;"><strong>Telefon:</strong> ${escapeHtml(settings.phone)}</div>`
    );
  }
  if (settings.email) {
    rows.push(
      `<div style="font-size:13px;color:#111827;margin:0 0 2px 0;"><strong>E-post:</strong> <a href="mailto:${escapeHtml(settings.email)}" style="color:#111827;text-decoration:none;">${escapeHtml(settings.email)}</a></div>`
    );
  }
  if (settings.address) {
    rows.push(
      `<div style="font-size:13px;color:#111827;margin:0 0 2px 0;">${escapeHtml(settings.address)}</div>`
    );
  }
  if (settings.website) {
    ...
    rows.push(
      `<div style="font-size:13px;margin:0 0 2px 0;"><a href="${escapeHtml(href)}" style="color:${accent};text-decoration:none;">${escapeHtml(label)}</a></div>`
    );
  }
  if (settings.freeText) {
    const free = escapeHtml(settings.freeText).replace(/\n/g, '<br />');
    rows.push(`<div style="font-size:13px;color:#111827;margin:10px 0 0 0;line-height:1.45;">${free}</div>`);
  }
  return rows.join('');
}
...
function disclaimerHtml(settings) {
  ...
  return `<div style="margin-top:14px;padding-top:10px;border-top:1px solid #d1d5db;font-size:11px;color:#6b7280;line-height:1.4;">${text}</div>`;
}

function renderSignatureHtml(rawSettings) {
  const settings = normalizeSettings(rawSettings);
  const accent = /^#[0-9a-fA-F]{3,8}$/.test(settings.accentColor)
    ? settings.accentColor
    : DEFAULT_SETTINGS.accentColor;
  const portrait = pickImage(settings, 1);
  const logo = pickImage(settings, 2);
  const contact = contactLinesHtml(settings, accent);
  ...
  if (layout === 'portrait-left-text-right') {
    return `<div style="font-family:Arial,Helvetica,sans-serif;margin-top:20px;padding-top:14px;border-top:1px solid #374151;">
  ...
  if (layout === 'stacked-centered') {
    return `<div style="font-family:Arial,Helvetica,sans-serif;margin-top:20px;padding-top:14px;border-top:1px solid #374151;text-align:center;">
  ...
  if (layout === 'compact-inline') {
    const bits = [];
    if (settings.name) bits.push(`<strong style="color:${accent};">${escapeHtml(settings.name)}</strong>`);
    if (settings.title) bits.push(escapeHtml(settings.title));
    if (settings.phone) bits.push(escapeHtml(settings.phone));
    if (settings.email) {
      bits.push(
        `<a href="mailto:${escapeHtml(settings.email)}" style="color:#111827;text-decoration:none;">${escapeHtml(settings.email)}</a>`
      );
    }
    if (settings.website) {
      const href = linkifyWebsite(settings.website);
      bits.push(
        `<a href="${escapeHtml(href)}" style="color:${accent};text-decoration:none;">${escapeHtml(settings.website.replace(/^https?:\/\//i, ''))}</a>`
      );
    }
    return `<div style="font-family:Arial,Helvetica,sans-serif;margin-top:16px;padding-top:10px;border-top:1px solid #374151;font-size:12px;color:#111827;line-height:1.5;">
  ...
  ${settings.address ? `<div style="margin-top:4px;color:#4b5563;">${escapeHtml(settings.address)}</div>` : ''}
