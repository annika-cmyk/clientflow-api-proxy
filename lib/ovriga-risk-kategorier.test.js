...
describe('ovriga-risk-kategorier', () => {
  it('delar in övriga riskfaktorer i samarbete, kund och verksamhet', () => {
    ...
    assert.ok(Kat.labelsForCategory('samarbete').includes('Kunden erbjuder pengar, ersättning eller ovanliga förmåner'));
    assert.equal(Kat.findFactor('Kunden erbjuder pengar, ersättning eller ovanliga förmåner').klass, 'GOLV_HOG');
    assert.equal(Kat.categoryFor('Kunden erbjuder pengar, ersättning eller ovanliga förmåner'), 'samarbete');
  ...
  it('kontant, krypto och högriskländer ingår inte i varningsflaggkatalogen', () => {
    ...
    assert.equal(katalog['Kunden erbjuder pengar, ersättning eller ovanliga förmåner'], undefined);
  });

  it('döljer övriga förslag på kundkort tills byrån aktiverat dem i katalogen', () => {
    assert.ok(!Kat.factorsForCategory('verksamheten', { kundkort: true, activeKatalog: {} })
      .some((f) => f.label.includes('osanna fakturor')));
    assert.ok(!Kat.factorsForCategory('samarbete', { kundkort: true, activeKatalog: {} })
      .some((f) => f.label === 'Kunden erbjuder pengar, ersättning eller ovanliga förmåner'));
    const katalog = { 'Fakturor utan koppling till kundens verksamhet / misstanke om osanna fakturor': 'GOLV_HOG' };
    assert.ok(Kat.factorsForCategory('verksamheten', { kundkort: true, activeKatalog: katalog })
      .some((f) => f.label.includes('osanna fakturor')));
    assert.ok(Kat.isInActiveKatalog('Fakturor utan koppling till kundens verksamhet / misstanke om osanna fakturor', katalog));
  });

  it('listar valbara övriga förslag per kategori utan redan aktiva flaggor', () => {
    const samarbete = Kat.suggestedFactorsForCategory('samarbete', []);
    assert.ok(samarbete.some((f) => f.label === 'Kunden erbjuder pengar, ersättning eller ovanliga förmåner'));
    assert.ok(samarbete.some((f) => f.label === 'Omotiverat hög kunskap om penningtvättsregelverket'));
    assert.ok(Kat.suggestedFactorsForCategory('kunden', []).some((f) => f.label === 'PEP, familjemedlem eller känd medarbetare till PEP'));
    assert.ok(Kat.suggestedFactorsForCategory('verksamheten', []).some((f) => f.label.includes('osanna fakturor')));
    const utanMuta = Kat.suggestedFactorsForCategory('samarbete', ['Kunden erbjuder pengar, ersättning eller ovanliga förmåner']);
    assert.ok(!utanMuta.some((f) => f.label === 'Kunden erbjuder pengar, ersättning eller ovanliga förmåner'));
  });
  ...
  it('lägger vanliga riskfaktorer och varningsflaggor i samma A/B/C-kort', () => {
    ...
    assert.match(js, /riskerForCheckboxList/);
    assert.match(js, /riskerForCheckboxList = riskerForList\.filter/);
    assert.doesNotMatch(js, /riskerForCheckboxList = embedded\s*\?\s*valda/);
    assert.match(js, /Aktuell'\] !== false/);
    assert.doesNotMatch(js, /fields\['Aktuell'\] === true/);
    assert.match(js, /cat\.id === 'kunden'/);
  ...
  it('håller högriskbransch infälld och tillåter SNI plus manuell ifyllning', () => {
    ...
    assert.match(html, /kundkort\.js\?v=/);
  ...
  it('har kategorival i katalogen för övriga varningsflaggor', () => {
    ...
    assert.match(html, /Övriga förslag/);
    assert.match(html, /ovriga-riskfaktorer\.js\?v=/);
    assert.match(html, /ovriga-risk-kategorier\.js\?v=/);
    assert.match(html, /styles\.css\?v=/);
    assert.match(js, /riskhoj-katalog-forslag/);
    assert.match(js, /riskhoj-forslag-chip/);
    assert.match(js, /suggestedFactorsForCategory/);
    assert.match(js, /riskhoj-katalog-kategori/);
    assert.match(js, /mergeRiskhojandeEntries/);
    assert.match(css, /\.riskhoj-forslag-chip/);
  ...
  it('lägger byrå-geo under Vem är kunden? och motpart-geo under Vad gör kunden?', () => {
    const js = fs.readFileSync(path.join(__dirname, '../public/js/kundkort.js'), 'utf8');
    const css = fs.readFileSync(path.join(__dirname, '../public/styles.css'), 'utf8');
    const dim = fs.readFileSync(path.join(__dirname, '../public/js/risk-dimensioner.js'), 'utf8');
    assert.match(js, /ovriga-risk-katalog--geo/);
    assert.match(js, /id="ovrigkyc-risker-geografiska"/);
    assert.match(js, /id="ovrigkyc-risker-geografiska_motparter"/);
    assert.match(js, /_collectEmbeddedRiskerPayload/);
    assert.match(js, /Länder kunden handlar med/);
    assert.match(js, /=== 'kunden'\) ids\.unshift\('geografiska'\)/);
    assert.match(js, /=== 'verksamheten'\) ids\.unshift\('geografiska_motparter'\)/);
    assert.doesNotMatch(js, /id="risker-kort-geografiska"/);
    assert.match(css, /\.ovriga-risk-katalog--geo/);
    assert.match(dim, /id: 'geografiska'[\s\S]*?cardTitle: 'Vem är kunden\?'/);
    assert.match(dim, /id: 'geografiska_motparter'[\s\S]*?cardTitle: 'Vad gör kunden\?'/);
    assert.equal(Kat.categoryById('verksamheten').hint.includes('leverantörer'), true);
    assert.equal(Kat.categoryById('kunden').hint.includes('hemvist'), true);
    assert.equal(Kat.categoryIdForDimension('geografiska'), 'kunden');
    assert.equal(Kat.categoryIdForDimension('geografiska_motparter'), 'verksamheten');
  });
