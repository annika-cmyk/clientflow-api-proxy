...
describe('ovriga-risk-kategorier', () => {
  it('delar in övriga riskfaktorer i samarbete, kund och verksamhet', () => {
    ...
    assert.deepEqual(Kat.filledDimensionsFromLabels(['Fysiskt möte', 'PEP eller RCA']), {
      distribution: true,
      kund: true
    });
    assert.ok(Kat.factorsForCategory('samarbete').some((f) => f.label === 'Fysiskt möte'));
  ...
  it('behåller val i andra kategorier när en kategori sparas', () => {
    ...
    const visible = Kat.mergeVisibleVal(
      ['Fysiskt möte', 'Ofta bytt redovisningskonsult/revisor utan naturlig förklaring', 'Egen byråflagga'],
      ['Svårt att bekräfta identitet']
    );
    assert.deepEqual(visible, ['Svårt att bekräfta identitet', 'Egen byråflagga']);
  });
  ...
  it('ligger på kundkortet som ett kort med tre kategorier', () => {
    ...
    assert.match(js, /isCoveredOvriga/);
    assert.match(js, /Alla övriga varningsflaggor samlas här/);
