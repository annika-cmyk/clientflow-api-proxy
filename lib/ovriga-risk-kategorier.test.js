...
describe('ovriga-risk-kategorier', () => {
  ...
  it('lägger vanliga riskfaktorer och varningsflaggor i samma A/B/C-kort', () => {
    ...
    assert.match(js, /embedded\s*\?\s*valda/);
    const embeddedDecl = js.indexOf('const embedded = !!(opts && opts.embedded)');
    const embeddedUse = js.indexOf('riskerForCheckboxList = embedded');
    assert.ok(embeddedDecl > 0 && embeddedUse > embeddedDecl, 'embedded måste deklareras före riskerForCheckboxList');
  ...
  it('håller högriskbransch infälld och tillåter SNI plus manuell ifyllning', () => {
    ...
    assert.match(html, /kundkort\.js\?v=16\.23/);
    assert.match(html, /kyc-verksamhet-styrning\.js/);
    assert.match(html, /styles\.css\?v=20260827c/);
  ...
  it('har kategorival i katalogen för övriga varningsflaggor', () => {
    ...
    assert.match(html, /ovriga-riskfaktorer\.js\?v=33/);
    assert.match(html, /ovriga-risk-kategorier\.js\?v=11/);
    assert.match(html, /styles\.css\?v=20260827d/);
  ...
  it('lägger geografiska residualer under Vem är kunden?', () => {
    const js = fs.readFileSync(path.join(__dirname, '../public/js/kundkort.js'), 'utf8');
    const css = fs.readFileSync(path.join(__dirname, '../public/styles.css'), 'utf8');
    ...
    assert.equal(Kat.categoryById('verksamheten').hint.includes('bransch'), true);
