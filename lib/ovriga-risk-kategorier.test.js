...
describe('ovriga-risk-kategorier', () => {
  ...
  it('lägger geografiska residualer under Vad gör kunden?', () => {
    ...
    assert.match(js, /=== 'verksamheten'\) ids\.unshift\('geografiska'\)/);
    assert.doesNotMatch(js, /=== 'kunden'\) ids\.unshift\('geografiska'\)/);
    assert.doesNotMatch(js, /id="risker-kort-geografiska"/);
    assert.match(css, /\.ovriga-risk-katalog--geo/);
    assert.match(dim, /kundkortLabel: 'Geografisk riskfaktorer - här finns kundens kunder & leverantörer'/);
    assert.match(dim, /id: 'geografiska'[\s\S]*?cardTitle: 'Vad gör kunden\?'/);
    ...
    assert.equal(Kat.categoryIdForDimension('geografiska'), 'verksamheten');
  });
