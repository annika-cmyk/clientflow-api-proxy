...
describe('ovriga-risk-kategorier', () => {
  ...
  it('tar bort borttagen riskfaktor om kundens egna kunder på distans', () => {
    const label = 'Kundens egna kunder är på distans (e-handel/anonyma köpare)';
    assert.equal(Kat.isRemovedFactor(label), true);
    assert.equal(Kat.findFactor(label), null);
    assert.equal(Kat.canonicalLabel(label), '');
    assert.ok(!Kat.labelsForCategory('verksamheten').includes(label));
    assert.equal(Kat.categoryFor(label), '');
  });
  ...
  it('håller högriskbransch infälld och tillåter SNI plus manuell ifyllning', () => {
    ...
    assert.match(html, /kundkort\.js\?v=16\.12/);
