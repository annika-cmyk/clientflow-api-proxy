...
describe('exakt dokumenthänvisning', () => {
    ...
    it('duplicerar inte dokumenttiteln som avsnitt', () => {
        const raw = 'Ekobrottsmyndigheten — Penningtvätt i näringsverksamhet (2024) — https://www.ekobrottsmyndigheten.se/wp-content/uploads/se/2024/12/penningtvatt-i-naringsverksamhet-inkl-sammanfattning-1.pdf';
        const out = normalizeKalla(raw);
        const hits = out.match(/Penningtvätt i näringsverksamhet/gi) || [];
        assert.equal(hits.length, 1);
        const r = resolveKalla(raw);
        assert.match(r.page, /Penningtvätt i näringsverksamhet\s*2024/);
        const d = require('../public/js/aml-kalla').formatKallaDisplay(r);
        assert.match(d.linkText, /Ekobrottsmyndigheten/);
        assert.match(d.linkText, /Penningtvätt i näringsverksamhet/);
        assert.doesNotMatch(d.linkText, /Penningtvätt i näringsverksamhet.*Penningtvätt i näringsverksamhet/);
    });
