...
test('strips leaked evidens suffix from sårbarhetsbeskrivning', () => {
    const mapped = mapByraTjanstRecord({
        id: 'rec-evidens',
        fields: {
            'Task Name': 'Moms',
            'Sårbarheter': JSON.stringify([{
                titel: 'Kunddata',
                beskrivning: 'Felaktig redovisning av kunddata kan påverka momsredovisningen negativt. Bekräftad.',
                evidens: 'bekraftad'
            }])
        }
    });
    assert.equal(
        mapped.sarbarheter[0].beskrivning,
