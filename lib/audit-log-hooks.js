...
async function logInneboendeArchived({ write, service, actor, byraId, customerId, fields }) {
  if (!write || !customerId) return null;
  const raw = fields && (fields['Kund inneboende riskprofil'] || fields.kundInneboendeRiskprofil);
  const value = raw == null ? '' : String(raw).trim();
  if (!value) return null;
  if (service && typeof service.getAuditLog === 'function') {
    try {
      const existing = await service.getAuditLog('kund', customerId);
      if ((existing || []).some((row) => row && row.actionType === 'kund_inneboende_arkiverad')) {
        return null;
      }
    } catch (err) {
      console.warn('audit-log inneboende-arkiv lookup:', err.message);
    }
  }
  return write({
    actionType: 'kund_inneboende_arkiverad',
    entityType: 'kund',
    entityId: customerId,
    actor: actor || auditLog.SYSTEM_ACTORS.system,
    byraId,
    fieldChanged: 'Kund inneboende riskprofil',
    valueBefore: value,
    valueAfter: null,
    motivering: 'Inneboende kundprofil tas bort från kundnivå. Värdet arkiveras för historik.',
    metadata: {
      entitetNamn: (fields && (fields.Namn || fields['Företagsnamn'])) || '',
      arkiveradInneboende: value
    }
  }, { allowEmptyMotivering: true });
}
