...
describe('risk-factor-catalog', () => {
  ...
  it('lazyOmprofileringFields returns patch only when stamp behind and not already flagged', () => {
    const stamp = Catalog.kundStampFields(2);
    assert.equal(Catalog.lazyOmprofileringFields(stamp, 2), null);
    assert.deepEqual(Catalog.lazyOmprofileringFields(stamp, 3), Catalog.kundOmprofileringFields());
    const already = {
      ...stamp,
      ...Catalog.kundOmprofileringFields()
    };
    assert.equal(Catalog.lazyOmprofileringFields(already, 3), null);
    assert.equal(Catalog.effectiveKundProfileStatus(stamp, 3), 'behöver_omprofilering');
    assert.equal(Catalog.effectiveKundProfileStatus(stamp, 2), '');
  });

  it('documents lazy cascade (no eager fan-out on bump)', () => {
    const src = fs.readFileSync(path.join(__dirname, 'risk-factor-catalog.js'), 'utf8');
    assert.match(src, /Cascade-strategi \(lazy/);
    assert.match(src, /lazyOmprofileringFields/);
    const index = fs.readFileSync(path.join(__dirname, '../index.js'), 'utf8');
    assert.match(index, /lazyOmprofileringFields/);
    assert.match(index, /Lazy katalog-cascade/);
  });
