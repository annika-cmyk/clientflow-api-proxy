...
describe('kundrisker-mm', () => {
  it('har egen sida med kundrisk, båda geo-spåren, varningsflaggor och risksänkande', () => {
    ...
    assert.match(html, /ovriga-riskfaktorer\.js\?v=53/);
    ...
    assert.match(html, /styles\.css\?v=20260915enkatstat/);
    ...
    assert.match(html, /kundrisker-enkat-sammanfattning\.js\?v=11/);
    assert.match(html, /statistik-kunder-modal\.js\?v=3/);
    assert.match(html, /kund-bransch-aggregat\.js/);
    assert.match(html, /id="statistik-bransch-drilldown"/);
    assert.match(html, /id="statistik-kunder-modal-overlay"/);
    ...
    assert.match(ovriga, /ovriga-riskfaktorer\.js\?v=53/);
    assert.match(ovriga, /Din resa/);
    assert.match(ovriga, /data-risk-tab="inneboende"/);
    assert.match(ovriga, /tjanst-modal-content/);
    assert.match(ovriga, /styles\.css\?v=20260916koppling/);
    assert.match(ovriga, /id="byra-profil-kaskad"/);
    assert.match(ovriga, /byra-profil-risk-forslag\.js\?v=2/);
    assert.match(ovriga, /byra-profil-analys-koppling\.js\?v=1/);
    assert.match(ovriga, /section=intern/);
    assert.match(ovriga, /section=distribution/);
    assert.match(ovriga, /section=geografi/);
    ...
    assert.match(js, /openEditModal\(recordId,\s*opts\s*=\s*\{\}\)[\s\S]*?clearOvrigInlineAi\('edit-risk-modal'\)/);
    ...
    assert.match(enkat, /kundrisker-bransch-chip/);
    assert.match(enkat, /bindBranschChips/);
    assert.match(enkat, /data-typ="kund-bransch"|drillTypForField/);
    assert.match(enkat, /Analysera/);
    assert.match(enkat, /StatistikKunderModal/);
    assert.match(enkat, /profilWithLiveClientflow/);
    assert.match(enkat, /pågående kunder i Clientflow/);
