...
describe('kundrisker-mm', () => {
  it('har egen sida med kundrisk, båda geo-spåren, varningsflaggor och risksänkande', () => {
    ...
    const kundModal = AnalyskortMall.renderPageModals('kundrisker');
    assert.match(kundModal, /Geografisk riskfaktorer – egen hemvist/);
    assert.match(kundModal, /Geografisk riskfaktorer – motparters geografi/);
    assert.match(kundModal, /value="Geografisk riskfaktorer - här finns byråns kunder">Geografisk riskfaktorer – egen hemvist/);
    assert.match(kundModal, /value="Geografisk riskfaktorer - här finns kundens kunder &amp; leverantörer">Geografisk riskfaktorer – motparters geografi/);
    ...
    assert.match(html, /data-analyskort-modals="kundrisker"/);
    assert.match(kundModal, /Din resa/);
    assert.match(kundModal, /data-risk-tab="utforande"/);
    assert.match(kundModal, /data-risk-tab="hot"/);
    assert.match(kundModal, /data-risk-tab="sarbarhet"/);
    assert.match(kundModal, /data-risk-tab="oversikt"/);
    assert.match(kundModal, /Frågor från din ClientFlow AI/);
    assert.match(kundModal, /tjanst-resa-progress/);
    assert.match(html, /styles\.css\?v=20260915enkatstat/);
    assert.match(kundModal, /novalidate/);
    ...
    const ovrigaModal = AnalyskortMall.renderPageModals('ovriga');
    assert.match(ovriga, /ovriga-riskfaktorer\.js\?v=\d+/);
    assert.match(ovriga, /analyskort-mall\.js/);
    assert.match(ovriga, /data-analyskort-modals="ovriga"/);
    assert.match(ovrigaModal, /Din resa/);
    assert.match(ovrigaModal, /data-risk-tab="inneboende"/);
    assert.match(ovrigaModal, /tjanst-modal-content/);
    assert.match(ovriga, /styles\.css\?v=20260916koppling/);
    assert.match(ovriga, /id="byra-profil-kaskad"/);
    assert.match(ovriga, /byra-profil-risk-forslag\.js\?v=\d+/);
