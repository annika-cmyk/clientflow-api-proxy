/**
 * Feedback från dashboarden och tjänstmodalen.
 * Dashboard → hej@clientflow.se
 * Residualrisk-flik → feedback@clientflow.se
 */
const FEEDBACK_TO = 'hej@clientflow.se';
const FEEDBACK_TO_RESIDUAL = 'feedback@clientflow.se';
const MAX_MESSAGE = 4000;
const MIN_MESSAGE = 8;
const MAX_CONTEXT = 200;

const TYPES = {
  forslag: 'Förslag',
  problem: 'Problem',
  fraga: 'Fråga'
};

const SOURCES = {
  dashboard: {
    to: FEEDBACK_TO
  },
  residualrisk: {
    to: FEEDBACK_TO_RESIDUAL,
    kindLabel: 'Residualrisk'
  }
};

function escapeHtml(value) {
  return String(value || '')
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;')
    .replace(/"/g, '&quot;')
    .replace(/'/g, '&#39;');
}

function normalizeSource(raw) {
  const key = String(raw || 'dashboard').trim().toLowerCase();
  return SOURCES[key] ? key : 'dashboard';
}

function normalizeFeedback(body, user) {
  const rawType = String(body && body.type != null ? body.type : 'forslag').trim();
  const type = TYPES[rawType] ? rawType : 'forslag';
  const source = normalizeSource(body && body.source);
  const tjanstNamn = String(body && body.tjanstNamn != null ? body.tjanstNamn : '')
    .trim()
    .slice(0, MAX_CONTEXT);
  return {
    type,
    typeLabel: TYPES[type],
    source,
    message: String(body && body.message != null ? body.message : '').trim(),
    name: String(user && user.name != null ? user.name : '').trim(),
    email: String(user && user.email != null ? user.email : '').trim(),
    byra: String(user && user.byra != null ? user.byra : '').trim(),
    byraId: String(user && user.byraId != null ? user.byraId : '').trim(),
    tjanstNamn
  };
}

function validateFeedback(input) {
  if (!input || !input.message || input.message.length < MIN_MESSAGE) {
    return 'Skriv lite mer så vi förstår din feedback.';
  }
  if (input.message.length > MAX_MESSAGE) {
    return 'Feedbacken är för lång (max 4000 tecken).';
  }
  if (!input.email || !input.email.includes('@')) {
    return 'Din inloggade e-post saknas.';
  }
  return null;
}

function feedbackKindLabel(input) {
  const sourceMeta = SOURCES[input.source] || SOURCES.dashboard;
  return sourceMeta.kindLabel || input.typeLabel;
}

function buildFeedbackEmail(input) {
  const who = input.name || input.email;
  const sourceMeta = SOURCES[input.source] || SOURCES.dashboard;
  const kindLabel = feedbackKindLabel(input);

  const textLines = [
    `Typ: ${kindLabel}`,
    `Från: ${who} <${input.email}>`
  ];
  if (input.byra) textLines.push(`Byrå: ${input.byra}`);
  if (input.byraId) textLines.push(`Byrå-ID: ${input.byraId}`);
  if (input.tjanstNamn) textLines.push(`Tjänst: ${input.tjanstNamn}`);
  textLines.push('', input.message);
  const text = textLines.join('\n');

  const metaHtml = [
    `Från <strong>${escapeHtml(who)}</strong>`,
    `&lt;${escapeHtml(input.email)}&gt;`
  ];
  if (input.byra) metaHtml.push(`<br>Byrå: ${escapeHtml(input.byra)}`);
  if (input.byraId) metaHtml.push(`<br>Byrå-ID: ${escapeHtml(input.byraId)}`);
  if (input.tjanstNamn) metaHtml.push(`<br>Tjänst: ${escapeHtml(input.tjanstNamn)}`);

  const html = `
<!DOCTYPE html>
<html>
<head><meta charset="utf-8"></head>
<body style="margin:0;padding:24px;font-family:system-ui,-apple-system,Segoe UI,sans-serif;color:#15181d;background:#f5f6f8;">
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:560px;margin:0 auto;background:#fff;border-radius:12px;padding:24px;">
    <tr><td>
      <p style="margin:0 0 8px;font-size:13px;color:#13505c;font-weight:600;">${escapeHtml(kindLabel)}</p>
      <p style="margin:0 0 16px;font-size:15px;line-height:1.5;">
        ${metaHtml.join('')}
      </p>
      <p style="margin:0;font-size:15px;line-height:1.55;white-space:pre-wrap;">${escapeHtml(input.message)}</p>
    </td></tr>
  </table>
</body>
</html>`.trim();

  const byraSuffix = input.byra ? ` (${input.byra})` : '';
  return {
    to: sourceMeta.to,
    replyTo: input.email,
    subject: `Feedback ClientFlow: ${kindLabel} från ${who}${byraSuffix}`,
    text,
    html
  };
}

module.exports = {
  FEEDBACK_TO,
  FEEDBACK_TO_RESIDUAL,
  MAX_MESSAGE,
  TYPES,
  SOURCES,
  escapeHtml,
  normalizeFeedback,
  validateFeedback,
  buildFeedbackEmail
};
