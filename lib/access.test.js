...
describe('userHasCustomerAccess', () => {
  ...
  it('matchar Byrå ID trots komma och länkobjekt', () => {
    assert.equal(sameByra({ byraId: '1,2' }, { 'Byrå ID': '12' }), true);
    assert.equal(sameByra({ byraId: '12' }, { 'Byrå ID': ['12'] }), true);
    assert.equal(userSharesAgency({ role: 'Anställd', byraId: '12' }, customer), true);
    assert.equal(userSharesAgency({ role: 'Anställd', byraId: '12', id: 'recNope' }, customer), true);
    assert.equal(userHasCustomerAccess({ role: 'Anställd', byraId: '12', id: 'recNope' }, customer), false);
  });
  ...
  it('känner igen anställd via länkfält (array) eller namn', () => {
    ...
    assert.equal(userListedOnCustomer({ id: 'recMgsv71CSwnmgMm' }, { 'Behöriga användare': ['recMgsv71CSwnmgMm'] }), true);
    assert.equal(userListedOnCustomer({ id: 'recMgsv71CSwnmgMm' }, { Anteckning: 'recMgsv71CSwnmgMm' }), true);
