...
describe('beskrivningskort på kundkort och KYC', () => {
  ...
  it('har en gemensam penna och Spara för alla fyra beskrivningsblock', () => {
    assert.match(kundkort, /id="beskrivning-edit-btn"/);
    assert.match(kundkort, /id="beskrivning-card-actions"/);
    assert.match(kundkort, /toggleBeskrivningCardEdit\(/);
    assert.match(kundkort, /saveBeskrivningCard\(/);
    assert.doesNotMatch(kundkort, /\$\{def\.id\}-edit-btn/);
    const renderStart = kundkort.indexOf('_renderBeskrivningDelkort(def, fields, mis) {');
    const renderEnd = kundkort.indexOf('toggleBeskrivningCardEdit() {');
    assert.ok(renderStart >= 0 && renderEnd > renderStart, 'render-metod saknas');
    const renderBody = kundkort.slice(renderStart, renderEnd);
    assert.doesNotMatch(renderBody, /card-edit-fab/);
    assert.doesNotMatch(renderBody, /kunduppgifter-actions/);
    assert.doesNotMatch(renderBody, /saveBeskrivningDelkort/);
  });
  ...
  it('visar Dokumentation - historik sist på dokumentationsfliken', () => {
    const match = kundkort.match(/categoryOrder = \[([^\]]+)\]/);
    assert.ok(match, 'categoryOrder saknas');
    const order = match[1];
    assert.match(order, /'ovrigt',\s*'historik'/);
    assert.doesNotMatch(order, /'historik',\s*'arsredovisning'/);
  });
});
