...
/** NRA-scenarier (B7): ja/nej + kort varför — metodnivå, ingen auto-mappning till hotkatalog. */
const NRA_SOURCE_ID = 'nra-2024-2025';
const NRA_ANSWERS = ['unset', 'ja', 'nej'];
const NRA_SCENARIOS = [
  {
    id: 'nra-kap-4-tf',
    title: 'Terrorfinansiering',
    tag: 'NRA kap. 4',
    desc: 'Kan er byrå, via era kunder eller tjänster, komma i kontakt med finansiering av terrorism? T.ex. kunder som är ideella föreningar, insamlingsorganisationer, eller gör internationella överföringar utan tydlig motprestation.'
  },
  {
    id: 'nra-7-19-bokforing',
    title: 'Bokföring som täckmantel',
    tag: 'NRA 7.19',
    desc: 'Kan bokföringstjänster användas för att få pengar från brottslig verksamhet att se ut som legitim affärsintäkt? T.ex. genom påhittade eller uppblåsta intäkter/kostnader i bokföringen.'
  },
  {
    id: 'nra-7-20-skatt',
    title: 'Skattebrott och slussbolag',
    tag: 'NRA 7.20',
    desc: 'Ett slussbolag är ett bolag som används för att flytta pengar genom skenbara transaktioner, för att dölja varifrån pengarna egentligen kommer. Förekommer skattebrott eller den här typen av upplägg i er kundkrets, eller är det något ni bör bevaka?'
  },
  {
    id: 'nra-7-18-revisor',
    title: 'Formella intyg',
    tag: 'NRA 7.18',
    desc: 'Gäller framförallt revisorer som signerar revisionsberättelser — men kan även omfatta andra formella intyg, t.ex. till banker. Utfärdar er byrå några sådana intyg?'
  },
  {
    id: 'nra-kap-5-mojliggorare',
    title: 'Oavsiktlig möjliggörare',
    tag: 'NRA kap. 5',
    desc: 'Kan era tjänster användas av en kund för att tvätta pengar utan att ni själva är medvetna om det — t.ex. genom att ni bokför transaktioner ni inte har särskild anledning att ifrågasätta?'
  },
  {
    id: 'nra-klientmedel-pml',
    title: 'Klientmedel och betalningsuppdrag',
    tag: '',
    desc: 'Hanterar er byrå kunders pengar direkt, eller sköter betalningar åt kunder (t.ex. leverantörsbetalningar)? Det är precis den typ av tjänst NRA lyfter som förhöjd risk.'
  }
];
...
function normalizeNraEntry(row, id) {
  const answer = row && NRA_ANSWERS.includes(row.answer) ? row.answer : 'unset';
...
function mergeNraChecklist(saved) {
...
function nraChecklistComplete(kallaMap, nraChecklist) {
  if (!nraChecklistRequired(kallaMap)) return true;
  const merged = mergeNraChecklist(nraChecklist);
  return merged.every((row) => {
...
function step8Ready(kallaMap, customKallor, nraChecklist) {
  return kallaCatalogComplete(kallaMap, customKallor) && nraChecklistComplete(kallaMap, nraChecklist);
}
