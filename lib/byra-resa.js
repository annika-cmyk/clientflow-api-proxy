...
/**
 * NRA är källtagg (DEFAULT_KALLOR + aml-kalla på hot/modus), inte ett eget checkliststeg.
 * Terrorfinansiering → kundriskfaktor; Formella intyg → tjänstemall; övriga scenarier
 * täcks redan av tjänstehot / klientmedel / kundrisker.
 * nraChecklist i sparad state ignoreras (bakåtkompatibilitet).
 */
const NRA_SOURCE_ID = 'nra-2024-2025';

function step8Ready(kallaMap, customKallor) {
  return kallaCatalogComplete(kallaMap, customKallor);
}
...
const RESA_STEPS = [
  ...
  },
  {
    ...
    title: 'Godkännande',
    desc: 'Slutgodkänn AR och rutiner med bekräftad CFA och BankID. Kräver ifylld källkatalog (NRA som källtagg, inte separat checklista).',
...
function parseByraResaState(raw) {
  ...
  return {
    ...
    riskFactorCatalogVersion: (Number.isFinite(catalogVersion) && catalogVersion >= 1)
      ...
      : 1,
    // Legacy: tidigare NRA-checklista ignoreras (NRA = källtagg).
    nraChecklist: {},
    kundriskAnalysSkipped: normalizeKundriskAnalysSkipped(obj.kundriskAnalysSkipped)
  };
}

/** Avstådda analysförslag under Vilka är våra kunder / Från byråprofilen (grupp-id:n). */
function normalizeKundriskAnalysSkipped(list) {
  const seen = new Set();
  const out = [];
...
function buildByraResaState({ steps, kalla, customKallor, arDeltaPending, riskEvents, riskFactorCatalogVersion, kundriskAnalysSkipped } = {}) {
  const parsedSteps = {};
  RESA_STEPS.forEach((s) => {
  ...
  return {
    ...
    riskFactorCatalogVersion: (Number.isFinite(catalogVersion) && catalogVersion >= 1)
      ...
      : 1,
    nraChecklist: {},
    kundriskAnalysSkipped: normalizeKundriskAnalysSkipped(kundriskAnalysSkipped)
  };
}
...
module.exports = {
  ...
  parseByraResaState,
  buildByraResaState,
  normalizeKundriskAnalysSkipped,
