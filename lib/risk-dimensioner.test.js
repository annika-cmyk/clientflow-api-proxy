...
describe('risk-dimensioner', () => {
  ...
  it('normaliserar geografiska riskfaktorer till byråns kunder', () => {
    assert.equal(
  ...
  it('matchar exakt rubrik före alias även när rubriken innehåller ordet kunder', () => {
    ...
    assert.equal(
      RiskDimensioner.dimensionOfTyp('Geografisk riskfaktorer - här finns kundens kunder & leverantörer').id,
  ...
  it('mappar byrå-geo till Vem är kunden? och motpart-geo till Vad gör kunden?', () => {
    assert.equal(
      RiskDimensioner.dimensionOfTyp('Geografiska riskfaktorer').cardTitle,
      'Vem är kunden?'
    );
    assert.equal(
      RiskDimensioner.dimensionOfTyp('Geografiska riskfaktorer').categoryId,
      'kunden'
    );
    assert.equal(
      RiskDimensioner.dimensionOfTyp('Geografisk riskfaktorer - här finns kundens kunder & leverantörer').cardTitle,
      'Vad gör kunden?'
    );
    assert.equal(
      RiskDimensioner.dimensionOfTyp('Geografisk riskfaktorer - här finns kundens kunder & leverantörer').categoryId,
      'verksamheten'
    );
    assert.equal(
      RiskDimensioner.kundkortTypLabel('Geografisk riskfaktorer - här finns byråns kunder'),
      'Geografisk riskfaktorer - här finns byråns kunder'
    );
    assert.equal(
      RiskDimensioner.kundkortTypLabel('Geografisk riskfaktorer - här finns kundens kunder & leverantörer'),
