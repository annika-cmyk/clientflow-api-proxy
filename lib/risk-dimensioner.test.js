...
describe('risk-dimensioner', () => {
  it('normaliserar Distrubutionskanaler till Distributionskanaler', () => {
    assert.equal(RiskDimensioner.normalizeTyp('Distrubutionskanaler'), 'Distributionskanaler');
    assert.equal(RiskDimensioner.normalizeTyp('Distributionskanaler'), 'Distributionskanaler');
  ...
  it('grupperar Via ombud med övriga distributionskanaler', () => {
    const groups = RiskDimensioner.groupOvrigaByTyp([
      ...
      { typ: 'Distributionskanaler', namn: 'Distanskund' },
      { typ: 'Distrubutionskanaler', namn: 'Via ombud' },
      { typ: 'Geografiska riskfaktorer', namn: 'Europa' }
    ]);
    const dist = groups.find((g) => g.typ === 'Distributionskanaler');
    assert.ok(dist);
  ...
  it('kräver minst ett val per dimension som byrån har mallar för', () => {
    ...
    assert.ok(incomplete.saknade.includes('Distributionskanaler'));
    assert.ok(incomplete.saknade.includes('Riskfaktorer kopplat till kund'));
    assert.match(incomplete.varning, /Riskbedömning ofullständig — saknar/);
  ...
  it('räknar övriga riskfaktorer som val i dimensionen', () => {
    const templates = [
      { fields: { 'Typ av riskfaktor': 'Geografiska riskfaktorer', Riskfaktor: 'Europa' } },
      { fields: { 'Typ av riskfaktor': 'Distrubutionskanaler', Riskfaktor: 'Fysiskt möte' } }
    ];
    const status = RiskDimensioner.assessCustomerDimensions({
      ...
      byraTemplates: templates,
      extraPresent: { distribution: true }
    });
    assert.equal(status.komplett, true);
    assert.ok(status.present.distribution.length);
  });
});
