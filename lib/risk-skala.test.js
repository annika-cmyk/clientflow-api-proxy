...
describe('femgradig riskskala', () => {
  ...
  it('har dimensionsordlista skild från risknivåetiketter', () => {
    ...
    assert.match(block, /Soft-default till S=2–3/);
    assert.match(block, /S×K ≥ 10/);
    assert.match(block, /undervärdering/i);
    assert.match(block, /Exempel rätt \(Förhöjd\)/);
  });
  ...
  it('mappar alla fem nivåer till CSS-klasser och badge-etiketter', () => {
    assert.equal(RiskSkala.riskItemClass('Låg'), 'risk-low');
    assert.equal(RiskSkala.riskItemClass('Normal'), 'risk-normal');
    assert.equal(RiskSkala.riskItemClass('Förhöjd'), 'risk-elevated');
    assert.equal(RiskSkala.riskItemClass('Hög'), 'risk-high');
    assert.equal(RiskSkala.riskItemClass('Oacceptabel'), 'risk-unacceptable');
    assert.equal(RiskSkala.riskPillClass('Oacceptabel'), 'risk-pill--unacceptable');
    assert.equal(RiskSkala.dominantRiskItemClass('Hög', 'Oacceptabel'), 'risk-unacceptable');
    assert.equal(RiskSkala.dominantRiskItemClass('Oacceptabel', 'Låg'), 'risk-unacceptable');
    const oacc = RiskSkala.assessRisk(5, 5);
    assert.equal(oacc.level, 'Oacceptabel');
    assert.match(RiskSkala.formatInneboendeBadge(oacc), /Oacceptabel/);
    assert.match(RiskSkala.listBadgeLabels(oacc).inneboende, /Oacceptabel/);
  });

  it('sparar och läser klarmarkerade flikar i Riskpoäng', () => {
    assert.deepEqual(
      RiskSkala.normalizeKlarmarkeradeFlikar(['hot', 'hot', 'okand', 'residual', '']),
      ['hot', 'residual']
    );
    const serialized = RiskSkala.serializeRiskPoang({
      sannolikhet: 3,
      konsekvens: 3,
      klarmarkeradeFlikar: ['utforande', 'oversikt', 'hot', 'bogus']
    });
    const parsed = JSON.parse(serialized);
    assert.deepEqual(parsed.klarmarkeradeFlikar, ['utforande', 'oversikt', 'hot']);
    const scored = RiskSkala.readTjanstRisk({ Riskpoäng: serialized });
    assert.deepEqual(scored.klarmarkeradeFlikar, ['utforande', 'oversikt', 'hot']);
    const onlyKlar = RiskSkala.parseRiskPoang(JSON.stringify({
      klarmarkeradeFlikar: ['sarbarhet', 'atgard']
    }));
    assert.deepEqual(onlyKlar.klarmarkeradeFlikar, ['sarbarhet', 'atgard']);
    const empty = RiskSkala.serializeRiskPoang({ sannolikhet: 2, konsekvens: 2 });
    assert.equal(JSON.parse(empty).klarmarkeradeFlikar, undefined);
    assert.equal(RiskSkala.isTjanstResaComplete(['utforande', 'oversikt']), false);
    assert.equal(RiskSkala.isTjanstResaComplete(RiskSkala.TJANST_RESA_FLIKAR), true);
    assert.deepEqual(RiskSkala.tjanstResaProgress(['hot', 'bogus', 'hot']), {
      done: ['hot'],
      doneCount: 1,
      total: RiskSkala.TJANST_RESA_FLIKAR.length,
      complete: false
    });
  });
});
