...
const { userHasCustomerAccess } = require('./access');
const {
  normalizeOrgNr,
  orgNrVariants,
  recordOrgNr,
...
function createMinibokSync(deps) {
  ...
  async function findCompanyForUser(orgNr, userEmail, opts) {
    ...
    const record = pickBestCompanyRecord(res.data.records || [], {
      orgNr,
      name: opts && opts.name,
      hasAccess: (r) => userHasCustomerAccess(user, r),
    });
  ...
  function registerRoutes(app) {
    ...
    app.post('/api/v1/companies', authenticateMinibokApi, async (req, res) => {
      try {
        ...
        if (existing) {
          const existingSrc = existing.fields?.['Minibok källa'] || existing.fields?.['Minibok kalla'];
          if ((source || 'minibok') === 'minibok' && !existingSrc) {
            await patchOptionalFields(existing.id, {
              'Minibok pending': true,
          ...
          return res.json({
            ...
            contacts: payload.contacts,
            officers: payload.officers,
            notificationQueued: !existingSrc
          });
        }
