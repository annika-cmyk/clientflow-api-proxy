...
const { userHasCustomerAccess, userSharesAgency, byraFilterFormula } = require('./access');
const auditLog = require('./audit-log');
const auditHooks = require('./audit-log-hooks');
const auditRuntime = require('./audit-log-runtime');
const {
  normalizeOrgNr,
  orgNrVariants,
  recordOrgNr,
...
function createMinibokSync(deps) {
  ...
  function pickUserFields(record) {
    ...
    return {
      id: record.id,
      email: fields.Email || '',
      name: findField(['Full Name']),
      role: fields.Role || 'user',
      byraId: findField(['Byrå ID i text 2', 'Byra ID i text 2']),
      byra: findField(['Byrå', 'Byra'])
    };
  }

  /** Matcha Application Users case-insensitive; välj post med Byrå ID om flera. */
  async function resolveMinibokUser(userEmail) {
    ...
    try {
      const formula = `LOWER({Email})="${escAirtable(normalized)}"`;
      const url = `https://api.airtable.com/v0/${airtableBaseId()}/${encodeURIComponent(USERS_TABLE)}?filterByFormula=${encodeURIComponent(formula)}&maxRecords=20`;
      ...
      if (!records.length) {
        return getAirtableUser(normalized);
      }
      const users = records.map(pickUserFields);
      return users.find((u) => u.byraId) || users[0] || null;
    } catch (err) {
      console.warn('resolveMinibokUser:', err.response?.data?.error?.message || err.message);
      return getAirtableUser(normalized);
    }
  }
  ...
  function formatApiError(err, fallback) {
    ...
    if (status === 422) return 'Airtable avvisade kunddata. Kontrollera fält i KUNDDATA.';
    if (status === 400) return fallback || 'Ogiltig förfrågan.';
    return fallback || msg || 'Serverfel';
  }
  ...
  async function queryKunddata(formula, maxRecords) {
    const token = airtableToken();
    const url = `https://api.airtable.com/v0/${airtableBaseId()}/${KUNDDATA_TABLE}?filterByFormula=${encodeURIComponent(formula)}&maxRecords=${maxRecords || 50}`;
    const res = await axios.get(url, {
      headers: { Authorization: `Bearer ${token}` },
      timeout: 15000
    });
  ...
  async function findCompanyForUser(orgNr, userEmail, opts) {
    const token = airtableToken();
    if (!token) throw new Error('Airtable token saknas');

    const user = await resolveMinibokUser(userEmail);
    if (!user?.id) return { user: null, record: null };

    const matchAgency = opts && opts.match === 'agency';
    const pick = (records) => pickBestCompanyRecord(records || [], {
      orgNr,
      name: opts && opts.name,
      hasAccess: (r) => (matchAgency ? userSharesAgency(user, r) : userHasCustomerAccess(user, r)),
    });

    const digitFormula = orgNrAirtableFormula(orgNr);
    if (digitFormula) {
      try {
        const record = pick(await queryKunddata(digitFormula, 50));
    ...
    if (variants.length) {
      ...
      try {
        const record = pick(await queryKunddata(formula, 50));
        if (record) return { user, record };
      } catch (err) {
        if (err.response?.status !== 422) throw err;
      }
    ...
    const byraF = byraFilterFormula(user.byraId);
    if (byraF) {
      try {
        const record = pick(await queryKunddata(byraF, 100));
        if (record) return { user, record };
      } catch (err) {
        if (err.response?.status !== 422) throw err;
      }
  ...
  async function fetchBolagsverketOrg(orgNr) {
    const baseOrg = normalizeOrgNr(orgNr);
    let token;
    try {
      token = await getBolagsverketToken();
    } catch (err) {
      console.warn('⚠️ Minibok Bolagsverket token:', err.message);
      return { orgData: null, cleanOrgNumber: baseOrg };
    }
    ...
    const tryLookup = async (ident) => {
      try {
        const res = await axios.post(orgUrl, { identitetsbeteckning: ident }, { headers, timeout: 15000 });
        if (res.data?.organisationer?.[0]) {
          return { orgData: res.data.organisationer[0], cleanOrgNumber: ident };
        }
      } catch (err) {
        const detail = err.response?.data?.detail || err.response?.data?.message || err.message;
        console.warn(`⚠️ Minibok Bolagsverket (${ident}):`, detail);
      }
      return null;
    };

    const first = await tryLookup(baseOrg);
    if (first) return first;
    if (baseOrg.length === 10) {
      const second = await tryLookup(toTwelve(baseOrg));
  ...
  function mapBolagsverketToFields(orgData, cleanOrgNumber, user, body) {
    ...
    const fields = {
      Orgnr: normalizeOrgNr(cleanOrgNumber || body.orgNr || ''),
  ...
  async function createAirtableRecord(fields) {
    ...
    try {
      ...
      const retryable = err.response?.status === 422 || err.response?.status === 400;
      if (retryable && (/UNKNOWN_FIELD_NAME|INVALID|unknown field/i.test(msg) || err.response?.status === 400)) {
        const safe = { ...fields };
        ['Minibok pending', 'Minibok källa', 'Minibok company id', 'Kundstatus', 'Aktivt företag', 'Bolagsform', 'Address', 'Verksamhetsbeskrivning', 'regdatum'].forEach((k) => delete safe[k]);
        try {
  ...
  function registerRoutes(app) {
      ...
      try {
        ...
        if (!user) {
          return res.status(404).json({
            error: `Ingen Clientflow-användare för ${String(userEmail || '').toLowerCase()}`,
            exists: false
          });
        }
        if (!record) {
    ...
    app.post('/api/v1/companies', authenticateMinibokApi, async (req, res) => {
      try {
        ...
        const user = await resolveMinibokUser(resolvedEmail);
        if (!user?.id) {
          return res.status(404).json({
            error: `Ingen Clientflow-användare hittades för ${resolvedEmail}. Logga in med samma e-post i Clientflow (Application Users).`
          });
        }
        if (!user.byraId) {
          return res.status(400).json({
            error: `Användaren ${resolvedEmail} saknar Byrå ID i Clientflow (Application Users).`
          });
        }

        const orgNormalized = normalizeOrgNr(orgNr);
        if (orgNormalized.length !== 10) {
          return res.status(400).json({ error: 'Ogiltigt organisationsnummer (kräver 10 siffror efter normalisering).' });
        }
        ...
        if (existing) {
          const linkFields = existingCustomerSyncFields(existing.fields, {
            source: source || 'minibok',
            minibokCompanyId,
          });
          if (Object.keys(linkFields).length) {
            await patchOptionalFields(existing.id, linkFields);
          }
          ...
          return res.json({
            ...
            contacts: payload.contacts,
            officers: payload.officers,
            notificationQueued: shouldQueueMinibokNewCustomerNotice(existing)
          });
        }
        ...
        try {
          ...
          auditHooks.logCustomerCreated({
            write: auditRuntime.writeAudit,
            actor: auditLog.SYSTEM_ACTORS.minibok,
            byraId: fields['Byrå ID'] || user?.byraId || '',
            customerId: createdRecord.id,
            fields: createdRecord.fields || fields
          }).catch((err) => console.warn('audit-log minibok create:', err.message));
        } catch (createErr) {
        ...
        const record = { id: createdRecord.id, fields: { ...(createdRecord.fields || fields), 'Minibok pending': true, 'Minibok källa': source || 'minibok' } };
        ...
        return res.status(status >= 400 && status < 600 ? status : 500).json({
          error: formatApiError(err, 'Kunde inte skapa kund i Clientflow.')
        });
      }
    });
    ...
    app.get('/api/minibok/notifications', authenticateToken, async (req, res) => {
      try {
        ...
        const user = await resolveMinibokUser(String(req.user.email || '').toLowerCase());
        ...
        let records = [];
        try {
          const url = `https://api.airtable.com/v0/${airtableBaseId()}/${KUNDDATA_TABLE}?filterByFormula=${encodeURIComponent(formula)}&maxRecords=20&fields[]=Namn&fields[]=Orgnr&fields[]=Minibok pending&fields[]=Minibok källa&fields[]=Kundstatus&fields[]=Riskniva&fields[]=Uppdrag`;
          ...
          if (err.response?.status === 422) {
            ...
            const url = `https://api.airtable.com/v0/${airtableBaseId()}/${KUNDDATA_TABLE}?filterByFormula=${encodeURIComponent(fallbackFormula)}&maxRecords=100&fields[]=Namn&fields[]=Orgnr&fields[]=Minibok källa&fields[]=Kundstatus&fields[]=Minibok pending`;
            const airtableRes = await axios.get(url, { headers: { Authorization: `Bearer ${token}` }, timeout: 15000 });
        ...
        const stale = records.filter((r) => !shouldShowMinibokNewCustomerNotice(r.fields));
        stale.forEach((r) => {
          patchOptionalFields(r.id, { 'Minibok pending': false }).catch(() => {});
        });
        const notifications = records.filter((r) => shouldShowMinibokNewCustomerNotice(r.fields)).map((r) => ({
          id: r.id,
          companyId: r.id,
          name: r.fields?.Namn || '',
    ...
    app.post('/api/minibok/notifications/:id/dismiss', authenticateToken, async (req, res) => {
      try {
        ...
        if (!userHasCustomerAccess(user, getRes.data)) {
