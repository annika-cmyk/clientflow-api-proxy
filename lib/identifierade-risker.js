...
function formatTjanstBlock(t) {
    ...
    const inherent = RiskSkala.assessRisk(t && t.sannolikhet, t && t.konsekvens);
    const residual = RiskSkala.assessRisk(t && t.sannolikhetEfter, t && t.konsekvensEfter);
    const niva = inherent.level
        ? `Inneboende risk: ${inherent.badge}`
        : riskLabel(t && t.riskbedomning);
    const residualTxt = residual.level ? `Residualrisk: ${residual.badge}` : '';
    const atgard = text(t && t.atgard);
    const riskRad = [niva, residualTxt, atgard].filter(Boolean).join('. ');
...
function formatOvrigBlock(r) {
    ...
    if (r.riskbedomning) lines.push(`**Risknivå:** ${riskLabel(r.riskbedomning)}`, '');
    return lines.join('\n').trim();
}
