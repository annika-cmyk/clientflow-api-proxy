...
function mapOvrigRiskRecord(record) {
    ...
    return {
        ...
        hot: parseJsonList(f.Hot),
        sarbarheter: parseJsonList(f['Sårbarheter']),
...
function formatOvrigBlock(r) {
    ...
    const hot = TjanstTfTackning.formatHotExport(r && r.hot);
    const sarbarhet = joinNamedBits(r && r.sarbarheter, ['titel', 'title'], ['beskrivning', 'description']);
    if (hot) lines.push(`**Hot och modus:** ${hot}`, '');
    if (sarbarhet) lines.push(`**Sårbarheter:** ${sarbarhet}`, '');
