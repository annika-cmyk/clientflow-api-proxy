...
function inherentMotiveringLine(item) {
    ...
    return mot ? `**Motivering av risknivå:** ${mot}` : '';
}
...
function formatTjanstBlock(t) {
    ...
    if (!TjanstTfTackning.hasTfHot(t && t.hot)) {
        const motivering = text(t && t.tfMotivering);
        if (motivering) lines.push(`**TF-analys:** ${motivering}`, '');
    }
...
function isIdentifieradeCompiledDump(text) {
    const t = String(text || '');
    return /\*\*Tjänst:|\*\*Produkter och tjänster\*\*|Tjänstebeskrivning och inneboende risk/.test(t);
}
