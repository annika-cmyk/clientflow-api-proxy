...
describe('kund-dold', () => {
  ...
  it('visar Radera kund bara för avslutad kund som inte är dold', () => {
    ...
    assert.equal(isAvslutadKund({ Kundstatus: 'Avslutade' }), true);
    assert.equal(isAvslutadKund({ Kundstatus: 'Avslutat' }), true);
  ...
  it('känner igen aktiv kund för statistik (pågående, ej lead/avslutad/dold)', () => {
    ...
    assert.equal(isPagandeKund({ Kundstatus: 'Pågående kund' }), true);
    assert.equal(isPagandeKund({ Kundstatus: 'Pågående' }), true);
    assert.equal(isPagandeKund({}), true);
    assert.equal(isAktivKund({ Kundstatus: 'Pågående kund' }), true);
    assert.equal(isAktivKund({ Kundstatus: 'Pågående' }), true);
    ...
    assert.equal(isAktivKund({ Kundstatus: 'Avslutade' }), false);
    assert.equal(isAktivKund({ Kundstatus: 'Pågående kund', Dold: true }), false);
    // Okänd status räknas inte som pågående i statistik
    assert.equal(isAktivKund({ Kundstatus: 'Vilande' }), false);
    assert.equal(isAktivKund({ Kundstatus: [{ name: 'Avslutad' }] }), false);
    assert.equal(isAktivKund({ Kundstatus: { name: 'Pågående kund' } }), true);
    ...
    const rows = [
      ...
      { id: 'e', fields: {} },
      { id: 'f', fields: { Kundstatus: 'Avslutade' } },
      { id: 'g', fields: { Kundstatus: 'Vilande' } }
    ];
