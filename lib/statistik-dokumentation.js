...
function summaryRows(stat) {
  return [
    ...
    { label: 'PEP eller anhörig till PEP (KYC)', value: stat.antalPepEllerSanktion }
  ];
}
...
function renderStatistikWebHtml(data, escapeFn) {
  ...
  const cards = [
    ...
    { label: 'PEP eller anhörig till PEP (KYC)', value: stat.antalPepEllerSanktion, icon: 'fa-user-secret' }
  ].map((c) => `
          <div class="stat-card">
