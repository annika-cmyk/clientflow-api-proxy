...
describe('kund-riskprofil', () => {
  ...
  it('låter borttagna default-flaggor stanna borttagna efter sparning', () => {
    const persisted = KundRiskprofil.persistRiskhojandeKatalog({
      Kontanthantering: 'GOLV_HOG'
    });
    assert.equal(persisted.visible['Kontantintensiv verksamhet'], 'GOLV_HOG');
    ...
    assert.equal(merged['Kontantintensiv verksamhet'], 'GOLV_HOG');
    assert.equal(merged['Historik av brott / ekonomisk brottslighet'], undefined);
    assert.equal(persisted.categories['Kontantintensiv verksamhet'], 'verksamheten');
    assert.deepEqual(persisted.stored['Kontantintensiv verksamhet'], {
      klass: 'GOLV_HOG',
  ...
  it('sparar kategori på varningsflaggor och läser gamla strängvärden', () => {
    ...
    assert.equal(entries['Fysiskt möte'].category, 'samarbete');
  ...
  it('höjer till Hög vid Hög-aktiv eller två som bidrar vid kombination', () => {
    ...
    assert.equal(kontanter.niva, 'Hög');
    assert.match(kontanter.drivandeFaktor, /Kontantintensiv verksamhet/);
    ...
    const kombi = KundRiskprofil.beraknaRiskhojandeGolv({
      'Riskhöjande faktorer övrigt': ['Distansrelation med BankID-verifiering'],
      'Kunden verkar i en högriskbransch': ['Bygg']
    });
    assert.equal(kombi.niva, 'Hög');
    assert.match(kombi.drivandeFaktor, /Högriskbransch/);
    assert.match(kombi.drivandeFaktor, /Distansrelation med BankID-verifiering/);

    assert.equal(KundRiskprofil.klassForRiskhojande('PEP eller RCA'), 'GOLV_HOG');
    assert.equal(KundRiskprofil.klassForRiskhojande('Fysiskt möte'), 'INFORMATIV');
    assert.equal(KundRiskprofil.klassForRiskhojande('Kontanthantering'), 'GOLV_HOG');
  });
});
