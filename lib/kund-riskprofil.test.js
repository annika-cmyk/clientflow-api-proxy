...
describe('kund-riskprofil', () => {
  ...
  it('visar ofullständig-varning i stället för tyst förslag när dimension saknas', () => {
    const result = KundRiskprofil.beraknaForeslagenNiva({
      tjanster: [{ namn: 'Bokslut', residualProduct: 9 }],
      riskfaktorer: [{ namn: 'Europa', residualProduct: 4 }],
      dimensionStatus: {
        komplett: false,
        saknade: ['Distributionskanaler', 'Riskfaktorer kopplat till kund']
      }
    });
    assert.equal(result.niva, '');
    assert.equal(result.ofullstandig, true);
    assert.match(result.varning, /Riskbedömning ofullständig — saknar val på Hur samarbetar vi\? och Vem är kunden\?/);
    assert.equal(
      KundRiskprofil.isPublicerbar({ Riskniva: 'Hög' }, {
        dimensionStatus: { komplett: false, saknade: ['Geografiska riskfaktorer'] }
      }),
      false
    );
    assert.equal(KundRiskprofil.isPublicerbar({ Riskniva: 'Hög' }), true);
  });
  ...
  it('kundkortet har ett bedömt residualval och visar beräknad residual som referens', () => {
    ...
    assert.match(js, /Bedömd risk/);
    assert.match(js, /Beräknad residual:/);
    assert.match(js, /Riskbedömning ofullständig|ofullstandig/);
    assert.match(js, /_dimensionStatus/);
    assert.match(js, /_unmatchedTjanster/);
    assert.match(js, /TjanstKatalog/);
    assert.match(html, /tjanst-katalog\.js/);
    assert.match(html, /risk-dimensioner\.js/);
    assert.match(js, /_hogriskBranschRiskRecord/);
    assert.match(js, /itemsFromRiskhojandeFlags/);
    assert.match(js, /applyRiskhojandeGolv/);
    assert.match(js, /Ofta bytt redovisningskonsult\/revisor/);
    ...
    assert.match(js, /riskf-check-item--oacceptabel/);
    ...
    assert.match(js, /Avviker från beräknad residual/);
    assert.match(js, /forutsattning-group-collapsed-risk/);
    assert.match(js, /tjanst-risk-pair-kicker/);
    assert.doesNotMatch(js, /Markera om kundens åtgärder görs/);
    assert.doesNotMatch(js, /Standardåtgärder ej uppfyllda hos denna kund/);
    ...
    assert.match(src, /strukturella-luckor-rapport/);
    assert.match(src, /pt_tf_kravs/);
    assert.match(src, /riskdimension_ofullstandig/);
    ...
    assert.match(ovriga, /Övriga varningsflaggor/);
    assert.doesNotMatch(ovriga, /Riskhöjande faktorer övrigt — allvarlighet/);
    const katalogPos = ovriga.indexOf('id="riskhoj-katalog-section"');
    const listPos = ovriga.indexOf('id="risk-list"');
    assert.ok(katalogPos > listPos, 'varningsflaggor ska ligga efter riskfaktorlistan');
  });

  it('låter borttagna default-flaggor stanna borttagna efter sparning', () => {
    const persisted = KundRiskprofil.persistRiskhojandeKatalog({
      'Otydlig affärsmodell': 'GOLV_HOG'
    });
    assert.equal(persisted.visible['Otydlig affärsmodell'], 'GOLV_HOG');
    assert.equal(persisted.visible['Historik av brott / ekonomisk brottslighet'], undefined);
    assert.equal(persisted.stored['Historik av brott / ekonomisk brottslighet'], 'BORTTAGEN');
    assert.equal(persisted.visible['Kunder med mycket kontanta transaktioner'], undefined);
    const merged = KundRiskprofil.mergeRiskhojandeKatalog(persisted.stored);
    assert.equal(merged['Otydlig affärsmodell'], 'GOLV_HOG');
    assert.equal(merged['Historik av brott / ekonomisk brottslighet'], undefined);
    assert.equal(merged['Kunder med mycket kontanta transaktioner'], undefined);
    assert.equal(persisted.categories['Otydlig affärsmodell'], 'verksamheten');
    assert.deepEqual(persisted.stored['Otydlig affärsmodell'], {
      klass: 'GOLV_HOG',
      category: 'verksamheten'
    });
  });

  it('sparar kategori på varningsflaggor och läser gamla strängvärden', () => {
    const persisted = KundRiskprofil.persistRiskhojandeKatalog({
      'Svårt att bekräfta identitet': { klass: 'GOLV_HOG', category: 'verksamheten' },
      'Min extraflagga': { klass: 'BIDRAR_VID_KOMBINATION', category: 'samarbete' }
    });
    assert.equal(persisted.visible['Svårt att bekräfta identitet'], 'GOLV_HOG');
    assert.equal(persisted.categories['Svårt att bekräfta identitet'], 'verksamheten');
    assert.equal(persisted.categories['Min extraflagga'], 'samarbete');
    assert.equal(persisted.stored['Historik av brott / ekonomisk brottslighet'], 'BORTTAGEN');
    const cats = KundRiskprofil.mergeRiskhojandeKategorier(persisted.stored);
    assert.equal(cats['Svårt att bekräfta identitet'], 'verksamheten');
    assert.equal(cats['Min extraflagga'], 'samarbete');
    assert.equal(cats['Fysiskt möte'], undefined);
    const fromLegacy = KundRiskprofil.mergeRiskhojandeKategorier({
      'Svårt att bekräfta identitet': 'GOLV_HOG'
    });
    assert.equal(fromLegacy['Svårt att bekräfta identitet'], 'samarbete');
    const entries = KundRiskprofil.mergeRiskhojandeEntries({
      'Min extraflagga': { klass: 'INFORMATIV', category: 'kunden' }
    });
    assert.equal(entries['Min extraflagga'].klass, 'INFORMATIV');
    assert.equal(entries['Min extraflagga'].category, 'kunden');
    assert.equal(entries['Fysiskt möte'], undefined);
    assert.equal(KundRiskprofil.normalizeRiskhojandeKlass({ klass: 'GOLV_HOG', category: 'kunden' }), 'GOLV_HOG');
    assert.equal(KundRiskprofil.normalizeRiskhojandeCategory({ klass: 'GOLV_HOG', category: 'kunden' }), 'kunden');
  });
  ...
  it('räknar in riskhöjande övrigt-kryss i beräknad residual', () => {
    assert.equal(KundRiskprofil.canonicalRiskhojandeLabel('Kortvarig/tillfällig affärsrelation'), 'Ofta bytt redovisningskonsult/revisor utan naturlig förklaring');
    assert.equal(KundRiskprofil.canonicalRiskhojandeLabel('Företaget har många kortvariga affärsrelationer'), 'Ofta bytt redovisningskonsult/revisor utan naturlig förklaring');
    ...
    assert.match(renamed.drivandeFaktor, /Ofta bytt redovisningskonsult\/revisor/);
  ...
  it('sätter Oacceptabel-golv direkt vid OACCEPTABEL-klassad varningsflagga', () => {
    const katalog = { 'Misstänkt penningtvätt': 'OACCEPTABEL' };
    const flags = { 'Riskhöjande faktorer övrigt': ['Misstänkt penningtvätt'] };

    const golv = KundRiskprofil.beraknaRiskhojandeGolv(flags, katalog);
    assert.equal(golv.niva, 'Oacceptabel');
    assert.equal(golv.skikt, 'OACCEPTABEL');
    assert.match(golv.drivandeFaktor, /Golv Oacceptabel: Misstänkt penningtvätt/);

    const applied = KundRiskprofil.applyRiskhojandeGolv(
      KundRiskprofil.beraknaForeslagenNiva({
        tjanster: [],
        riskfaktorer: KundRiskprofil.itemsFromRiskhojandeFlags(flags, [], katalog)
      }),
      flags,
      katalog
    );
    assert.equal(applied.niva, 'Oacceptabel');
    assert.equal(applied.golv.skikt, 'OACCEPTABEL');

    const marks = KundRiskprofil.marksForRiskhojandeVal(['Misstänkt penningtvätt'], katalog);
    assert.deepEqual(marks.oacceptabel, ['Misstänkt penningtvätt']);
    assert.equal(
      KundRiskprofil.markKindForRiskhojande('Misstänkt penningtvätt', ['Misstänkt penningtvätt'], katalog),
      'OACCEPTABEL'
    );
  });
  ...
  it('höjer till Hög vid Hög-aktiv eller två som bidrar vid kombination', () => {
    const kontanter = KundRiskprofil.beraknaRiskhojandeGolv({
      'Riskhöjande faktorer övrigt': ['Kontanthantering']
    });
    assert.equal(kontanter, null);

    const golvHog = KundRiskprofil.beraknaRiskhojandeGolv({
      'Riskhöjande faktorer övrigt': ['Historik av brott / ekonomisk brottslighet']
    });
    assert.equal(golvHog.niva, 'Hög');
    assert.match(golvHog.drivandeFaktor, /Historik av brott/);

    const fysiskt = KundRiskprofil.beraknaRiskhojandeGolv({
      'Riskhöjande faktorer övrigt': ['Fysiskt möte']
    });
    assert.equal(fysiskt, null);

    const kombi = KundRiskprofil.beraknaRiskhojandeGolv({
      'Riskhöjande faktorer övrigt': ['Otydlig affärsmodell', 'Transaktioner utan tydligt syfte']
    });
    assert.equal(kombi.niva, 'Hög');
    assert.match(kombi.drivandeFaktor, /Otydlig affärsmodell/);
    assert.match(kombi.drivandeFaktor, /Transaktioner utan tydligt syfte/);

    assert.equal(KundRiskprofil.klassForRiskhojande('Historik av brott / ekonomisk brottslighet'), 'GOLV_HOG');
    assert.equal(KundRiskprofil.klassForRiskhojande('Otydlig affärsmodell'), 'BIDRAR_VID_KOMBINATION');
  });

  it('bygger Driven av-kedja med länkar och golvmarkering', () => {
    const fields = { 'Riskhöjande faktorer övrigt': ['Svårt att bekräfta identitet'] };
    const items = KundRiskprofil.itemsFromRiskhojandeFlags(fields, [], null);
    const base = KundRiskprofil.beraknaForeslagenNiva({
      tjanster: [{ namn: 'Bokföring', residualProduct: 12, residualLevel: 'Förhöjd' }],
      riskfaktorer: items
    });
    const calc = KundRiskprofil.applyRiskhojandeGolv(base, fields, null);
    const kedja = KundRiskprofil.buildDrivandeKedja(calc);
    assert.ok(kedja.length >= 2);
    assert.equal(kedja.some((x) => x.namn === 'Bokföring' && x.kind === 'tjänst'), true);
    assert.equal(kedja.some((x) => x.golv && /identitet/i.test(x.namn)), true);
    assert.equal(kedja.every((x) => typeof x.href === 'string' && x.href.endsWith('.html')), true);
  });

  it('kundkort visar Driven av-kedja, katalogversion och default-till-beräknat', () => {
    const js = fs.readFileSync(path.join(__dirname, '../public/js/kundkort.js'), 'utf8');
    assert.match(js, /buildDrivandeKedja/);
    assert.match(js, /Din residual drivs av/);
    assert.match(js, /ai-rb-driven-chip/);
    assert.match(js, /Riskfaktor katalogversion/);
    assert.match(js, /behöver_omprofilering/);
    assert.match(js, /Bedömd residual är förvald till den beräknade nivån/);
    assert.match(js, /Motivering till avvikelse från beräknad nivå/);
  });

});

describe('hogriskBranschLabelFromRiskNamn', () => {
  const KP = require('../public/js/kund-riskprofil');
  it('extraherar etikett från vanliga riskfaktornamn', () => {
    assert.equal(
      KP.hogriskBranschLabelFromRiskNamn('Kunder i högriskbransch: Smycken/antikviteter'),
      'Smycken/antikviteter'
    );
    assert.equal(
      KP.hogriskBranschLabelFromRiskNamn('Byrån har kunder inom högriskbranschen Smycken/antikviteter'),
      'Smycken/antikviteter'
    );
    assert.equal(KP.hogriskBranschLabelFromRiskNamn('Kunden verkar i en högriskbransch'), '*');
    assert.equal(KP.hogriskBranschLabelFromRiskNamn('Distansmöte'), '');
  });
});
