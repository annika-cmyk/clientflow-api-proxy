...
describe('kund-riskprofil', () => {
  ...
  it('låter borttagna default-flaggor stanna borttagna efter sparning', () => {
    ...
    assert.equal(persisted.visible.Kontanthantering, 'GOLV_HOG');
    assert.equal(persisted.visible['Historik av brott eller ekonomisk misskötsel'], undefined);
    assert.equal(persisted.stored['Historik av brott eller ekonomisk misskötsel'], 'BORTTAGEN');
    const merged = KundRiskprofil.mergeRiskhojandeKatalog(persisted.stored);
    assert.equal(merged.Kontanthantering, 'GOLV_HOG');
    assert.equal(merged['Historik av brott eller ekonomisk misskötsel'], undefined);
  });
