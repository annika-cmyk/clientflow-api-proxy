...
/** Residualrisknivåer för kundstockens fördelning (samma skala som RiskSkala). */
const CHOICE_KUND_RESIDUALRISK = ['Låg', 'Normal', 'Förhöjd', 'Hög', 'Oacceptabel'];
...
/** Legacy short labels still present in many Airtable bases before choice sync. */
const LEVERANS_LEGACY_WRITE = {
  distans: 'Distans',
  'pa plats': 'På plats',
  blandat: 'Blandat'
};

function leveranssattLegacyWriteValue(raw) {
  const normalized = normalizeLeveranssatt(raw);
  if (!normalized) return '';
  if (isLeveransEndastDistans(normalized)) return LEVERANS_LEGACY_WRITE.distans;
  if (isLeveransFramstFysiskt(normalized)) return LEVERANS_LEGACY_WRITE['pa plats'];
  return LEVERANS_LEGACY_WRITE.blandat;
}

/**
 * Pick a Leveranssätt value that already exists in Airtable choices.
 * Prefers descriptive labels; falls back to Distans/På plats/Blandat.
 */
function leveranssattForExistingChoices(raw, existingChoiceNames) {
  const normalized = normalizeLeveranssatt(raw);
  if (!normalized) return '';
  const have = new Set(
    (existingChoiceNames || [])
      .map((n) => foldLeveranssatt(n))
      .filter(Boolean)
  );
  if (!have.size) return normalized;
  if (have.has(foldLeveranssatt(normalized))) return normalized;
  const legacy = leveranssattLegacyWriteValue(normalized);
  if (have.has(foldLeveranssatt(legacy))) return legacy;
  return normalized;
}

/**
 * Airtable often wraps rejected select values as ""Value"" (doubled quotes).
 * Also handles "Value", “Value”, and bare Value after "select option".
 */
function extractRejectedSelectOption(message) {
  const msg = String(message || '');
  const patterns = [
    /select option[:\s]*""([^"]+)""/i,
    /select option[:\s]*["“”]([^"“”]+)["“”]/i,
    /select option[:\s]+([^\s"].+?)\s*$/i
  ];
...
const CHOICE_KUNDINTRO = [
  ...
  'Rekommendation via befintliga kunder',
  'Via personligt nätverkande',
...
const BYRA_PROFIL_FIELDS = [
  ...
  {
    key: 'kundResidualriskFordelning',
    airtable: 'Kund residualrisk fördelning',
    type: 'risk-fordelning',
    label: 'Kundernas residualrisk',
    question:
      'Så här många av våra kunder har låg / normal / förhöjd / hög / oacceptabel risk',
    hint:
      'Ange ungefärligt antal kunder per residualrisknivå. Siffrorna syns under Från byråprofilen på Vilka är våra kunder. Om ni hämtar från Clientflow fylls de i utifrån bedömda residualrisker.',
    choices: CHOICE_KUND_RESIDUALRISK,
    section: 'kundstock'
  },
  {
  ...
  // Tjänster med förhöjd risk (sammanslaget med sektion tjanster / Byråns tjänster)
  {
...
const BYRA_PROFIL_SECTIONS = [
  {
    ...
    subtitle:
      'Verksamhetsspecifika omständigheter, ekonomiska beroenden och outsourcing som påverkar inre sårbarhet.'
  },
  {
    id: 'kundstock',
  ...
  {
    id: 'tjanster',
    title: 'Byråns tjänster',
    subtitle: 'Hur tjänsterna utförs och tjänster med förhöjd risk i sig.'
  },
  {
    id: 'geografi',
...
function existingSelectChoiceNames(airtableField) {
  return ((airtableField && airtableField.options && airtableField.options.choices) || [])
    .map((c) => String((c && c.name) || '').trim())
    .filter(Boolean);
}

/**
 * When Meta API cannot add new Leveranssätt choices, rewrite to a value
 * that already exists on the live singleSelect field (often Distans/På plats/Blandat).
 */
function adaptSelectWritesToExistingChoices(fields, tableMeta) {
  const out = { ...(fields || {}) };
  const leveransField = (tableMeta && tableMeta.fields || []).find(
    (f) => (f.name || '').trim() === 'Leveranssätt' && f.type === 'singleSelect'
  );
  if (leveransField && Object.prototype.hasOwnProperty.call(out, 'Leveranssätt') && out['Leveranssätt'] != null) {
    out['Leveranssätt'] = leveranssattForExistingChoices(
      out['Leveranssätt'],
      existingSelectChoiceNames(leveransField)
    );
  }
  return out;
}
...
/**
 * Extra singleSelect-val som skrivs i en patch men saknas i schema-listan
 * (t.ex. egna alternativ från enkäten). Synkas in i Airtable innan patch.
 * @param {Record<string, unknown>} airtableFields
 * @param {typeof BYRA_PROFIL_FIELDS} [fields]
 * @returns {Map<string, string[]>}
 */
function extraSelectChoicesFromAirtableFields(airtableFields, fields = BYRA_PROFIL_FIELDS) {
  const map = new Map();
  const byAirtable = new Map((fields || []).map((f) => [f.airtable, f]));
  Object.keys(airtableFields || {}).forEach((name) => {
    const def = byAirtable.get(name);
    if (!def || def.type !== 'select') return;
    const raw = airtableFields[name];
    if (raw == null) return;
    const label = String(raw).trim();
    if (!label || isBlankWriteValue(label)) return;
    const known = new Set((def.choices || []).map((c) => String(c).trim().toLowerCase()));
...
/**
 * Schema-val + eventuella egna alternativ som ska finnas i Airtable.
 * @param {typeof BYRA_PROFIL_FIELDS} [fields]
 * @param {Map<string, string[]>|Record<string, string[]>|null} [extraByName]
 */
function selectChoicesNeededByAirtableName(fields = BYRA_PROFIL_FIELDS, extraByName = null) {
  const map = selectChoicesByAirtableName(fields);
  if (!extraByName) return map;
  const entries = extraByName instanceof Map
    ? extraByName.entries()
    : Object.entries(extraByName || {});
  for (const [name, extras] of entries) {
    if (!name || !Array.isArray(extras) || !extras.length) continue;
    const list = (map.get(name) || []).slice();
    const have = new Set(list.map((n) => String(n).trim().toLowerCase()));
    extras.forEach((raw) => {
      const label = String(raw || '').trim();
      if (!label) return;
...
function isAnsweredValue(value, field) {
  if (field && (field.type === 'bolagsformer' || field.type === 'branscher' || isRiskFordelningField(field))) {
...
/** Quiz-sektion ↔ analyssida (Övriga riskfaktorer / Vilka är våra kunder). */
const ByraProfilAnalysKoppling = require('../public/js/byra-profil-analys-koppling');
const PROFIL_SECTION_ANALYSIS_TARGETS = ByraProfilAnalysKoppling.TARGETS;
...
function buildKunderEnkatSummary(profil, opts = {}) {
  ...
  return {
    ...
    answeredCount,
    groups,
    enkateHref: ByraProfilAnalysKoppling.enkateHref('kundstock'),
    analysisHref: ByraProfilAnalysKoppling.analysisHref('kundstock')
  };
}
...
module.exports = {
  ...
  CHOICE_BOLAGSFORMER,
  CHOICE_KUND_RESIDUALRISK,
  matchBolagsform,
  parseBolagsformer,
  formatBolagsformer,
  isBolagsformerAnswered,
  isRiskFordelningField,
  isHogriskAnswered,
  isBlankWriteValue,
  toTextOrNull,
  toNumberOrNull,
