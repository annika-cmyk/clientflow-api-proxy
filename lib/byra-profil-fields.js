...
const BYRA_PROFIL_FIELDS = [
  ...
  },
  {
    ...
    hint: 'Lägg till de branscher som är vanligast i kundstocken och ange ungefär hur många kunder per bransch.',
    section: 'kundstock'
  },
  {
...
function buildProfilAirtableFields(body, fields = BYRA_PROFIL_FIELDS) {
  ...
  (fields || []).forEach((def) => {
    ...
    out[def.airtable] = toTextOrNull(
      def.key === 'betalningsmonster'
        ? normalizeBetalningsmonsterValue(body[def.key], def.choices || CHOICE_BETALNINGSMONSTER)
        : body[def.key]
    );
  });
...
function mapProfilFromAirtable(fields) {
  ...
  BYRA_PROFIL_FIELDS.forEach((def) => {
    ...
    if (def.key === 'betalningsmonster' && normalized) {
