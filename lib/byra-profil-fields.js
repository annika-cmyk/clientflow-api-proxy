 ...
 * @typedef {{ key: string, airtable: string, type: 'number'|'text'|'multiline'|'percent'|'select'|'multiselect', label: string, hint?: string, question: string, choices?: string[], section: string, existing?: boolean, optional?: boolean, requiredWhen?: ByraProfilRequiredWhen }} ByraProfilField
 */
...
const BYRA_PROFIL_FIELDS = [
  ...
  },
  {
    ...
    question: 'Vilket bokföringssystem använder byrån i det dagliga arbetet?',
  ...
  },
  {
    ...
    question: 'Vilket bokslutssystem använder byrån?',
  ...
  },
  {
    ...
    question: 'Vilket kundhanteringssystem använder byrån?',
...
function isFieldRequired(field, profil) {
  ...
  if (field.requiredWhen && field.requiredWhen.key) {
    return String((profil || {})[field.requiredWhen.key] || '') === String(field.requiredWhen.equals || '');
  }
...
function unansweredKeys(profil, fields = BYRA_PROFIL_FIELDS) {
  const p = profil || {};
  return fields.filter((f) => isFieldRequired(f, p) && !isAnsweredValue(p[f.key])).map((f) => f.key);
}
