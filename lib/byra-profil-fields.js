...
const CHOICE_BOLAGSFORMER = ['AB', 'Enskild firma', 'HB', 'KB', 'Ekonomisk förening', 'Stiftelse', 'Filial/utländskt bolag', 'Övrigt'];
...
const BYRA_PROFIL_FIELDS = [
  ...
  },
  {
    ...
    question: 'Vilka bolagsformer finns i kundstocken, och ungefär hur många kunder per form?',
    hint: 'Bocka alla som förekommer och ange ett uppskattat antal kunder för respektive form.',
...
function parseBolagsformer(value) {
  ...
  if (!raw) return [];
  // Format: "AB: 40, Enskild firma: 25" eller legacy fritext "AB, enskild firma"
  if (!raw.includes(':')) {
    return raw.split(/[,;|]/).map((s) => s.trim()).filter(Boolean).map((form) => ({ form, count: '' }));
  }
