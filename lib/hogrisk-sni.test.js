...
test('SNI 2025 46872 metallskrot är högrisk liksom gamla 46770', () => {
    const neu = HogriskSni.matchSni('46872 Partihandel med metallavfall och metallskrot');
    assert.deepEqual(neu.branscher, ['Skrot- och metallhandel']);
    assert.deepEqual(neu.codes, ['46872']);
    const old = HogriskSni.matchSni('46770');
    assert.deepEqual(old.branscher, ['Skrot- och metallhandel']);
    assert.ok(HogriskSni.codeMatchesPattern('46873', '^(4677\\d*|4687\\d*)$'));
});

test('withDefaultPatterns behåller Airtable och fyller på SNI 2025-skrot', () => {
    const merged = HogriskSni.withDefaultPatterns([
        { label: 'Skrot- och metallhandel', regex: '46770' }
    ]);
    const found = HogriskSni.matchSni('46872', merged);
    assert.ok(found.branscher.includes('Skrot- och metallhandel'));
    assert.ok(found.codes.includes('46872'));
});
