...
describe('dokument-redigera', () => {
  ...
  it('sätter underkategori KYC under aktuell riskbedömning', () => {
    const plan = planDokumentEdit({
      fields: {
        Dokumentation: [
          { id: 'att1', filename: 'Ola Zaar - Kvalitetsfrågor signerat.pdf', url: 'https://example.com/k.pdf' }
        ],
        'Dokumentation Kategorier': JSON.stringify([
          { filename: 'Ola Zaar - Kvalitetsfrågor signerat.pdf', category: 'ovrigt', createdDate: '2024-09-20' }
        ])
      },
      sourceField: 'Dokumentation',
      sourceIndex: 0,
    ...
    assert.equal(plan.nextCategory, 'riskbedomning');
    assert.equal(plan.subcategory, 'kyc');
    const meta = JSON.parse(plan.patchFields['Dokumentation Kategorier']);
    assert.equal(meta[0].subcategory, 'kyc');
    assert.equal(meta[0].createdDate, '2024-09-20');
  });
});
