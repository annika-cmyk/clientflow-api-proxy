...
describe('utsatt-omrade-styrning', () => {
  ...
  it('aggregateUtsattStat räknar kategorier', () => {
    const stat = aggregateUtsattStat([
      ...
      { fields: { 'Utsatt område (JSON)': JSON.stringify({ trff: false, kontrolleradAt: 'x', geocoding: { ok: true, precision: 'approximate' } }) } },
      { fields: { 'Utsatt område (JSON)': JSON.stringify({ trff: false, kontrolleradAt: 'x', geocoding: { ok: false, reason: 'geocoder_no_hit' } }) } },
      { fields: {} }
    ]);
    assert.equal(stat.antalKontrollerade, 4);
    assert.equal(stat.antalTrff, 1);
    assert.equal(stat.antalEjKontrollerade, 1);
    assert.ok(stat.rader.some((r) => r.namn === STAT_LABELS.SEU));
    assert.ok(stat.rader.some((r) => r.namn === STAT_LABELS.APPROX && r.antal === 1));
    assert.ok(stat.rader.some((r) => r.namn === STAT_LABELS.GEO_FAIL && r.antal === 1));
    assert.ok(stat.rader.some((r) => r.namn === STAT_LABELS.EJ_TRFF && r.antal === 1));
  });
  ...
  it('recordMatchesUtsattStat filtrerar på etikett', () => {
    ...
    const approx = { 'Utsatt område (JSON)': JSON.stringify({ trff: false, kontrolleradAt: 'x', geocoding: { ok: true, precision: 'approximate' } }) };
    assert.equal(recordMatchesUtsattStat(approx, STAT_LABELS.APPROX), true);
    assert.equal(recordMatchesUtsattStat(approx, STAT_LABELS.EJ_TRFF), false);
    assert.equal(recordMatchesUtsattStat(approx, STAT_LABELS.GEO_FAIL), false);
  });

  it('statistik UI gör alla utsatt-område-rader klickbara som chips', () => {
    const fs = require('node:fs');
    const path = require('node:path');
    const js = fs.readFileSync(path.join(__dirname, '../public/js/statistik-riskbedomning.js'), 'utf8');
    assert.match(js, /utsatt-omrade/);
    assert.match(js, /statistik-stat-chip/);
    assert.match(js, /Endast pågående kunder/);
    assert.doesNotMatch(js, /actionable\.has\(u\.namn\)/);
  });
});
