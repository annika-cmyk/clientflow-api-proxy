...
describe('audit-log', () => {
  ...
  it('använder inte System som aktör för HTTP-anrop utan inloggad användare', () => {
    assert.equal(actorFromUser(null, {}), null);
    assert.equal(actorFromUser(null, { user: {} }), null);
    assert.deepEqual(actorFromUser({ email: 'anna@byra.se', name: 'Anna' }, {}), {
      actorId: 'anna@byra.se',
      actorName: 'Anna'
    });
    assert.throws(() => requireHttpActor(null), /inloggad användare/);
    assert.throws(() => requireHttpActor(SYSTEM_ACTORS.system), /inloggad användare/);
    assert.throws(() => normalizeActor(null, { requireNamedActor: true }), /AUDIT_ACTOR|aktör/);
    const named = requireHttpActor({ actorId: 'usr1', actorName: 'Anna Ledare' });
    assert.equal(named.actorId, 'usr1');
  });

  it('arkiverar inneboende kundprofil en gång per kund', async () => {
    assert.equal(ACTION_TYPES.includes('kund_inneboende_arkiverad'), true);
    const store = createMemoryStore();
    const svc = createAuditLogService(store);
    const fields = {
      Namn: 'Test AB',
    ...
    const first = await auditHooks.logInneboendeArchived({
    ...
    assert.equal(first.actionType, 'kund_inneboende_arkiverad');
    assert.equal(first.valueBefore, 'Förhöjd');
    const second = await auditHooks.logInneboendeArchived({
      write: (input, options) => svc.insert(input, options),
      service: svc,
      actor: SYSTEM_ACTORS.system,
      byraId: '7',
    ...
    assert.equal((await svc.getAuditLog('kund', 'recKund')).length, 1);
    assert.equal(await auditHooks.logInneboendeArchived({
      write: (input, options) => svc.insert(input, options),
      service: svc,
      customerId: 'recTom',
      fields: { Namn: 'Utan värde' }
    }), null);
  });
});
