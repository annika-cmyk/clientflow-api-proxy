...
describe('ar-kartlaggning', () => {
  ...
  it('serialiserar historik i AR Kartläggning JSON', () => {
    assert.equal(
      arKartlaggning.serializeKartlaggningJson({ kunder: 'A', historik: 'Track record text' }),
      '{"kunder":"A","historik":"Track record text"}'
    );
    assert.ok(arKartlaggning.KARTLAGGNING_SECTION_KEYS.includes('historik'));
    assert.equal(arKartlaggning.SECTION_LABELS.historik, '2.1.6 Historik och track record');
  });

  it('formatHistorikUnderlag läser byråprofilfält', () => {
    const text = arKartlaggning.formatHistorikUnderlag({
      finanspolisenAvvikelserAntal: 2,
      nearMisses: 'Ja',
      nearMissesDetalj: 'Avböjt kund med oklart kapital'
    });
    assert.match(text, /Avvikelserapporter till Finanspolisen \(antal\): 2/);
    assert.match(text, /Near misses/);
    assert.match(text, /Avböjt kund/);
    assert.equal(
      arKartlaggning.formatHistorikUnderlag({}),
      'Inga historikuppgifter ifyllda i byråprofilen ännu.'
    );
  });

  it('buildAiSystemPrompt för historik fokuserar på track record', () => {
    const prompt = arKartlaggning.buildAiSystemPrompt('historik');
    assert.match(prompt, /2\.1\.6 Historik/);
    assert.match(prompt, /Avvikelserapporter/);
    assert.match(prompt, /Near misses/);
    assert.match(prompt, /inte kundernas branschrisker/);
  });

  it('buildAiUserPrompt för historik använder historikunderlag', () => {
    const prompt = arKartlaggning.buildAiUserPrompt('historik', {
      statistikText: 'Near misses: Ja',
      befintligText: 'Tidigare text'
    });
    assert.match(prompt, /HISTORIKUNDERLAG FRÅN BYRÅPROFIL/);
    assert.match(prompt, /Near misses: Ja/);
    assert.match(prompt, /BEFINTLIG TEXT/);
    assert.doesNotMatch(prompt, /^BYRÅPROFIL:/m);
  });
});
...
describe('allmän riskbedömning HTML – sektion 2 kartläggning', () => {
  ...
  it('har underkort 2.1 med tjänster, kunder, distribution, geografi, verksamhet och historik', () => {
    ...
    assert.match(html, /2\.1\.6 Historik och track record/);
    assert.match(html, /id="fld-ar-historik"/);
    assert.match(html, /data-ai-kartlaggning="historik"/);
    assert.match(html, /id="ar-historik-chips"/);
  ...
  it('JS laddar och sparar kartläggning JSON inkl. historik', () => {
    assert.match(js, /AR Kartläggning \(JSON\)/);
    assert.match(js, /fld-ar-historik/);
    assert.match(js, /key: 'historik'/);
    ...
    assert.match(js, /loadByraProfilHistorik|applyHistorikFokusFromUrl/);
  });
  ...
  it('backend skapar AR Kartläggning-fält vid behov och accepterar historik', () => {
    const indexSrc = fs.readFileSync(path.join(__dirname, '../index.js'), 'utf8');
    assert.match(indexSrc, /ensureArKartlaggningField/);
    assert.match(indexSrc, /arKartlaggning\.KARTLAGGNING_FIELD/);
    assert.match(indexSrc, /AR_KARTLAGGNING_SECTIONS[\s\S]*historik/);
    assert.match(indexSrc, /formatHistorikUnderlag/);
  });
});
