...
function runAnsvarigFromHistory(fields, periodKey) {
  const pk = String(periodKey || '').trim();
  if (!pk) return '';
  const hist = safeJson((fields?.Historik || '').toString().trim(), []);
  if (!Array.isArray(hist)) return '';
  const hit = hist.find((it) => it && String(it.periodKey || '').trim() === pk);
  return hit ? airtableCellName(hit.ansvarig) : '';
}

function resolveRunPerformer(runAnsvarig, uppdragAnsvarig) {
  const run = String(runAnsvarig || '').trim();
  if (run) return { name: run, inherited: false };
  const inherited = String(uppdragAnsvarig || '').trim();
...
function mapUppdragBoardRow(instance) {
  ...
  const uppdragAnsvarig = airtableCellName(f.Ansvarig);
  const histRunAnsvarig = runAnsvarigFromHistory(f, x.periodKey);
  const performer = resolveRunPerformer(histRunAnsvarig, uppdragAnsvarig);
  return {
    ...
    ansvarig: uppdragAnsvarig,
    runAnsvarig: performer.name,
    runAnsvarigInherited: performer.inherited,
