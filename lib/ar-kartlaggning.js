...
const SECTION_LABELS = {
  ...
  historik: '2.1.6 Historik och track record'
};

const KARTLAGGNING_SECTION_KEYS = ['kunder', 'distribution', 'geografi', 'verksamhet', 'historik'];
...
function serializeKartlaggningJson(obj) {
  const out = {};
  KARTLAGGNING_SECTION_KEYS.forEach((key) => {
...
function formatHistorikValue(value) {
  ...
  if (Array.isArray(value)) return value.map((v) => trimStr(v)).filter(Boolean).join(', ');
  return trimStr(value);
}

/** Underlag från byråquiz för AR historik/track record. */
function formatHistorikUnderlag(byraFieldsOrProfil) {
  const f = byraFieldsOrProfil || {};
  const rows = [
    { keys: ['finanspolisenAvvikelserAntal', 'Avvikelserapporter Finanspolisen antal'], label: 'Avvikelserapporter till Finanspolisen (antal)' },
    { keys: ['finanspolisenAvvikelserTyp', 'Avvikelserapporter Finanspolisen typ'], label: 'Typ av misstanke i avvikelserapporter' },
    { keys: ['lanstyrelsenAnmarkningar', 'Anmärkningar Länsstyrelsen tillsyn'], label: 'Anmärkningar från Länsstyrelsens tillsyn' },
    { keys: ['lanstyrelsenAnmarkningarDetalj', 'Anmärkningar Länsstyrelsen detalj'], label: 'Beskrivning av tillsynsanmärkningar' },
    { keys: ['nearMisses', 'Near misses'], label: 'Near misses (avböjt/avslutat pga misstanke eller risk)' },
    { keys: ['nearMissesDetalj', 'Near misses detalj'], label: 'Beskrivning av near misses' }
  ];
  const lines = [];
  rows.forEach((row) => {
    let val = '';
    for (let i = 0; i < row.keys.length; i++) {
      val = formatHistorikValue(f[row.keys[i]]);
      if (val) break;
    }
    if (val) lines.push(`${row.label}: ${val}`);
  });
  return lines.length
    ? lines.join('\n')
    : 'Inga historikuppgifter ifyllda i byråprofilen ännu.';
}
...
För historik och track record ska du ENDAST beskriva byråns egna erfarenheter och vad de innebär för riskbedömning och kontroller – inte kundernas branschrisker eller geografiska hemvist.

Fokusera på:
- Avvikelserapporter till Finanspolisen (antal och typ av misstanke) och vad det säger om byråns upptäcktsförmåga
- Anmärkningar från Länsstyrelsens tillsyn och hur de påverkar kontrollmiljön
- Near misses (avböjda eller avslutade uppdrag pga misstanke/risk) och lärdomar för trösklar och arbetssätt
- Konkret koppling till den allmänna riskbedömningen: vilka risker som bekräftats och vilka åtgärder som bör förstärkas

Om underlaget saknar uppgifter: skriv kort att historiken är begränsad/oklar och vilka uppgifter byrån bör dokumentera.`;
  }
  return prompt;
}
...
function buildAiUserPrompt(section, ctx) {
  ...
  const underlagRubrik = section === 'verksamhet'
    ? 'BYRÅDATA (endast byråns egna omständigheter):'
    : section === 'historik'
      ? 'HISTORIKUNDERLAG FRÅN BYRÅPROFIL:'
      : 'STATISTIK OCH UNDERLAG:';
  const emptyUnderlag = section === 'verksamhet'
    ? 'Ingen byrådata.'
    : section === 'historik'
      ? 'Inga historikuppgifter.'
      : 'Ingen statistik.';
  const parts = [
    `Skriv avsnittet "${label}" för byråns allmänna riskbedömning.`,
    '',
    underlagRubrik,
    ctx.statistikText || emptyUnderlag,
    ''
  ];
  if (ctx.byraProfil && section !== 'verksamhet' && section !== 'historik') {
  ...
  if (section === 'historik') {
    parts.push(
      'VÄGLEDNING: Analysera track record konkret. Koppla fakta till risker och åtgärder. Hitta inte på antal eller händelser som saknas i underlaget.',
      ''
    );
  }
  if (infotext) {
