...
describe('kyc-huvudman', () => {
  ...
  it('markerar utländsk hemvist och styr riskflaggan', () => {
    ...
    assert.equal(HM.hasForeignFromKyc({
      huvudman: [{ namn: 'Anna', skatterattslig_hemvist: 'Sverige' }],
      foretradare: [{ namn: 'Bo', skatterattslig_hemvist: 'Finland' }]
    }), true);
    assert.equal(HM.hasForeignFromKyc({
      foretradare: [{ namn: 'Bo', skatterattslig_hemvist: 'Sverige' }]
    }), false);
    assert.equal(HM.isUboFlagLabel('Kunder med utländska huvudmän'), true);
  ...
  it('har KYC-rader och koppling till riskbedömningen', () => {
    ...
    assert.match(js, /_maybeSteerUboFromSavedKyc/);
    assert.match(js, /_collectKycForetradare/);
