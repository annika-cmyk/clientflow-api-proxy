/**
 * Lager C — Kundresa (fas 2 embryo): steg som pekar in i befintliga kundkort-ytor.
 * Statusmotor / koordinator — inte en låst wizard. BankID-signering är comingSoon.
 * VH-steget utelämnas för enskild firma / fysisk person (samma regel som vhIsRelevant).
 */
...
/** Stegmall enligt arkitektur fas 2 (3 kap PTL). VH utelämnas när ej aktuell. */
...
/**
 * Stegmall filtrerad efter bolagsform. VH (vh_ombud) utelämnas när ej aktuell.
 */
function visibleKundresaSteps(bolagsform) {
  if (Kundformular.vhIsRelevant(bolagsform)) return KUNDRESA_STEPS.slice();
  return KUNDRESA_STEPS.filter((step) => step.key !== 'vh_ombud');
}

/**
 * Bygg stegstatusrad. Steg efter VH-gate markeras gated när blocked.
 * När VH inte är aktuell ingår steget inte i listan (räknas inte i progress).
 */
function resolveKundresaSteps(ctx = {}) {
  ...
  const template = visibleKundresaSteps(bolagsform);
