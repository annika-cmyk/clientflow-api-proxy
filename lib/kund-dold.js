const DOLD_FIELD = 'Dold';
const DOLD_DATUM_FIELD = 'Dold datum';
const DOLD_AV_FIELD = 'Dold av';

function isKundDold(fields) {
  const v = (fields || {})[DOLD_FIELD];
  return v === true || v === 'true' || v === 1 || v === '1';
}

/** Normaliserat Kundstatus-värde (hanterar array/objekt från Airtable). */
function kundstatusValue(fields) {
  const raw = (fields || {}).Kundstatus;
  if (Array.isArray(raw)) {
    return raw
      .map((x) => {
        if (x && typeof x === 'object' && x.name != null) return String(x.name).trim();
        return String(x == null ? '' : x).trim();
      })
      .filter(Boolean)
      .join(' ');
  }
  if (raw && typeof raw === 'object' && raw.name != null) {
    return String(raw.name).trim();
  }
  return String(raw == null ? '' : raw).trim();
}

function kundstatusFolded(fields) {
  return kundstatusValue(fields).toLowerCase().normalize('NFC');
}

function isAvslutadKund(fields) {
  const v = kundstatusFolded(fields);
  if (!v) return false;
  // Avslutad / Avslutat / Avslutade / Avslutad kund m.fl.
  return /^(avslutad|avslutat|avslutade)(\s|$)/.test(v) || v === 'avslutad kund';
}

function isLeadKund(fields) {
  const v = kundstatusFolded(fields);
  return v === 'lead';
}

/** Explicit pågående, eller saknad status (äldre poster utan Kundstatus). */
function isPagandeKund(fields) {
  const v = kundstatusFolded(fields);
  if (!v) return true;
  return (
    v === 'pågående kund'
    || v === 'pågående'
    || v === 'pagaende kund'
    || v === 'pagaende'
  );
}

/**
 * Pågående kund (eller saknad status) som inte är dold, lead eller avslutad.
 * Används för Clientflow-statistik: leads och avslutade räknas inte.
 */
function isAktivKund(fields) {
  if (isKundDold(fields) || isAvslutadKund(fields) || isLeadKund(fields)) return false;
  return isPagandeKund(fields);
}

function filterAktivaKunder(records) {
  return (records || []).filter((r) => isAktivKund(r && r.fields));
}

function shouldShowRaderaKund(fields) {
  return isAvslutadKund(fields) && !isKundDold(fields);
}

function filterVisibleKunder(records) {
  return (records || []).filter((r) => !isKundDold(r && r.fields));
}

function filterDoldaKunder(records) {
  return (records || []).filter((r) => isKundDold(r && r.fields));
}

function mapDoldKundListItem(record) {
  const f = (record && record.fields) || {};
  const id = record && record.id ? String(record.id) : '';
  return {
    id,
    namn: f.Namn || f['Företagsnamn'] || 'Namn saknas',
    orgnr: f.Orgnr || f.Organisationsnummer || f['Org.nr'] || '',
    kundstatus: f.Kundstatus || '',
    doldDatum: f[DOLD_DATUM_FIELD] || '',
    doldAv: f[DOLD_AV_FIELD] || '',
    kundkortUrl: id ? `kundkort.html?id=${encodeURIComponent(id)}` : ''
  };
}

function hideFields(actorName, at = new Date().toISOString()) {
  return {
    [DOLD_FIELD]: true,
    [DOLD_DATUM_FIELD]: at,
    [DOLD_AV_FIELD]: String(actorName || '').trim()
  };
}

function unhideFields() {
  return {
    [DOLD_FIELD]: false
  };
}

module.exports = {
  DOLD_FIELD,
  DOLD_DATUM_FIELD,
  DOLD_AV_FIELD,
  kundstatusValue,
  isKundDold,
  isAvslutadKund,
  isLeadKund,
  isPagandeKund,
  isAktivKund,
  shouldShowRaderaKund,
  filterVisibleKunder,
  filterAktivaKunder,
  filterDoldaKunder,
  mapDoldKundListItem,
  hideFields,
  unhideFields
};
