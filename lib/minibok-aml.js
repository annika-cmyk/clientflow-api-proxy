...
function fieldStr(fields, key) {
  ...
  if (Object.prototype.hasOwnProperty.call(fields, key)) {
    return airtableCellStr(fields[key]);
  }
  const trimmed = String(key).trim();
  if (Object.prototype.hasOwnProperty.call(fields, trimmed)) {
    return airtableCellStr(fields[trimmed]);
  }
  const wanted = normalizeFieldKey(key);
  if (!wanted) return '';
  const entries = Object.entries(fields);
  for (let i = 0; i < entries.length; i++) {
    if (normalizeFieldKey(entries[i][0]) === wanted) return airtableCellStr(entries[i][1]);
  }
  return '';
}
...
/** SNI-koder från "62010 Dataprogrammering" / "70200 - Konsult…, 01500 - …". */
...
function mapCustomerAmlRisk(record, extra) {
  ...
  const industryCodes = parseSniCodes(
    f['SNI kod'] || f['SNI-koder'] || (kyc && (kyc.sniKoder || kyc.sniCodes)) || ''
  );
  const internationalTrade = parseInternationalTrade(f, kyc);
  const businessSummary =
    firstFieldStr(f, ['Verksamhet', 'Beskrivning av kunden']) ||
    (kyc && kyc.verksamhet ? String(kyc.verksamhet).trim() : '') ||
    fieldStr(f, 'Verksamhetsbeskrivning') ||
    '';

  const riskRaisingFactors = []
    .concat(parseChipList(f['Kunden verkar i en högriskbransch']))
    .concat(parseChipList(f['Riskhöjande faktorer tjänster']))
    .concat(parseChipList(f['Riskhöjande faktorer övrigt']));
  ...
  return {
    ...
    businessSummary,
    customerDescription: fieldStr(f, 'Beskrivning av kunden') || '',
    bolagsverketSummary: fieldStr(f, 'Verksamhetsbeskrivning') || '',
    ...
    riskReducingFactors: parseChipList(f['Risksänkande faktorer']),
    riskRaisingFactors,
    riskFactorComment: fieldStr(f, 'Kommentar till riskfaktorerna ovan') || '',
    rationale,
    ...
    kycDoneAt: fieldStr(f, 'KYC UTFÖRD DATUM') || null,
    document: null,
  };
}
...
function createMinibokAml({
  ...
  async function buildCustomerPayload(record, byraId) {
    ...
    const f = (record && record.fields) || {};
    const [behoriga, clientflowNotes] = await Promise.all([
      resolveApplicationUsers(parseLinkedIds(f['Användare'])),
