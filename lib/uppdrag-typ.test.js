...
describe('uppdrag-typ', () => {
  ...
  it('sätter årsvis först för bokslut och deklaration', () => {
    const bokslut = UppdragTyp.frekvensChoicesForTyp('Bokslut', false);
    const dek = UppdragTyp.frekvensChoicesForTyp('Deklaration', false);
    assert.equal(bokslut[0], 'Årsvis');
    assert.equal(dek[0], 'Årsvis');
    assert.ok(bokslut.includes('Engång'));
    assert.ok(dek.includes('Engång'));
  });
});
