...
function extractPeopleFromFields(fields) {
  ...
  const huvudmanList = Array.isArray(kycObj.huvudman)
    ? kycObj.huvudman
    : (Array.isArray(kycObj.verkligaHuvudman) ? kycObj.verkligaHuvudman : []);
  huvudmanList.forEach((p) => {
    if (!p || !(p.namn || p.name || p.personnr || p.personnummer)) return;
    pushPerson(people, {
      namn: p.namn || p.name,
      personnr: p.personnr || p.personnummer,
  ...
  if (huvudmanInfo && !huvudmanList.length) {
