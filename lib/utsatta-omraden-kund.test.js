...
describe('utsatta-omraden-kund', () => {
  ...
  it('shouldRecheck vid ny adress, force eller tidigare geokodningsfel', () => {
    ...
    const geoFail = toStored({
      trff: false,
      geocoding: { ok: false, reason: 'geocoder_no_hit' },
      kontrolleradAt: '2026-01-01'
    }, 'Gatan 1');
    assert.equal(shouldRecheck(geoFail, 'Gatan 1', false), true);
  });
  ...
  it('statistik kan öppna kundlista för utsatta områden', () => {
    ...
    assert.match(js, /typ:\s*'utsatt-omrade'/);
    assert.match(js, /fetchKunderForRow\('utsatt-omrade'/);
    assert.match(index, /typ === 'utsatt-omrade'/);
    assert.match(html, /statistik-riskbedomning\.js\?v=9/);
  });

  it('bulk-script stödjer --failed-only för omkörning av geokodningsfel', () => {
    const src = fs.readFileSync(path.join(__dirname, '../scripts/bulk-utsatt-omrade.js'), 'utf8');
    assert.match(src, /--failed-only/);
    assert.match(src, /isFailedGeocode/);
    assert.match(src, /geocoding\.ok === false/);
    assert.match(src, /geokodade:/);
    assert.match(src, /isAktivKund/);
  });
});
