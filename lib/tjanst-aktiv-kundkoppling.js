...
/**
 * Hitta egna (custom:) katalogposter som tas bort mellan två tillstånd.
 * Standardmallar kan inte raderas — bara inaktiveras.
 */
function findRemovals(prevState, nextState) {
  const prev = cloneState(prevState);
  const next = cloneState(nextState);
  const out = [];
  for (const mallId of Object.keys(prev.tjanster || {})) {
    if (!TjanstUtforandeMallar.isCustomId(mallId)) continue;
    if (Object.prototype.hasOwnProperty.call(next.tjanster || {}, mallId)) continue;
    out.push({
      mallId,
      namn: entryNamn(mallId, prev.tjanster[mallId])
    });
  }
  return out;
}
...
/**
 * Första radering av egen tjänst som blockeras av kundkoppling, eller null.
 */
function findBlockedDeletion(prevState, nextState, riskRecords, kundAntalTjanster) {
  for (const item of findRemovals(prevState, nextState)) {
    const count = kundCountForNamn(item.namn, riskRecords, kundAntalTjanster);
    if (count > 0) {
      return {
        ...item,
        kundCount: count,
        message: count === 1
          ? `Kan inte radera «${item.namn}»: 1 kund har tjänsten. Ta bort den från kunden först.`
          : `Kan inte radera «${item.namn}»: ${count} kunder har tjänsten. Ta bort den från kunderna först.`
      };
    }
  }
  return null;
