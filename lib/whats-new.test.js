...
describe('whats-new', () => {
  it('har inga kvarlämnade git-merge-markörer (Render-boot)', () => {
    // Konfliktmarkörer i den här filen har tidigare kraschat node-require
    // vid deploy (SyntaxError) — samma klass av fel som stray } i gmail/api.js.
    const src = fs.readFileSync(path.join(__dirname, 'whats-new.js'), 'utf8');
    assert.equal(/^(<<<<<<<|=======|>>>>>>>)/m.test(src), false);
  });
