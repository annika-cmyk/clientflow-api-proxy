...
function renderUtforandeCardHtml(existing, opts = {}) {
  ...
  global.RiskSkala = RiskSkala;
  global.AnalyskortMall = AnalyskortMall;
  ...
  const ctx = {
    ...
    formatCatalogRiskBadge(label) {
      return AnalyskortMall.formatCatalogRiskBadge(label);
    },
...
describe('tjanst-utforande-mallar', () => {
  ...
  it('årsredovisning har fyra utförandefrågor utan upprättar- och bank/investerare-frågan', () => {
    const template = Mallar.templateById('arsredovisning');
    const qs = Mallar.questionsForTemplate(template);
    const serviceQs = qs.filter((q) => q.id !== 'hamtaClientflowStatistik' && q.id !== 'antalKunderTjanst');
    assert.equal(serviceQs.length, 4);
    ...
    assert.ok(!qs.some((q) => q.id === 'arUpprattar'));
    assert.ok(!qs.some((q) => /Upprättar byrån årsredovisningen/i.test(q.label || '')));
  ...
  it('sidan visar katalogval och Klarmarkera-stöd i enkäten', () => {
    ...
    assert.match(js, /data-has-overview|toggleUtforandeOverview/);
  ...
  it('kortlistan använder växel i menyn, progresscirkel och redigera i trepunktsmenyn', () => {
    const js = fs.readFileSync(path.join(__dirname, '../public/js/riskbedomning-byra.v5.js'), 'utf8');
    const css = fs.readFileSync(path.join(__dirname, '../public/styles.css'), 'utf8');
    assert.match(js, /tjanst-mall-switch/);
    assert.match(js, /tjanst-mall-progress/);
    assert.match(js, /renderUtforandeProgressIcon/);
    assert.match(js, /Aktivera tjänsten/);
    assert.match(js, /Inaktivera tjänsten/);
    assert.match(js, /risk-row-menu-item tjanst-mall-edit/);
    assert.match(js, /formatCatalogRiskBadge/);
    assert.doesNotMatch(js, /utforandeServiceIcon/);
    assert.doesNotMatch(js, /tjanst-mall-icon/);
    ...
    assert.match(css, /\.tjanst-mall-progress\.is-complete/);
    assert.match(css, /\.tjanst-mall-progress\.is-partial/);
    ...
    assert.match(withAnalysis, /tjanst-mall-progress is-empty/);
    assert.doesNotMatch(withAnalysis, /fas fa-scale-balanced/);
    assert.doesNotMatch(withAnalysis, /tjanst-mall-icon/);
    assert.match(withAnalysis, /tjanst-mall-desc/);
    assert.match(withAnalysis, /risk-row-menu-item tjanst-mall-edit/);
    ...
    assert.match(withAnalysis, /data-has-overview/);
    assert.match(withAnalysis, /data-utforande-aktiv-toggle/);
    assert.doesNotMatch(withAnalysis, /Visa översikt/);
    assert.doesNotMatch(withAnalysis, /data-toggle-overview/);
    ...
    assert.doesNotMatch(js, /fa-calendar-check/);
    assert.doesNotMatch(js, /fa-calculator/);
    ...
    assert.match(css, /risknivå visas bara i badges/i);
    ...
    assert.match(oacceptabelCard, /tjanst-mall-progress is-empty/);
    assert.doesNotMatch(oacceptabelCard, /tjanst-mall-icon/);
    const partialCard = renderUtforandeCardHtml({
      id: 'risk-partial',
      fields: {
        'Task Name': 'Bokslut',
        Riskpoäng: JSON.stringify({
          sannolikhet: 2,
          konsekvens: 2,
          klarmarkeradeFlikar: ['utforande', 'oversikt', 'hot']
        })
      }
    }, {
      template: { id: 'bokslut', name: 'Bokslut', description: '' },
      entry: { aktiv: true, namn: 'Bokslut' },
      useRealRiskMeta: true
    });
    assert.match(partialCard, /tjanst-mall-progress is-partial/);
    assert.match(partialCard, /--progress:43/);
    assert.match(partialCard, /3\/7 klara/);
    const klarCard = renderUtforandeCardHtml({
      id: 'risk-klar',
      fields: {
        'Task Name': 'Bokslut',
        Riskpoäng: JSON.stringify({
          sannolikhet: 2,
          konsekvens: 2,
          klarmarkeradeFlikar: ['utforande', 'oversikt', 'hot', 'sarbarhet', 'inneboende', 'atgard', 'residual']
        })
      }
    }, {
      template: { id: 'bokslut', name: 'Bokslut', description: '' },
    ...
    assert.match(klarCard, /tjanst-mall-progress is-complete/);
    assert.match(klarCard, /is-resa-complete/);
    assert.match(klarCard, /tjanst-mall-klar-badge is-complete/);
  ...
  it('kortet öppnar riskbedömning med utförandefrågor i modalens utförandeflika', () => {
    ...
    const pageHtml = require('node:fs').readFileSync(require('node:path').join(__dirname, '../public/riskbedomning-byra.html'), 'utf8');
    ...
    assert.match(pageHtml, /analyskort-mall\.js/);
    assert.match(pageHtml, /data-analyskort-modals="tjanst"/);
    ...
    assert.match(html, /data-resa-blob="utforande"/);
    ...
    const blobOrder = [...html.matchAll(/data-resa-blob="([^"]+)"/g)].map((m) => m[1]);
    assert.deepEqual(blobOrder, tabOrder);
    ...
    assert.match(js, /onToggleKlar/);
    assert.match(js, /markTjanstFlikKlar/);
    assert.match(js, /advanceTjanstResaAfterKlar/);
    assert.match(js, /turningOn/);
    assert.match(js, /persistKlarmarkeringViaSave/);
    assert.match(js, /putRiskAssessment/);
    ...
    assert.match(js, /markTjanstFlikKlar\('utforande'\)/);
    assert.match(js, /markTjanstFlikKlar\('utforande',\s*\{\s*advance:\s*false\s*\}\)/);
    assert.match(js, /autosizeTjanstTextareas/);
    assert.match(js, /syncEditingRiskKlarmarkering/);
    assert.doesNotMatch(js, /persistKlarmarkeringQuietly/);
    ...
    assert.match(css, /tjanst-panel-klar-footer/);
    assert.match(css, /tjanst-resa-progress/);
    assert.match(html, /tjanst-panel-klar-footer[\s\S]*?id="tjanst-klarmarkera-btn"/);
    assert.match(
      html,
      /<div class="tjanst-panel-klar-bar">\s*<span id="tjanst-resa-progress" class="tjanst-resa-progress" hidden><\/span>\s*<\/div>/
    );
  ...
  it('låter kort med analys fälla ut översikt utan att öppna redigera', () => {
    ...
    assert.match(js, /toggleUtforandeOverview/);
    assert.match(js, /data-has-overview/);
    assert.match(js, /utforandeCardClickShouldIgnore/);
    assert.match(js, /tjanst-mall-summary-heading/);
    assert.match(js, /Hot och modus/);
    assert.match(js, /Riskreducerande åtgärder/);
    assert.doesNotMatch(js, /Utförandesvar/);
    assert.doesNotMatch(js, /renderUtforandeOverviewAnswers/);
    ...
    assert.match(html, /riskbedomning-byra\.v5\.js\?v=\d+/);
    assert.doesNotMatch(js, /Visa översikt/);
    assert.doesNotMatch(js, /Dölj översikt/);
    assert.match(css, /\.tjanst-mall-body:has\(> \.tjanst-mall-overview\[hidden\]\)/);
    ...
    assert.match(withAnalysis, /data-has-overview/);
    assert.match(withAnalysis, /data-utforande-aktiv-toggle/);
    assert.match(withAnalysis, />Inaktivera</);
    assert.doesNotMatch(withAnalysis, /Visa översikt/);
    assert.doesNotMatch(withAnalysis, /tjanst-mall-expand/);
    assert.match(withAnalysis, /tjanst-mall-overview/);
    assert.match(withAnalysis, /data-delete-tjanst/);
    assert.doesNotMatch(withAnalysis, /Låt AI skapa ett utkast/);
    assert.doesNotMatch(withAnalysis, /type="checkbox"/);
    ...
    assert.doesNotMatch(empty, /data-has-overview/);
    assert.match(empty, /data-utforande-aktiv-toggle/);
    assert.match(empty, />Skapa analys</);

    const overviewMatch = js.match(/renderUtforandeOverview\(risk, entry, template\) \{([\s\S]*?)\n    \}/);
    assert.ok(overviewMatch, 'hittar renderUtforandeOverview');
    const overviewFn = new Function('risk', 'entry', 'template', overviewMatch[1]);
    const RiskSkala = require('../public/js/risk-skala');
    const overviewCtx = {
      esc(value) { return String(value ?? ''); },
      parseJsonField(v) {
      ...
      stripEvidensLeakFromText(t) { return String(t || ''); },
      normalizeAtgardStatus(s) { return String(s || '').toLowerCase(); },
      renderDiscreteKalla() { return ''; },
      renderOverviewSemanticPanels(kind, categoryLabel, iconClass, items, mapItem) {
        if (!items || !items.length) return '';
        return `<article class="tjanst-ov-panel tjanst-ov-panel--${kind}"><span>${categoryLabel}</span></article>`;
      }
    };
    ...
    let overviewHtml;
    try {
      overviewHtml = overviewFn.call(overviewCtx, {
        fields: {
          Riskbedömning: 'Oacceptabel',
          'Motivering inneboende risk': 'Hög exponering',
          Tjänstebeskrivning: 'Betalningshantering',
          Hot: JSON.stringify([{ titel: 'Felaktiga betalningar', beskrivning: 'Risk' }]),
          Sårbarheter: JSON.stringify([{ titel: 'Begränsad bemanning', beskrivning: 'Risk' }]),
          'Tjänstespecifika åtgärder': JSON.stringify([{ titel: 'Kontroll', beskrivning: 'Görs', status: 'befintlig' }])
        }
      }, {
        answers: {
          klientmedelskonto: 'Nej',
          betalningarNormalt: ['Löner', 'Skatter']
        }
      }, Mallar.templateById('betalningsuppdrag') || { id: 'betalningsuppdrag' });
    } finally {
      if (prevWin === undefined) delete global.window;
      else global.window = prevWin;
    }
    assert.match(overviewHtml, /Risknivå/);
    assert.match(overviewHtml, /Tjänsten/);
    assert.match(overviewHtml, /tjanst-ov-panel--hot/);
    assert.doesNotMatch(overviewHtml, /Utförandesvar/);
    assert.doesNotMatch(overviewHtml, /klientmedelskonto|Har byrån klientmedelskonto/i);
  });
  ...
  it('förklarar närståendefrågan för anläggningsregister', () => {
    const template = Mallar.templateById('anlaggningsregister');
    const qs = Mallar.questionsForTemplate(template);
    const narstaende = qs.find((q) => q.id === 'anlNarstaende');
    assert.ok(narstaende);
    assert.match(narstaende.helpText || '', /ägare, närstående eller koncernbolag/i);
    assert.match(narstaende.helpText || '', /felaktig värdering|värde flyttas/i);
  });
  ...
  it('sparar egna svar i kommentarer och inkluderar dem i AI-format', () => {
    const template = Mallar.templateById('momsredovisning');
    const state = Mallar.upsertEntry(Mallar.emptyState(), 'momsredovisning', {
      aktiv: true,
      answers: {
        hamtaClientflowStatistik: 'Ja',
        momsGor: 'Ja, upprättar'
      },
      kommentarer: {
        momsGor: 'Vi lämnar också in för några kunder via Ombud'
      }
    });
    const entry = Mallar.getEntry(state, 'momsredovisning');
    assert.equal(entry.kommentarer.momsGor, 'Vi lämnar också in för några kunder via Ombud');
    const formatted = Mallar.formatAnswersForAi(template, entry);
    const row = formatted.rows.find((r) => r.id === 'momsGor');
    assert.ok(row);
    assert.equal(row.svar, 'Ja, upprättar');
    assert.match(row.kommentar, /Ombud/);
  });

  it('inkluderar enbart eget svar när inget chip är valt', () => {
    const template = Mallar.templateById('momsredovisning');
    const formatted = Mallar.formatAnswersForAi(template, {
      answers: { hamtaClientflowStatistik: 'Ja', momsGor: 'Ja, upprättar' },
      kommentarer: {
        momsOvanliga: 'Stickprov vid EU-handel varje kvartal'
      }
    });
    const row = formatted.rows.find((r) => r.id === 'momsOvanliga');
    assert.ok(row);
    assert.equal(row.svar, '(eget svar)');
    assert.match(row.kommentar, /Stickprov/);
    assert.ok(!formatted.unanswered.includes(row.label));
  });

  it('renderar diskret penna för chip-frågor med ifyllt eget svar', () => {
    const js = fs.readFileSync(path.join(__dirname, '../public/js/riskbedomning-byra.v5.js'), 'utf8');
    const match = js.match(/renderUtforandeQuestion\(mallId, question, entry\) \{([\s\S]*?)\n    \}/);
    assert.ok(match, 'hittar renderUtforandeQuestion');
    const fn = new Function('mallId', 'question', 'entry', match[1]);
    const ctx = {
      esc(value) { return String(value ?? ''); }
    };
    global.window = { TjanstUtforandeMallar: Mallar };
    try {
      const html = fn.call(ctx, 'momsredovisning', {
        id: 'momsGor',
        label: 'Upprättar eller lämnar byrån momsredovisning åt kunden?',
        type: 'single',
        options: ['Ja, upprättar', 'Nej']
      }, {
        answers: { momsGor: 'Ja, upprättar' },
        kommentarer: { momsGor: 'Komplettering' }
      });
      assert.match(html, /data-note-pen/);
      assert.match(html, /tjanst-mall-note-pen is-filled/);
      assert.match(html, /Komplettering/);
      assert.doesNotMatch(html, /data-comment-add/);
      const textHtml = fn.call(ctx, 'custom:x', {
        id: 'egenBeskrivning',
        label: 'Beskriv',
        type: 'text'
      }, { answers: {}, kommentarer: {} });
      assert.doesNotMatch(textHtml, /data-note-pen/);
    } finally {
