...
describe('tjanst-utforande-mallar', () => {
  ...
  it('removeEntry tar bort egna tjänster men lämnar standardmallar', () => {
    ...
    assert.ok(afterMall.tjanster.bokslut);
    const cards = Mallar.listCatalogCards(afterCustom);
  ...
  it('sidan visar katalogval och Klarmarkera-stöd i enkäten', () => {
    ...
    assert.match(css, /byra-enkate-bolagsformer[\s\S]*max-width:\s*38rem/);
    assert.match(css, /grid-template-columns:\s*minmax\(0,\s*1fr\)\s*5rem/);
  });
  ...
  it('kortlistan använder växel, ikonbadge och ghost-redigera', () => {
    ...
    assert.doesNotMatch(withAnalysis, /data-delete-tjanst/);
