...
describe('tjanst-utforande-mallar', () => {
  it('har 15 standardtjänster med korta AML-frågor utan gemensamma basfrågor', () => {
    ...
    Mallar.SERVICE_TEMPLATES.forEach((t) => {
      ...
      assert.ok(qs.length >= 4 && qs.length <= 8, t.id);
      assert.ok(qs.every((q) => q.type === 'multi' || q.type === 'single'));
      assert.ok(!qs.some((q) => q.id === 'aterkommande'));
      assert.ok(!qs.some((q) => /varierar mellan kunder/i.test((q.options || []).join(' '))));
    });
  ...
  it('kortet växlar mellan utförandefrågor och riskbedömning', () => {
    ...
    assert.match(css, /data-panel="analys"] \.risk-content-section \{\s*margin:\s*0;\s*padding:\s*1\.45rem 0 1\.55rem/);
  ...
  it('visar ROT-följdfrågor bara när tjänsten utförs', () => {
    ...
    const hidden = Mallar.formatAnswersForAi(rot, { answers: { rotHjalper: 'Nej' }, kommentarer: {} });
    assert.equal(hidden.rows.length, 1);
    assert.equal(hidden.unanswered.length, 0);
    const open = Mallar.formatAnswersForAi(rot, { answers: { rotHjalper: 'Ja' }, kommentarer: {} });
