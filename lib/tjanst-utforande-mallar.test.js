...
function renderUtforandeCardHtml(existing) {
...
describe('tjanst-utforande-mallar', () => {
  ...
  it('kortet öppnar riskbedömning med utförandefrågor i modalens utförandeflika', () => {
    ...
    assert.match(html, /tjanst-tab-icon/);
    ...
    assert.match(css, /tjanst-tab-icon/);
    assert.match(css, /\.tjanst-tabs\s*\{[^}]*flex-direction:\s*column/);
    assert.match(css, /@media \(max-width: 960px\)[\s\S]*?\.tjanst-tabs\s*\{[^}]*flex-direction:\s*row/);
