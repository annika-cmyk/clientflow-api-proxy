...
describe('tjanst-utforande-mallar', () => {
  ...
  it('sidan visar katalogval och Klarmarkera-stöd i enkäten', () => {
    ...
    assert.match(js, /setUtforandeKatalogFooterVisible|addStandardUtforandeTjanst/);
  ...
  it('låter kort med analys fälla ut översikt utan att öppna redigera', () => {
    ...
    assert.match(js, /tjanst-mall-summary-answers/);
    assert.match(js, /Dölj översikt/);
    assert.match(css, /\.tjanst-mall-expand\.btn-ghost/);
    assert.match(css, /tjanst-mall-summary-answers/);
    assert.match(css, /tjanst-utforande-katalog \.tjanst-mall-actions/);
    assert.match(html, /riskbedomning-byra\.v5\.js\?v=55/);
