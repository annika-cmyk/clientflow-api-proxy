...
function verksamhetRecordsFromList(records) {
  return kundRecordsFromList(records);
}
...
function kycHasHogriskHandel(kyc) {
  ...
  const raw = parsed.internationellaLander || '';
  if (!String(raw).trim()) return false;
  const labels = EuHogriskLander.parseLabels
    ? EuHogriskLander.parseLabels(raw)
    : String(raw).split(',').map((s) => s.trim()).filter(Boolean);
  if (!labels.length) return false;
  const result = EuHogriskLander.assess ? EuHogriskLander.assess(labels) : {};
...
function templateExistsForFactor(records, factorId) {
  ...
  if (!factor) return false;
  return kundRecordsFromList(records).some((rec) => {
...
function suggestedRecordIds(records, kyc) {
  const parsed = parseKyc(kyc);
  const out = [];
  for (const rec of kundRecordsFromList(records)) {
    const matched = matchFactor(recordNamn(rec));
...
function countKycJa(kunder, factorId) {
  ...
  for (const rec of kunder || []) {
    ...
    if (factorActive(kyc, factor)) count += 1;
  }
  return count;
}

function buildSaknadeRiskfaktorer(kunder, templates) {
  const verksamhet = kundRecordsFromList(templates);
