...
describe('atgard-konkret', () => {
  it('nekar vaga avsikter som Inför striktare krav', () => {
    ...
    assert.match(check.error, /inför/);
  });
  ...
  it('godkänner införd åtgärd även om syftet nämner förbättra', () => {
    const check = Atgard.assessAtgard({
      titel: 'Automatiserade avstämningar',
      beskrivning: 'Vi har implementerat programvara ("minibok") för att automatisera och förbättra noggrannheten i avstämningar.'
    });
    assert.equal(check.ok, true);
  });

  it('säger vilket ord som stoppade sparandet', () => {
    const check = Atgard.assessAtgard({
      titel: 'Dokumentation',
      beskrivning: 'Förbättra dokumentationen kring bokslutet för alla transaktioner.'
    });
    assert.equal(check.ok, false);
    assert.match(check.error, /Dokumentation/);
    assert.match(check.error, /förbättra/);
  });
