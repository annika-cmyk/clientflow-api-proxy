...
describe('ai-ovrig-riskfaktor-quality', () => {
  it('har hårda prioriteringar för extra underlag, åtgärd och TF', () => {
    ...
    assert.match(OVRIG_HARD_PRIORITY_RULES, /upptäcktskedja|upptäckts-/);
    assert.match(OVRIG_COMPLIANCE_FRAME, /PVML/);
    assert.match(OVRIG_COMPLIANCE_FRAME, /compliance-dokumentation/i);
  });
  ...
  it('kopplas in i ai-ovriga-riskfaktor-endpointen med retrieval och repair', () => {
    ...
    assert.match(chunk, /looksLikeModelRefusal/);
    assert.match(chunk, /repairFallbackUsed/);
    assert.match(chunk, /refusalRetryUsed/);
    assert.match(chunk, /OVRIG_COMPLIANCE_FRAME/);
    assert.match(chunk, /cf-ovrig-riskfaktor-soft/);
