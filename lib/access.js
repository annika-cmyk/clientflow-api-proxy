...
function normalizeByraId(raw) {
  if (raw == null || raw === '') return '';
  if (Array.isArray(raw)) return normalizeByraId(raw[0]);
  if (typeof raw === 'object') {
    return normalizeByraId(raw.name || raw.Name || raw.id || '');
  }
...
function customerByraId(fields) {
  const f = fields || {};
  return normalizeByraId(f['Byrå ID'] ?? f['Byra_ID'] ?? f['ByraID'] ?? f.Byrå ?? f.Byra ?? '');
}
...
function sameByra(userData, fields) {
  const userByra = normalizeByraId(userData?.byraId);
  const custByra = customerByraId(fields);
  return !!(userByra && custByra && userByra === custByra);
}

/** Admin eller samma Byrå ID — inte samma som personlig kundbehörighet (Användare). */
function userSharesAgency(userData, customerRecord) {
  if (!userData) return false;
  if (isClientFlowAdmin(userData.role)) return true;
  const fields = customerRecord?.fields || customerRecord || {};
  return sameByra(userData, fields);
}
...
/** CFA är attribut, inte roll — läs från user.isCfa / user.cfaConfirmedAt. */
function isCfa(user) {
  ...
  return s === 'true' || s === '1' || s === 'ja' || s === 'yes' || s === 'checked';
}

function isCfaConfirmed(user) {
  return isCfa(user) && !!String(user?.cfaConfirmedAt || '').trim();
}
