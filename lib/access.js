...
function anvandareFieldValues(fields) {
  const f = fields || {};
  return [f['Användare'], f.Anvandare, f['Behöriga'], f.Behoriga];
}

function userListedOnCustomer(userData, fields) {
  const tokens = [userData?.id, userData?.name, userData?.fullName, userData?.email]
    .map((v) => String(v || '').trim().toLowerCase())
    .filter(Boolean);
  if (!tokens.length) return false;
  const listed = [];
  anvandareFieldValues(fields).forEach((value) => {
    if (value == null || value === '') return;
    if (Array.isArray(value)) {
      value.forEach((v) => {
        const raw = String(v || '').trim();
        if (raw) listed.push(raw);
        parseAnvandareIds(v).forEach((id) => listed.push(id));
      });
      return;
    }
    ...
    parseAnvandareIds(raw).forEach((id) => listed.push(id));
  });
  const listedLower = listed.map((v) => v.toLowerCase()).filter(Boolean);
  return tokens.some((token) => listedLower.includes(token));
}
