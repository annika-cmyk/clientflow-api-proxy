...
describe('ai-falt-granskning', () => {
  ...
  it('markerar tillägg och borttag i befintlig text', () => {
    const html = Ai.wordDiffHtml('Byrån hjälper kunder med ROT.', 'ROT-hantering hjälper kunder med avdrag.');
    assert.match(html, /<del>/);
    assert.match(html, /<ins>/);
    assert.match(html, /kunder/);
  });

  it('markerar vad som ändras i en befintlig hotrad', () => {
    ...
    assert.match(Ai.wordDiffHtml(current[0].beskrivning, forslag[0].beskrivning), /<ins>/);
    ...
    assert.match(host.innerHTML, /data-ai-item-diff/);
    assert.match(host.innerHTML, /<ins>/);
  ...
  it('lägger AI-kort på rätt flik i redigeringsvyn', () => {
    ...
    assert.match(hosts.oversikt.innerHTML, /Ändring i texten/);
