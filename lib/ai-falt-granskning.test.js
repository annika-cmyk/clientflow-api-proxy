...
describe('ai-falt-granskning', () => {
  ...
  it('normaliserar motiveringsförslag i granskning', () => {
    ...
    assert.equal(Ai.tabForFalt('motiveringInneboende', 'tjanst'), 'oversikt');
  ...
  it('sammanfattar liständringar som korta punkter', () => {
    ...
    assert.deepEqual(points, ['Lägger till «Medel till terrorgrupp»: AI vill lägga till «Medel till terrorgrupp» (TF) eftersom hotet saknas i er lista men är relevant för tjänsten enligt analysen.']);
  });
  ...
  it('normaliserar per-rad andringar och ger fallback-motivering', () => {
    ...
    assert.match(
      ...
      /justera typ \(PT → Båda\), beskrivningen/
    );
  });
