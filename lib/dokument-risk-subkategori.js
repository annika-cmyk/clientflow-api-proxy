...
const kycUppfoljning = require('./kyc-uppfoljning');
...
function inferSubcategoryFromAttachment(item, meta) {
  ...
  if (fn.includes('aml-kollen') || fn.includes('aml_kollen')) {
    return 'aml_kollen';
  }
...
function archiveYearsForSubcategory(subcategory, context) {
  const sub = normalizeSubcategory(subcategory);
  if (sub === 'kyc') {
    return kycUppfoljning.kycArchiveYears(context?.customerFields || {});
  }
  return ARCHIVE_YEARS[sub];
}

function shouldArchiveByAge(subcategory, createdDate, today, context) {
  const years = archiveYearsForSubcategory(subcategory, context);
...
function shouldArchiveKycByTrigger(context) {
  if (!context?.customerFields) return false;
  const raw = context.customerFields[kycUppfoljning.UPPFOLJNING_FIELD];
  return kycUppfoljning.hasPendingKycTriggers(raw);
}
...
function applyAgeBasedArchive(kategorier, today, context) {
  const day = dokumentKategori.toDateOnly(today) || new Date().toISOString().slice(0, 10);
  const list = Array.isArray(kategorier) ? kategorier.map((k) => (k && typeof k === 'object' ? { ...k } : k)) : [];
  const archiveKycByTrigger = shouldArchiveKycByTrigger(context);
  return list.map((entry) => {
    if (!entry || typeof entry !== 'object' || !isAktuellRiskEntry(entry)) return entry;
    ...
    if (sub === 'kyc' && archiveKycByTrigger) {
      return archiveEntry({ ...entry, subcategory: sub });
    }
    if (!shouldArchiveByAge(sub, createdDate, day, context)) return entry;
    return archiveEntry({ ...entry, subcategory: sub });
  });
}
...
function buildMetadataUpdatesForItem(item, meta, today, context) {
  ...
  const archiveKycByTrigger = subcategory === 'kyc' && shouldArchiveKycByTrigger(context);
  if (category === AKTUELL_RISK_CATEGORY && (archiveKycByTrigger || shouldArchiveByAge(subcategory, createdDate, today, context))) {
...
function syncRiskMetadata(kategorier, attachments, today, context) {
  const list = Array.isArray(kategorier) ? kategorier.map((k) => (k && typeof k === 'object' ? { ...k } : k)) : [];
  let next = list;
  const day = dokumentKategori.toDateOnly(today) || new Date().toISOString().slice(0, 10);
  ...
  (attachments || []).forEach((item) => {
    ...
    const updates = buildMetadataUpdatesForItem(item, meta, day, context);
    if (!updates) return;
    next = dokumentKategori.upsertDokumentKategori(next, item, updates);
  });

  next = applyAgeBasedArchive(next, day, context);
  return { nextKategorier: next, changed: kategorierChanged(list, next) };
}

function archiveAktuellKyc(kategorier) {
  const list = Array.isArray(kategorier) ? kategorier.map((k) => (k && typeof k === 'object' ? { ...k } : k)) : [];
  return list.map((entry) => {
    if (!entry || typeof entry !== 'object' || !isAktuellRiskEntry(entry)) return entry;
    const sub = normalizeSubcategory(entry.subcategory) || normalizeSubcategory(inferSubcategoryFromAttachment({ filename: entry.filename }, entry));
    if (sub !== 'kyc') return entry;
    return archiveEntry({ ...entry, subcategory: 'kyc' });
  });
}
...
module.exports = {
  ...
  resolveRiskCategory,
  shouldArchiveByAge,
  archiveYearsForSubcategory,
  shouldArchiveKycByTrigger,
  archiveAktuellKyc,
  archivePriorOnNew,
