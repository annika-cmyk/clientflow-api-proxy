...
test('normalizeAirtableContentType strippar charset (Airtable Content API 403 annars)', () => {
  assert.equal(
    archiveSave.normalizeAirtableContentType('text/html; charset=utf-8', 'mail.html'),
    'text/html'
  );
  assert.equal(
    archiveSave.normalizeAirtableContentType('application/pdf; name=x.pdf', 'x.pdf'),
    'application/pdf'
  );
  assert.equal(archiveSave.normalizeAirtableContentType('text/html', 'mail.html'), 'text/html');
  assert.equal(archiveSave.normalizeAirtableContentType('', 'mail.html'), 'text/html');
  assert.equal(archiveSave.normalizeAirtableContentType(null, 'doc.txt'), 'text/plain');
});
...
test('saveBufferToTarget(korning) normaliserar text/html; charset innan upload', async () => {
  archiveSave.clearCachesForTests();
  const uploaded = [];
  const originalLoad = Module._load;
  Module._load = function (request, parent, isMain) {
    if (request === 'axios') {
      return {
        get: async (url) => {
          ...
          if (String(url).includes('/tblRuns/recRunHtml')) {
            return {
              data: {
                id: 'recRunHtml',
                fields: {
                  'Uppdrag ID': 'recUppHtml',
                  'Kund ID': 'recCust1',
                  Deadline: '2028-06-15',
                  Typ: 'Bokslut',
                  'Period Label': '2028'
        ...
        post: async (url, body) => {
          uploaded.push({ url, filename: body.filename, contentType: body.contentType });
          return { data: { id: 'attH', url: 'https://example.test/' + body.filename } };
        }
      };
    }
    return originalLoad(request, parent, isMain);
  ...
  try {
    ...
    const fresh = require('./archive-save');
    process.env.AIRTABLE_ACCESS_TOKEN = process.env.AIRTABLE_ACCESS_TOKEN || 'patTEST';
    await fresh.saveBufferToTarget(
      Buffer.from('<html><body>mejl</body></html>'),
      'snapshot.html',
      'text/html; charset=utf-8',
      { type: 'korning', runId: 'recRunHtml', customerId: 'recCust1' }
    );
    assert.ok(uploaded.length >= 1);
    for (const u of uploaded) {
      assert.equal(u.contentType, 'text/html', u.contentType);
      assert.ok(!String(u.contentType).includes('charset'));
    }
  } finally {
    Module._load = originalLoad;
    delete require.cache[require.resolve('./archive-save')];
