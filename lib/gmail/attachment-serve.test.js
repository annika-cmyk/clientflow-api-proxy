...
test('resolveAttachmentMeta: id först, annars unik filnamnsmatch', () => {
  const { resolveAttachmentMeta } = require('./attachment-serve');
  const atts = [
    { filename: 'Faktura 827051.pdf', attachmentId: 'fresh-id', mimeType: 'application/pdf' },
    { filename: 'other.pdf', attachmentId: 'att2', mimeType: 'application/pdf' }
  ];
  assert.equal(resolveAttachmentMeta(atts, 'fresh-id', '').attachmentId, 'fresh-id');
  assert.equal(
    resolveAttachmentMeta(atts, 'truncated-or-stale', 'Faktura 827051.pdf').attachmentId,
    'fresh-id'
  );
  assert.equal(resolveAttachmentMeta(atts, 'missing', 'saknas.pdf'), null);
  const dupes = [
    { filename: 'same.pdf', attachmentId: 'a1' },
    { filename: 'same.pdf', attachmentId: 'a2' }
  ];
  assert.equal(resolveAttachmentMeta(dupes, 'bad', 'same.pdf'), null);
});
