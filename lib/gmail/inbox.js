...
/** Gmail q som matchar rot + underetiketter (OR), med valfri mapp och datumfönster. */
function buildKunderSearchQuery(labels, kunderRoot = 'KUNDER', folder = 'inbox', opts = {}) {
  ...
  const f = normalizeFolder(folder);
  const base =
    f === 'sent' ? `${labelPart} in:sent -in:trash` : `${labelPart} -in:sent -in:trash`;
  if (opts.includeDateWindow === false) return base;
  const days =
    opts.windowDays != null ? opts.windowDays : syncCache.SYNC_WINDOW_DAYS;
  return syncCache.appendDateWindowToQuery(base, days);
}
