...
test('listSharedWithUser filtrerar på Shared With och returnerar [] vid saknad tabell', async (t) => {
  t.mock.method(axios, 'get', async () => {
    throw airtableErr(404, 'NOT_FOUND', 'Could not find table');
  });
  assert.deepEqual(await archiveStore.listSharedWithUser('recJohanna'), []);
});

test('listSharedWithUser hämtar och filtrerar delade poster', async (t) => {
  let sawFormula = '';
  t.mock.method(axios, 'get', async (_url, opts) => {
    sawFormula = opts && opts.params && opts.params.filterByFormula;
    return {
      data: {
        records: [
          {
            ...
            fields: {
              ...
              Visibility: 'privat',
              'Shared With': JSON.stringify(['recJohanna', 'recOther']),
              'Owner User ID': 'recOwner',
              Subject: 'Delat mejl',
          ...
          },
          {
            ...
            fields: {
              'Kund ID': 'recCust2',
              'Gmail Message ID': 'msg2',
              Visibility: 'byra',
              // FIND kan träffa substring – ska filtreras bort client-side
              'Shared With': JSON.stringify(['recJohannaXX']),
              'Owner User ID': 'recOwner',
              Subject: 'Fel träff'
            }
          }
  ...
  const rows = await archiveStore.listSharedWithUser('recJohanna');
  assert.ok(String(sawFormula).includes('recJohanna'));
  assert.equal(rows.length, 1);
  assert.equal(rows[0].id, 'recA1');
  assert.equal(rows[0].subject, 'Delat mejl');
  assert.deepEqual(rows[0].sharedWith, ['recJohanna', 'recOther']);
});
