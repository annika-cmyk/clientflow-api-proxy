...
function registerArchiveRoutes(app, ctx) {
      ...
      for (const target of targets) {
        ...
        if (targetIncludeEmail) {
          try {
            ...
            // Airtable Content API rejects MIME params (charset) — normalize in archive-save.
            const saved = await archiveSave.saveBufferToTarget(Buffer.from(snap, 'utf8'), fname, 'text/html', t);
  ...
  app.post('/api/setup/airtable-gmail-mejlcache', authenticateToken, async (req, res) => {
    try {
      const user = await requireUser(req, res);
      if (!user) return;
      if (!access.isClientFlowAdmin(user.role) && !access.isLedare(user.role)) {
      ...
      const result = await messageCache.ensureMejlcacheTable();
      if (!result.ok) return res.status(503).json({ success: false, error: result.error });
      res.json({
        success: true,
        message: result.created
          ? 'Tabellen Gmail Mejlcache skapades.'
          : 'Tabellen Gmail Mejlcache finns redan.',
        ...result
      });
    } catch (err) {
      console.error('setup gmail mejlcache:', err.message);
      res.status(archiveErrorStatus(err)).json({ success: false, error: archiveErrorMessage(err) });
    }
  });
}
