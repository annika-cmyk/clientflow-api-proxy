...
/**
 * Google APIs förväntar upprepade nycklar (labelIds=a&labelIds=b), inte axios
 * default labelIds[]=a. Bracket-form ger 400 från Gmail.
 */
function serializeGmailParams(params) {
...
function gmailGet(url, accessToken, { params, timeout } = {}) {
  const opts = {
  ...
  if (params) {
    opts.params = params;
    opts.paramsSerializer = serializeGmailParams;
  }
  return axios.get(url, opts);
}

async function getProfile(accessToken) {
  const res = await gmailGet(`${GMAIL_BASE}/profile`, accessToken, { timeout: 15000 });
  return res.data;
}
...
async function listHistory(accessToken, { startHistoryId, historyTypes, labelId, maxResults = 100 } = {}) {
  ...
  do {
    ...
    const res = await gmailGet(`${GMAIL_BASE}/history`, accessToken, {
      params,
      timeout: 30000
    });
...
async function listLabels(accessToken) {
  const res = await gmailGet(`${GMAIL_BASE}/labels`, accessToken, { timeout: 20000 });
...
async function listMessages(accessToken, { q, labelIds, maxResults = 50, pageToken } = {}) {
  ...
  if (pageToken) params.pageToken = pageToken;
  const res = await gmailGet(`${GMAIL_BASE}/messages`, accessToken, {
    params,
    timeout: 25000
  });
  return {
...
function filenameFromPartHeaders(part) {
  ...
  for (const h of headers) {
    ...
    if (name === 'content-disposition') {
      const star = value.match(/filename\*\s*=\s*(?:UTF-8''|utf-8'')([^;\s]+)/i);
      if (star && star[1]) {
        try {
      ...
      const plain = value.match(/filename\s*=\s*"([^"]+)"/i) || value.match(/filename\s*=\s*([^;\s]+)/i);
      if (plain && plain[1]) return plain[1].replace(/^["']|["']$/g, '').trim();
    }
    if (name === 'content-type') {
      const named = value.match(/name\s*=\s*"([^"]+)"/i) || value.match(/name\s*=\s*([^;\s]+)/i);
...
function collectAttachments(part, acc = []) {
  if (!part) return acc;
  let filename = part.filename ? String(part.filename).trim() : '';
  if (!filename) filename = filenameFromPartHeaders(part);
...
async function sendMessage(accessToken, { from, to, subject, text, html, inReplyTo, references, threadId, labelIds, inlineImages }) {
  const raw = buildRawMime({ from, to, subject, text, html, inReplyTo, references, threadId, inlineImages });
  const body = { raw };
