...
test('scoreEmailMatch: excludeEmails ignorerar byråns egen adress', () => {
  const customer = {
    id: 'c1',
    namn: 'TRUE HORSES AB',
  ...
  assert.equal(
    scoreEmailMatch(['annika@rydenredovisning.se'], customer, {
      excludeEmails: ['annika@rydenredovisning.se']
    }),
    0
  );
  assert.equal(
    scoreEmailMatch(['info@truehorses.se'], customer, {
      excludeEmails: ['annika@rydenredovisning.se']
    }),
    THRESHOLDS.EMAIL_EXACT
  );
});
