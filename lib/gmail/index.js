...
const DATE_WINDOW_Q = () => syncCache.gmailNewerThanQuery();
...
function createGmailIntegration(deps) {
  ...
  async function getValidAccessToken(user) {
    ...
    if (!refreshed || !refreshed.access_token) {
      const e = new Error(
        'Kunde inte förnya Gmail-åtkomsten. Klicka Koppla Gmail och ge åtkomst igen.'
      );
  ...
  async function pullGoogleEventsForUser(user, { timeMin, timeMax } = {}) {
    // Pull när Gmail+kalender-scope finns — oberoende av syncEnabled (push-preferens).
    // Utan UI-toggle i kalendern ska händelser synas så fort kopplingen finns.
    const prefs = await calendarSync.loadPrefs(user.id);
    let accessToken;
    let tokens;
    try {
    ...
    const [raw, colors, primaryCal] = await Promise.all([
      calendarApi.listEvents(accessToken, { timeMin, timeMax }),
      calendarApi.getColors(accessToken),
      calendarApi.getPrimaryCalendarMeta(accessToken)
    ]);
    const colorCtx = calendarSync.buildColorContext(colors, primaryCal);
    const events = calendarSync.filterAndMapPulledEvents(raw, prefs, colorCtx);
    return {
      events,
  ...
  function registerRoutes(app) {
    app.get('/api/gmail/status', authenticateToken, async (req, res) => {
      try {
        ...
        let reconnectHint = null;
        if (configured) {
          const state = await store.loadTokenState(user.id);
          tokenState = state.reason;
          connected = !!(state.tokens && state.tokens.refreshToken);
          email = (state.tokens && state.tokens.email) || '';
          hasCalendarScope = oauth.tokenHasCalendarScope(state.tokens);
          if (connected) {
            const prefs = await calendarSync.loadPrefs(user.id);
            calendarSyncEnabled = !!prefs.enabled;
          } else if (state.reason === 'decrypt_failed') {
            reconnectHint =
              'Sparad Gmail-koppling kunde inte läsas (t.ex. efter byte av krypteringsnyckel). Klicka Koppla Gmail igen.';
          }
        }
        ...
        res.json({
          ...
          hasCalendarScope,
          calendarSyncEnabled,
          /** missing | ok | decrypt_failed – hjälper skilja aldrig kopplad vs oläsbar blob. */
          tokenState,
          reconnectHint,
          missingEnv,
    ...
    function buildInboxResponsePayload({
      ...
      const filteredMatches =
        customerIdFilter && customerIdFilter !== inbox.UNMATCHED_FILTER
      ...
      if (!messages.length) {
        const unmatchedPreview = (unmatchedLabels || [])
          ...
          .filter(Boolean);
        let note =
          customerIdFilter === inbox.UNMATCHED_FILTER
            ? 'Inga mejl under omatchade KUNDER-etiketter just nu.'
            : customerIdFilter
              ? 'Ingen Gmail-etikett eller mejladress matchade den kunden under KUNDER.'
              : folder === 'sent'
                ? 'Inga skickade kundmejl under KUNDER ännu.'
                : 'Inga mejl under etiketten KUNDER ännu.';
        if (
          customerIdFilter &&
          customerIdFilter !== inbox.UNMATCHED_FILTER &&
        ...
        } else if (!customerIdFilter && unmatchedPreview.length && !(detailed || []).length) {
          note =
            folder === 'sent'
              ? 'Inga skickade mejl hittades under etiketten KUNDER.'
              : 'Inga mejl hittades under etiketten KUNDER.';
    ...
    /** Full hämtning under KUNDER + spara cache med historyId (bara ≤ 30 dagar). */
    async function fullSyncKunderMessages(accessToken, labels, folder, limit, opts = {}) {
      const ownerUserId = opts.ownerUserId || null;
      const kunderRoot = KUNDER_ROOT();
      const kunderLabels = inbox.collectKunderLabels(labels, kunderRoot);
      const childLabels = (kunderLabels || []).filter((l) =>
        match.isUnderKunder(l && l.name, kunderRoot)
      );
      const byId = new Map();
      const windowQ = DATE_WINDOW_Q();
      ...
      // Alltid sampla per underetikett så en aktiv kund inte dominerar cachen
      // (annars kan övriga KUNDER-mejl "försvinna" från listan).
      const perLabel = Math.min(
        10,
        Math.max(4, Math.ceil((limit || 40) / Math.max(childLabels.length, 1)))
      );
      const sampledIds = [];
      const sampledSeen = new Set();
      for (const lab of childLabels) {
        try {
          const listed = await api.listMessages(accessToken, {
            labelIds: [lab.id],
            q: windowQ,
      ...
      // Prioritera per-etikett-stickprov så omatchade/övriga kunder inte trängs bort.
      const prioritized = [];
      const seen = new Set();
      ...
      const candidateIds = prioritized.slice(0, fetchCap);
      const { detailed, toPersist } = await fetchMessageSummaries(accessToken, candidateIds, {
        ownerUserId
      });
      ...
      return { detailed, historyId, kunderLabels, toPersist };
    }

    /**
     * Hämta sammanfattningar. Använder metadata (ingen body) när Airtable redan
     * har body för samma internalDate — annars full + kö för persist.
     */
    async function fetchMessageSummaries(accessToken, ids, opts = {}) {
      const ownerUserId = opts.ownerUserId || null;
      const bodyIndex = ownerUserId
        ? await messageCache.indexBodiesForOwner(ownerUserId, ids)
        : new Map();
      const out = [];
      const toPersist = [];
      for (const id of ids) {
        try {
          const cached = bodyIndex.get(String(id));
          const useMeta = !!(cached && cached.hasBody);
          const raw = await api.getMessage(accessToken, id, useMeta ? 'metadata' : 'full');
          const summary = api.summarizeMessage(raw);
          if (!syncCache.isWithinSyncWindow(summary)) continue;

          let attachmentCount = Array.isArray(summary.attachments)
            ? summary.attachments.length
            : 0;
          if (useMeta && cached && cached.record) {
            attachmentCount = Array.isArray(cached.record.attachmentMeta)
              ? cached.record.attachmentMeta.length
              : syncCache.attachmentCountFromMsg(cached.record) || attachmentCount;
          }
          ...
          if (!useMeta && (summary.text || summary.html || (summary.attachments || []).length)) {
            toPersist.push(summary);
          }
        } catch (_) {
          /* hoppa över */
        }
      }
      return { detailed: out, toPersist };
    }

    async function persistFetchedBodies(ownerUserId, toPersist) {
      if (!ownerUserId || !(toPersist && toPersist.length)) return;
      try {
        await messageCache.upsertMessages(ownerUserId, toPersist);
      } catch (err) {
        console.warn('gmail mejlcache persist:', err.message);
      }
    }

    async function incrementalSyncFromHistory(accessToken, existingCache, labels, opts = {}) {
      const ownerUserId = opts.ownerUserId || null;
      const startHistoryId = existingCache && existingCache.historyId;
      if (!startHistoryId) {
        return { fullSyncNeeded: true, cache: existingCache, changed: false, toPersist: [] };
      }
      let historyResult;
      try {
      ...
      const fetched = toFetch.size
        ? await fetchMessageSummaries(accessToken, [...toFetch], { ownerUserId })
        : { detailed: [], toPersist: [] };
      const updatedMessages = fetched.detailed || [];
      ...
      const noChanges =
        ...
        String(nextHistoryId) === String(startHistoryId);
      ...
      return {
        ...
        toPersist: fetched.toPersist || []
      };
    }

    async function respondFromSyncCache(user, {
      folder,
      customerIdFilter,
      limit,
      customers,
      ...
      // Om list-cachen är tom: bygg från Gmail Mejlcache (body-tabellen).
      if (!messages.length) {
        try {
          const rows = await messageCache.listByOwner(user.id, { maxRecords: 100 });
          if (rows.length) {
            messages = rows
              .map((r) => {
                const summary = messageCache.recordToSummary(r);
                return syncCache.toCompactMessage({
                  ...summary,
            ...
            cached = {
              ...
              syncedAt: cached.syncedAt || rows[0].syncedAt || null,
              labels: cached.labels || [],
              messages
            };
          }
        } catch (err) {
          console.warn('gmail/inbox mejlcache list fallback:', err.message);
        }
      }

      const hasCache = (messages && messages.length) || cached.historyId;
      if (!hasCache) {
        return {
          success: true,
          folder,
      ...
      const labels =
        labelsOverride && labelsOverride.length
          ? labelsOverride
          : cached.labels && cached.labels.length
            ? cached.labels
            : [];
      const tokens = await store.loadTokens(user.id).catch(() => null);
      ...
      return buildInboxResponsePayload({
        folder,
        detailed: messages,
        labels,
        matches,
        ...
        syncMeta: {
          ...
          historyId: cached.historyId || null,
          windowDays: syncCache.SYNC_WINDOW_DAYS,
          ...syncMeta
        }
      });
    }
    ...
    app.get('/api/gmail/inbox', authenticateToken, async (req, res) => {
      try {
        ...
        if (mode === 'cache') {
          const payload = await respondFromSyncCache(user, {
            folder,
            customerIdFilter,
            limit,
            customers,
            syncMeta: { syncMode: 'cache' }
          });
          if (payload.messages && payload.messages.length) delete payload.note;
        ...
        try {
          ...
          const reconnect =
            needsGmailReconnect(tokenErr.code) || tokenErr.code === 'GMAIL_NOT_CONNECTED';
          if (reconnect) {
            const payload = await respondFromSyncCache(user, {
              folder,
              customerIdFilter,
              ...
              syncMeta: {
                syncMode: mode,
                syncState: 'cached_disconnected',
                gmailConnected: false,
                note:
                  'Gmail är inte kopplad just nu. Visar sparade mejl från den senaste månaden.'
              }
            });
        ...
        if (mode === 'full' || !cached.historyId) {
          const syncResult = await fullSyncKunderMessages(accessToken, labels, folder, limit, {
            ownerUserId: user.id
          });
          toPersist = syncResult.toPersist || [];
          cached = syncCache.buildCacheFromFullSync({
            historyId: syncResult.historyId,
            labels,
            messages: syncResult.detailed,
            syncedAt: new Date().toISOString()
          });
          ...
          try {
            ...
            if (inc.fullSyncNeeded) {
              const syncResult = await fullSyncKunderMessages(
                accessToken,
                labels,
                folder,
                limit,
        ...
        // Spara body i Airtable i bakgrunden (lista svarar snabbare)
        if (toPersist.length) {
          persistFetchedBodies(user.id, toPersist).catch(() => {});
        }
      ...
      try {
        ...
        async function enrichAndRespond(summary, labels, opts = {}) {
          ...
          const resolved =
            ...
            inbox.resolveUnmatchedLabelMeta(summary, {
              kunderLabelIds: kunderIds,
              labels,
              kunderRoot
            });
        ...
        // 1) Airtable-cache (Gmail Mejlcache) — undvik Gmail round-trip
        if (!forceRefresh) {
          try {
            const cachedRow = await messageCache.findByOwnerAndMessageId(user.id, messageId);
            if (cachedRow && messageCache.hasUsableBody(cachedRow)) {
              const summary = messageCache.recordToSummary(cachedRow);
              const syncCached = await store.loadSyncCache(user.id);
              const labels =
                syncCached.labels && syncCached.labels.length ? syncCached.labels : [];
              return enrichAndRespond(summary, labels, { fromCache: true });
            }
          } catch (cacheErr) {
            console.warn('gmail/messages cache read:', cacheErr.message);
          }
        }

        // 2) Gmail full fetch + persist
        let accessToken;
        try {
          ({ accessToken } = await getValidAccessToken(user));
        } catch (tokenErr) {
          // Fallback: Mejlarkiv (sparat/delat mejl)
          try {
            const archive = await archiveStore.findByGmailMessageId(messageId);
            if (archive && (archive.bodyHtml || archive.bodyText)) {
              const summary = {
                id: archive.gmailMessageId || messageId,
                threadId: archive.threadId || null,
                labelIds: [],
                internalDate: archive.date ? Date.parse(archive.date) || null : null,
                snippet: archive.snippet || '',
                subject: archive.subject || '(utan ämne)',
                from: archive.from || '',
                to: archive.to || '',
                cc: archive.cc || '',
                date: archive.date || '',
        ...
        const [raw, labels] = await Promise.all([
          api.getMessage(accessToken, messageId, 'full'),
          api.listLabels(accessToken)
        ]);
        const summary = api.summarizeMessage(raw);
        if (syncCache.isWithinSyncWindow(summary)) {
          persistFetchedBodies(user.id, [summary]).catch(() => {});
        }
        return enrichAndRespond(summary, labels, { fromCache: false });
      } catch (err) {
        const status = err.code === 'GMAIL_NOT_CONNECTED' ? 400 : 500;
        res.status(status).json({ success: false, error: err.message, code: err.code || null });
      }
    ...
    app.post('/api/gmail/send', authenticateToken, async (req, res) => {
      try {
        ...
        const inlineImages = Array.isArray(req.body.inlineImages) ? req.body.inlineImages : [];
        if (!to || !to.includes('@')) {
  ...
  return {
    ...
    ensureMejlcacheTable: messageCache.ensureMejlcacheTable,
