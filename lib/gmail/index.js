  ...
  function registerRoutes(app) {
      ...
      try {
        ...
        res.json({
          ...
          redirectHint: configured
            ? null
            : 'Sätt GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET och GOOGLE_GMAIL_REDIRECT_URI i miljövariablerna.'
        });
      } catch (err) {
      ...
      try {
        ...
        if (!oauth.isOAuthConfigured()) {
          return res.status(503).json({
            success: false,
            error: 'Gmail-integration är inte konfigurerad på servern ännu.'
          });
        }
    ...
    app.get('/api/gmail/inbox', authenticateToken, async (req, res) => {
      try {
        ...
        const labels = await api.listLabels(accessToken);
        const customers = await resolveCustomers(user);
        const { matches } = match.matchLabelsToCustomers(labels, customers, {
          kunderRoot: KUNDER_ROOT()
        });
        ...
        if (!activeMatches.length) {
          return res.json({
            ...
            note: customerIdFilter
              ? 'Ingen Gmail-etikett matchade den kunden under KUNDER.'
