  ...
  function registerRoutes(app) {
    ...
    app.get('/api/gmail/inbox', authenticateToken, async (req, res) => {
      try {
        ...
        // Andra pass: e-post hjälper etikett→kund bland redan KUNDER-etiketter (inte hela inkorgen).
        ...
        // Hämta endast mejl under KUNDER (+ underetiketter) – aldrig godtycklig inkorg via e-post.
        const byId = new Map();
        ...
        // Fallback / komplettering: lista per KUNDER-etikett (OR via flera anrop).
        if (byId.size < limit) {
        ...
        if (!byId.size) {
          ...
          if (unmatchedPreview.length) {
            note =
              `Hittade etiketter under KUNDER men inga mejl ännu` +
              ` (t.ex. ${unmatchedPreview.join(', ')}).`;
          }
          return res.json({
            success: true,
            folder,
        ...
        // Hämta metadata för kandidater; sortering+limit sker efteråt (inte före).
        const fetchCap = Math.min(Math.max(byId.size, limit), 100);
    ...
    app.get('/api/gmail/messages/:id', authenticateToken, async (req, res) => {
      try {
        ...
        const summary = api.summarizeMessage(raw);
        const customers = await resolveCustomers(user);
        const matches = match.matchLabelsToCustomers(labels, customers, { kunderRoot }).matches;
        const kunderIds = inbox.kunderLabelIdSet(labels, kunderRoot);
