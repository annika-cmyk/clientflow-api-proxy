...
function createGmailIntegration(deps) {
  ...
  function registerRoutes(app) {
     ...
     * Auth via session/JWT – Gmail-token exponeras aldrig till klienten.
     */
    app.get(
        ...
        try {
          ...
          if (!messageId || !attachmentId) {
            return res.status(400).json({
              success: false,
              error: 'Meddelande-id och attachmentId krävs'
            });
          }
          ...
          const summary = api.summarizeMessage(raw);
          const meta = attachmentServe.findAttachmentMeta(summary.attachments, attachmentId);
          if (!meta) {
          ...
          const buf = await api.getAttachment(accessToken, messageId, attachmentId);
          if (!buf || !buf.length) {
          ...
          const filename = attachmentServe.safeFilename(meta.filename || 'bilaga');
          const mime = attachmentServe.guessMimeType(filename, meta.mimeType);
