...
describe('extractCustomerEmails / mapCustomerRecord', () => {
  it('mappar Airtable-record-id och företags-e-post', () => {
    ...
    assert.deepEqual(c.emails, ['info@example.com']);
    assert.equal(c.orgnr, '556677-8899');
  });

  it('samlar kontaktpersoners e-post (case-insensitive, unika)', () => {
    const emails = extractCustomerEmails({
      'e-post': 'Info@TrueHorses.se',
      Kontaktpersoner: JSON.stringify([
        { namn: 'Anna', epost: 'anna@truehorses.se' },
        { namn: 'Bertil', email: 'INFO@TRUEHORSES.SE' },
        { namn: 'Cesar', epost: 'cesar@other.se' }
      ])
    });
    assert.deepEqual(emails, ['info@truehorses.se', 'anna@truehorses.se', 'cesar@other.se']);
  });
