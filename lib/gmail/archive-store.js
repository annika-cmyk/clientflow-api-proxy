 ...
 * Axios often surfaces that as "Request failed with status code 403".
 */
...
function formatArchiveStoreError(err) {
  ...
  if (err.code === 'MEJLARKIV_SETUP' || isMissingMejlarkivError(err)) {
    const detail = err.code === 'MEJLARKIV_SETUP' && err.message ? ` (${err.message})` : '';
    return (
      'Mejlarkiv-tabellen saknas eller är inte tillgänglig' +
      detail +
      '. Kör `node scripts/setup-mejlarkiv.js` eller POST /api/setup/airtable-mejlarkiv (Ledare/Admin).'
    );
  }
  const apiMsg =
    err.response &&
  ...
  if (/INVALID_PERMISSIONS_OR_MODEL_NOT_FOUND|Invalid permissions, or the requested model/i.test(msg)) {
    return (
      'Airtable hittade inte tabellen (Mejlarkiv). Kör `node scripts/setup-mejlarkiv.js` ' +
      'eller kontrollera AIRTABLE_TABLE_MEJLARKIV_ID.'
    );
  }
...
/**
 * Mejlarkiv-poster där Shared With innehåller userId (JSON-array i Airtable).
 * Extra client-side filter via isSharedWith så FIND-träffar inte blir falska positiva.
 */
async function listSharedWithUser(userId, { maxRecords = 50 } = {}) {
  const { token, baseId } = airtableConfig();
  const uid = String(userId || '').trim().replace(/"/g, '');
  if (!token || !uid) return [];
  try {
    const res = await axios.get(`https://api.airtable.com/v0/${baseId}/${tableRef()}`, {
      headers: { Authorization: `Bearer ${token}` },
      params: {
        filterByFormula: `FIND("${uid}",{Shared With})`,
        maxRecords: Math.min(Number(maxRecords) || 50, 100)
      },
      timeout: 20000
    });
    return (res.data.records || [])
      .map((r) => fieldsToRecord(r.id, r.fields))
      .filter((r) => archiveAccess.isSharedWith({ id: uid }, r));
  } catch (err) {
    if (isMissingMejlarkivError(err)) {
      clearCachedTableId();
      return [];
