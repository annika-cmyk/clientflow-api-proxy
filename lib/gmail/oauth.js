...
/** Kalender: skapa/uppdatera/radera händelser i användarens primära Google-kalender. */
...
function tokenHasCalendarScope(tokensOrScope) {
  const raw =
    typeof tokensOrScope === 'string'
      ? tokensOrScope
      : String((tokensOrScope && tokensOrScope.scope) || '');
  if (!raw.trim()) return false;
  return raw.split(/\s+/).includes(CALENDAR_EVENTS_SCOPE);
}
...
/**
 * Läs env, trimma whitespace och eventuella omslutande citattecken
 * (vanligt misstag vid inklistring i Render).
 */
 ...
 * Inga secret-värden returneras – bara length (0 = saknad/tom).
 */
function getOAuthEnvPresence() {
  ...
  const lengths = {
    GOOGLE_CLIENT_ID: cfg.clientId.length,
    GOOGLE_CLIENT_SECRET: cfg.clientSecret.length,
    GOOGLE_GMAIL_REDIRECT_URI: cfg.redirectUri.length
  };
  return {
    present,
    lengths,
