...
test('collectAttachments läser filename från Content-Disposition när part.filename saknas', () => {
  const { collectAttachments } = require('./api');
  const payload = {
    parts: [
      {
        ...
        filename: '',
        headers: [
          { name: 'Content-Disposition', value: 'attachment; filename="Faktura 827051.pdf"' }
        ],
        body: { attachmentId: 'ANGjdJ-rXyz', size: 192000 }
      },
      {
        mimeType: 'image/png',
        filename: '',
        headers: [{ name: 'Content-ID', value: '<image001.jpg@01>' }],
        body: { attachmentId: 'inline-img', size: 12 }
      }
    ]
  };
  const atts = collectAttachments(payload);
  assert.equal(atts.length, 1);
  assert.equal(atts[0].filename, 'Faktura 827051.pdf');
  assert.equal(atts[0].attachmentId, 'ANGjdJ-rXyz');
});
