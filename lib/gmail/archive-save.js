...
/**
 * Resolve Airtable table id by env override or table name (meta API).
 * Prefer name lookup when env id is stale/wrong.
 */
...
/**
 * Try Dokumentation / Attachments; ignore NOT_FOUND on a candidate and try the next.
 */
...
/**
 * Build the Airtable target descriptor used by saveBufferToTarget (testable mapping).
 */
...
async function saveBufferToTarget(buffer, filename, contentType, target) {
  ...
  if ((spec.type === 'uppdrag' || spec.type === 'korning') && !tableId) {
    const err = new Error(
  ...
  if (spec.type === 'uppdrag') {
    return { type: 'uppdrag', uppdragId: spec.recordId, attachment: att, filename: name, fieldName, tableId };
  }
  return { type: 'korning', runId: spec.recordId, attachment: att, filename: name, fieldName, tableId };
}
