...
describe('källadaptrar', () => {
  ...
  it('hämtar Skatteverket-nyheter med AML-filter', async () => {
    const html = `
      <a href="https://www.skatteverket.se/omoss/pressochmedia/nyheter/andringar-i-mekanismen.html">Ändringar i Mekanismen med anledning av sjätte penningtvättsdirektivet</a>
      <a href="https://www.skatteverket.se/omoss/pressochmedia/nyheter/sankt-skatt-pa-bensin.html">Tillfälligt sänkt skatt på bensin</a>
    `;
  ...
  it('tar bara Finanspolisens RSS-nyheter, inte rapporter och ämnessidor', async () => {
    ...
    const items = await getSource('finanspolisen').fetch({
      fetchText: async (url) => (String(url).includes('rss') ? rss : '')
    });
    assert.ok(items.some((i) => /stockholm/i.test(i.title)));
    assert.ok(!items.some((i) => /trafikolycka/i.test(i.title)));
    assert.ok(!items.some((i) => /omvärldsbevakning|sns\.se/i.test(`${i.title} ${i.source_url}`)));
  });
});
