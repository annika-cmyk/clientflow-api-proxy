...
describe('ingest (layer 1)', () => {
    ...
    const sources = [
      {
        id: 'ok',
        async fetch() {
          return [{ source: 'ok', source_url: 'https://ex/1', title: 'FATF grey list', published_at: '2026-09-01T12:00:00.000Z', raw_content: 'x' }];
        }
      },
      {
        id: 'broken',
  ...
  it('hämtar artikeltext när listningen bara har titeln', async () => {
    const out = await ingestSources({
      ...
      fetchText: async (url) => {
        if (!String(url).includes('aterkomma')) return '';
        return `
