...
function createAirtableStore({ axios, token, baseId, tableName = NEWS_TABLE }) {
  ...
  return {
    ...
    async saveClassification(id, classification) {
      // Skriv BARA klassningsfält — toFields() skulle nolla title/source/raw_content.
      const fields = {
        category: classification.category || '',
        severity: classification.severity || '',
        summary_sv: String(classification.summary_sv || '').slice(0, 4000),
        affected_industries: JSON.stringify(classification.affected_industries || []),
        affected_geography: JSON.stringify(classification.affected_geography || []),
