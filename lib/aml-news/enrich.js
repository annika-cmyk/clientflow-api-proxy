const { extractArticleLead, extractTrustedPublishedAt, isIndexOrSectionUrl, isRealPublishedAt } = require('./sources/html');
...
function isArticleUrl(url) {
  ...
  if (/rss=true|\.xml(\?|$)/.test(u)) return false;
  if (isIndexOrSectionUrl(url)) return false;
  return true;
}
  ...
  return mapPool(rows, concurrency, async (item) => {
    ...
    try {
      ...
      const published_at = needsDate
        ? (extractTrustedPublishedAt(html, item.source_url) || item.published_at || '')
        : item.published_at;
