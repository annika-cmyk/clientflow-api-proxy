...
const { isRealPublishedAt } = require('./sources/html');
const { isRecentNews } = require('./feed');
...
async function ingestSources(opts = {}) {
  ...
  const now = opts.now || fetchedAt;
  const items = enriched.filter((item) => isRecentNews(item, now));
  return {
    items,
    errors,
    fetchedAt
...
function mergeLongerContent(prev, next) {
  ...
  if (nextRaw.length > prevRaw.length) {
    out = { ...out, raw_content: nextRaw, fetched_at: next.fetched_at || prev.fetched_at };
  }
  if (!isRealPublishedAt(out.published_at) && isRealPublishedAt(next.published_at)) {
    out = { ...out, published_at: next.published_at };
  }
  return out;
}
