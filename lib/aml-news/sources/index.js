...
const AML_HINT = /penningtv[aä]tt|aml|kyc|ptl|finansiering av terrorism|terroristfinans|högrisk|hogrisk|fatf|kundk[aä]nnedom|misstanke|rapportering|redovisningskonsult|ekonomisk brott|organiserad brott|svartarbete|momsbedr[aä]geri|verklig huvudman|bokf[oö]ring/i;
const RI_CORE = /penningtv[aä]tt|penningtvatts|\baml\b|\bptl\b|\bkyc\b|finansiering av terrorism|terroristfinans|kundk[aä]nnedom|ekonomisk brott|organiserad brott|bokf[oö]ringsbrott|svartarbete|momsbedr[aä]geri|verklig huvudman|redovisningskonsult|redovisningsbyr/i;
const RI_EXCLUDE = /återkomma till yrket|bli[- /]revisor|revisorsexamen|hållbarhetsrapport|obligatoriska ämnesområden|förnyad bedömning|auktorisation|godkännande som revisor/i;

function isRelevantRevisorsinspektionen(item) {
  if (!item) return false;
  const url = String(item.source_url || '');
  const hay = `${item.title || ''} ${url} ${item.raw_content || ''} ${item.summary_sv || ''} ${item.summary || ''}`;
  if (url && !/\/publikationer\/nyheter\//i.test(url)) return false;
  if (RI_EXCLUDE.test(hay) && !RI_CORE.test(hay)) return false;
  return RI_CORE.test(hay);
}

function isRelevantForConsultants(item) {
  if ((item && item.source) === 'revisorsinspektionen') return isRelevantRevisorsinspektionen(item);
  return true;
}
...
function toSourceItems(id, links) {
...
function dedupeByUrl(items) {
  ...
  for (const item of items || []) {
    const key = String(item.source_url || '').toLowerCase().replace(/\/$/, '').split('#')[0];
...
async function fetchPolisenReportPages(sourceId, pages, fetchText) {
  const items = [];
  for (const page of pages) {
    try {
      const html = await fetchText(page);
      items.push(...toSourceItems(sourceId, extractPolisenAmlReports(html, page)));
    } catch (_) {
      /* isolated per page */
    }
  }
...
const SOURCES = [
  ...
  ], { hrefIncludes: ['lansstyrelsen.se'] }),
  {
    ...
    async fetch({ fetchText }) {
      ...
      const reports = await fetchPolisenReportPages('finanspolisen', [
        POLISEN_AML_REPORTS_URL
      ], fetchText);
      return dedupeByUrl([...filtered, ...reports]);
    }
  },
  ...
  ], { hrefIncludes: ['regeringen.se'], requireAmlHint: false }),
  {
    ...
    async fetch({ fetchText }) {
      const rows = await listingAdapter('revisorsinspektionen', [
        'https://www.revisorsinspektionen.se/publikationer/nyheter/'
      ], {
        hrefIncludes: ['revisorsinspektionen.se/publikationer/nyheter/'],
        titleOrHrefRe: RI_CORE,
        requireAmlHint: false,
        limit: 20
      }).fetch({ fetchText });
      return rows.filter(isRelevantRevisorsinspektionen);
    }
  },
  listingAdapter('srf', [
...
module.exports = {
  ...
  SKV_HINT,
  RI_CORE,
  isRelevantRevisorsinspektionen,
  isRelevantForConsultants
};
