...
const NEWS_ARTICLE_PATH = /\/(nyheter|aktuellt|press(?:meddelande|rum)?s?|news|press-?releases?|publikationer\/nyheter)\b/i;
const RESEARCH_HOST = /(?:^|\.)sns\.se$|(?:^|\.)bra\.se$|(?:^|\.)riksdagen\.se$/i;

function isTopicPageTitle(title) {
  const t = String(title || '').replace(/\s+/g, ' ').trim();
  if (!t) return true;
  const words = t.split(/\s+/);
  return words.length === 1 && t.length <= 48 && !/[.!?:]/.test(t);
}

function hasFullDateInUrl(url) {
  return /\/(20\d{2})[/_-](\d{1,2})[/_-](\d{1,2})(?:[/_-]|$)/.test(String(url || ''));
}

function isNewsArticleUrl(url) {
  const raw = String(url || '').trim();
  if (!raw || isIndexOrSectionUrl(raw)) return false;
  let host = '';
  let path = '';
  try {
  ...
  if (RESEARCH_HOST.test(host)) return false;
  if (/\.pdf(\?|#|$)/i.test(raw)) return false;
  if (/\/siteassets\/|\/contentassets\//i.test(path)) return false;
  if (hasFullDateInUrl(raw)) return true;
  return NEWS_ARTICLE_PATH.test(path);
}
...
function extractLinks(html, baseUrl, opts = {}) {
  ...
  while ((m = re.exec(String(html || '')))) {
    ...
    if (opts.articleUrlsOnly !== false) {
      if (isIndexOrSectionUrl(href) || isGenericNewsTitle(title) || isTopicPageTitle(title)) continue;
      if (!isNewsArticleUrl(href)) continue;
    }
    seen.add(href);
...
function extractPolisenAmlReports(html, baseUrl) {
  const links = extractLinks(html, baseUrl, { minTitleLength: 8, articleUrlsOnly: false });
  const seen = new Set();
  const out = [];
