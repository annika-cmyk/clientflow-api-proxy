...
function extractPublishedAt(html, url, title) {
  ...
  while ((m = metaRe.exec(raw))) {
    ...
    if (!/^(article:published_time|og:published_time|date|dc\.date|dc\.date\.issued|pubdate|publishdate|publication_date|parsely-pub-date)$/.test(name)) continue;
  ...
  while ((m = timeRe.exec(raw))) {
    const parsed = toIsoDate(m[1]) || extractPublishedAtFromText(m[1]);
    if (parsed) return parsed;
  }
...
function extractLinks(html, baseUrl, opts = {}) {
  ...
  while ((m = re.exec(String(html || '')))) {
    ...
    const published_at = extractPublishedAt(around, href, title);
