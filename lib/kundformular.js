 ...
 * Byrån kan prefilla; kunden redigerar via publikt invite-länk (BankID senare).
...
const VERSION = 2;
/** Invite-länk giltig i 30 dagar (fas 2 embryo – utan BankID). */
...
function emptyForm() {
  return {
    ...
    lastRemindedAt: null,
    reminderCount: 0,
    /** Byråns VH-bekräftelse (kundresa steg 2) — separat från kundens svar i answers.vh_bekraftelse. */
    byraVhBekraftelse: '',
    byraVhNote: '',
    byraVhUpdatedAt: null,
    /** När byrån senast synkade svar → KYC-utkast. */
    lastSyncedToKycAt: null,
...
function inviteSecret() {
  return String(
    process.env.KUNDFORMULAR_INVITE_SECRET
      ...
      || 'clientflow-kundformular-invite'
  ).trim();
}
...
/**
 * HMAC-signerad invite: bäddar in customerId + expiry så publikt API
 * kan lösa upp kund utan Airtable-scan på tokenfält.
 */
...
function buildInviteUrl(token, baseUrl) {
  ...
  if (!t) return '';
  return `${publicAppBaseUrl(baseUrl)}/kundformular-svar.html?token=${encodeURIComponent(t)}`;
}

function inviteIsActive(form, token) {
  const f = parseForm(form);
  if (!f.inviteToken || !token) return false;
  if (trimStr(f.inviteToken) !== trimStr(token)) return false;
  if (f.inviteExpiresAt) {
    const exp = Date.parse(f.inviteExpiresAt);
    if (Number.isFinite(exp) && Date.now() > exp) return false;
  }
  if (f.status === 'besvarat' || f.status === 'signerat') return false;
  return f.status === 'skickat' || f.status === 'prefillad' || f.status === 'utkast';
}
...
function parseForm(raw) {
  ...
  return {
    ...
    inviteToken: trimStr(obj.inviteToken),
    inviteExpiresAt: obj.inviteExpiresAt || null,
    lastRemindedAt: obj.lastRemindedAt || null,
    reminderCount: Math.max(0, Number(obj.reminderCount) || 0),
    byraVhBekraftelse: normalizeVhBekraftelse(obj.byraVhBekraftelse),
    byraVhNote: trimStr(obj.byraVhNote),
    byraVhUpdatedAt: obj.byraVhUpdatedAt || null,
    lastSyncedToKycAt: obj.lastSyncedToKycAt || null,
    prefillMeta: {
...
function applySave(form, incomingAnswers, {
  actor = 'byra',
  action = 'save',
  customerId = '',
  ...
  const current = parseForm(form);
  const metaOnly = action === 'remind' || action === 'set_byra_vh' || action === 'mark_synced_to_kyc';
  const answers = metaOnly
    ? current.answers
    : normalizeAnswers({
      ...current.answers,
  ...
  let inviteToken = current.inviteToken;
  let inviteExpiresAt = current.inviteExpiresAt;
  let lastRemindedAt = current.lastRemindedAt;
  let reminderCount = current.reminderCount;
  let byraVhBekraftelse = current.byraVhBekraftelse;
  let byraVhNote = current.byraVhNote;
  let byraVhUpdatedAt = current.byraVhUpdatedAt;
  let lastSyncedToKycAt = current.lastSyncedToKycAt;

  if (action === 'mark_sent') {
    status = 'skickat';
    sentAt = now;
    const invite = createInviteToken(customerId || current.inviteCustomerId || '', {
      ttlMs: inviteTtlMs
    });
    if (invite) {
    ...
    // Endast stämpla påminnelse — status/invite oförändrad
    lastRemindedAt = now;
    reminderCount = (Number(current.reminderCount) || 0) + 1;
  } else if (action === 'set_byra_vh') {
    const src = incomingAnswers && typeof incomingAnswers === 'object' ? incomingAnswers : {};
    byraVhBekraftelse = normalizeVhBekraftelse(src.byraVhBekraftelse ?? src.vh_bekraftelse);
    if (Object.prototype.hasOwnProperty.call(src, 'byraVhNote') || Object.prototype.hasOwnProperty.call(src, 'note')) {
      byraVhNote = trimStr(src.byraVhNote ?? src.note);
    }
    byraVhUpdatedAt = now;
  } else if (action === 'mark_synced_to_kyc') {
    lastSyncedToKycAt = now;
...
/**
 * Fas 3: automatisk påminnelse när invite är aktiv och tillräckligt lång tid gått.
 */
function shouldAutoRemind(form, {
  now = Date.now(),
  minDays = 3,
  maxReminders = 5
} = {}) {
  const f = parseForm(form);
  if (!canRemind(f)) return false;
  if ((Number(f.reminderCount) || 0) >= maxReminders) return false;
  const anchor = f.lastRemindedAt || f.sentAt;
  if (!anchor) return false;
  const ts = Date.parse(anchor);
  if (!Number.isFinite(ts)) return false;
  const minMs = Math.max(0, Number(minDays) || 0) * 24 * 60 * 60 * 1000;
  return (now - ts) >= minMs;
}

/**
 * Brygga: overlappande fält från kundformulär → KYC-utkast (ersätter inte full merge).
 * Skriver aldrig över utskick/signering — anroparen ska merge:a via kyc-formular-merge.
 */
function buildKycSyncPatchFromKundformular(form) {
  const f = parseForm(form);
  const a = f.answers || emptyAnswers();
  const patch = {
    foretagsnamn: a.foretagsnamn || undefined,
    orgnr: a.orgnr || undefined,
    verksamhet: a.verksamhet || undefined,
    syfte_affarsrelation: a.syfte_affarsrelation || undefined,
    forvantad_omfattning: a.forvantad_omfattning || undefined,
    kapitalUrsprung: a.kapitalUrsprung || undefined,
    kapitalUrsprungSkarpt: a.kapitalUrsprungSkarpt || undefined,
    anstallda: a.anstallda || undefined,
    omsattning: a.omsattning || undefined,
    pep: a.pep || undefined,
    pepDetaljer: a.pepDetaljer || undefined,
    pepFamilj: a.pepFamilj || undefined,
    pepFamiljDetaljer: a.pepFamiljDetaljer || undefined,
    internationellHandel: a.internationellHandel || undefined,
    internationellaLander: a.internationellaLander || undefined,
    kontanter: a.kontanter || undefined,
    kontanterAndel: a.kontanterAndel || undefined,
    kryptovaluta: a.kryptovaluta || undefined,
    vh_bekraftelse: a.vh_bekraftelse || undefined,
    vh_agarstruktur: a.vh_agarstruktur || undefined,
    ombud_annan: a.ombud_annan || undefined
  };
  if (Array.isArray(a.foretradare) && a.foretradare.some((p) => p && (p.namn || p.personnr))) {
    patch.foretradare = a.foretradare;
  }
  if (Array.isArray(a.huvudman) && a.huvudman.some((p) => p && (p.namn || p.personnr))) {
    patch.huvudman = a.huvudman;
  }
  if (Array.isArray(a.ombud) && a.ombud.some((p) => p && (p.namn || p.personnr))) {
    patch.ombud = a.ombud;
  }
  if (Array.isArray(a.tjanster) && a.tjanster.length) {
    patch.tjanster = a.tjanster;
  }
...
/** Steg för fas-2-statusstrip (UI). signerat visas men är ännu inte live. */
function statusStripForUi(form) {
  const f = parseForm(form);
  const order = STATUSES;
  const idx = Math.max(0, order.indexOf(normalizeStatus(f.status)));
  return order.map((id, i) => {
    let state = 'upcoming';
    if (i < idx) state = 'done';
    else if (i === idx) state = 'current';
    // signerat: alltid "kommande" tills BankID finns — även om status råkar vara signerat i embryo
    if (id === 'signerat' && f.status !== 'signerat') state = 'upcoming';
    return {
      id,
      label: STATUS_LABELS[id] || id,
      state,
      comingSoon: id === 'signerat' && f.status !== 'signerat'
    };
  });
}

function canRemind(form) {
  const f = parseForm(form);
  if (f.status !== 'skickat') return false;
  if (!f.inviteToken) return false;
  if (f.inviteExpiresAt) {
    const exp = Date.parse(f.inviteExpiresAt);
    if (Number.isFinite(exp) && Date.now() > exp) return false;
  }
...
function summaryForUi(form, { baseUrl } = {}) {
  const f = parseForm(form);
  const inviteActive = !!(
    f.inviteToken
    && f.status === 'skickat'
    && (!f.inviteExpiresAt || Date.parse(f.inviteExpiresAt) > Date.now())
  );
  return {
    status: f.status,
    ...
    inviteUrl: inviteActive ? buildInviteUrl(f.inviteToken, baseUrl) : '',
    hasInvite: inviteActive,
    lastRemindedAt: f.lastRemindedAt,
    reminderCount: f.reminderCount || 0,
    canRemind: canRemind(f),
    canAutoRemind: shouldAutoRemind(f),
    byraVhBekraftelse: f.byraVhBekraftelse || '',
    byraVhNote: f.byraVhNote || '',
    byraVhUpdatedAt: f.byraVhUpdatedAt || null,
    lastSyncedToKycAt: f.lastSyncedToKycAt || null,
    statusStrip: statusStripForUi(f)
  };
}
...
/**
 * Publik kundvy: whitelist av formulärfält. Inga byrå-/sync-/påminnelse-interna.
 */
...
/**
 * Publik sammanfattning — bara statusmetadata kunden behöver.
 */
function publicSummaryForUi(form) {
  const f = parseForm(form);
  return {
    status: f.status,
    statusLabel: statusLabel(f.status),
...
/**
 * Publik UI-meta utan byråns riskbedömning.
 * riskFlags = [], hogrisksland = false, kyc = {} så vhKomplex/skarptKapital
 * inte läcker via vilka frågor som visas.
 */
function buildPublicUiMeta(form, {
  tjansterOptions = [],
  bolagsform = '',
  fields = {}
} = {}) {
  const meta = buildUiMeta(form, {
    tjansterOptions,
    riskFlags: [],
    kyc: {},
    fields,
    bolagsform: bolagsform || resolveBolagsform({ fields }),
    hogrisksland: false
  });
  return {
    ...meta,
...
/**
 * Fullt publikt API-svar (GET/PUT kundformulär via invite-token).
 */
function buildPublicResponse(form, { tjansterOptions = [], bolagsform = '', fields = {} } = {}) {
  return {
    success: true,
    form: sanitizePublicForm(form),
    summary: publicSummaryForUi(form),
    meta: buildPublicUiMeta(form, { tjansterOptions, bolagsform, fields })
  };
}
...
module.exports = {
  ...
  canRemind,
  shouldAutoRemind,
  normalizeVhBekraftelse,
  buildKycSyncPatchFromKundformular
};
