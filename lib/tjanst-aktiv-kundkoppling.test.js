...
describe('tjanst-aktiv-kundkoppling', () => {
  ...
  it('hittar radering av egen tjänst men inte standardmall', () => {
    let prev = TjanstUtforandeMallar.emptyState();
    ...
    assert.equal(removals[0].mallId, added.id);
    assert.equal(removals[0].namn, 'ffff');
    assert.ok(Object.prototype.hasOwnProperty.call(next.tjanster, 'bokslut'));
  });
