...
describe('tjanst-aktiv-kundkoppling', () => {
  ...
  it('hittar radering av egen och standardtjänst', () => {
    let prev = TjanstUtforandeMallar.emptyState();
    const added = TjanstUtforandeMallar.addCustomService(prev, 'ffff');
    prev = added.state;
    prev = TjanstUtforandeMallar.upsertEntry(prev, 'bokslut', { aktiv: true });
    const prevSnap = JSON.parse(JSON.stringify(prev));
    let next = TjanstUtforandeMallar.removeEntry(prev, added.id);
    next = TjanstUtforandeMallar.removeEntry(next, 'bokslut');
    const removals = TAK.findRemovals(prevSnap, next);
    assert.equal(removals.length, 2);
    assert.ok(removals.some((r) => r.mallId === added.id && r.namn === 'ffff'));
    assert.ok(removals.some((r) => r.mallId === 'bokslut'));
    assert.equal(Object.prototype.hasOwnProperty.call(next.tjanster, 'bokslut'), false);
    assert.ok(next.excludedMallIds.includes('bokslut'));
  });

  it('blockerar radering av egen tjänst när kunder är kopplade', () => {
    let prev = TjanstUtforandeMallar.emptyState();
    const added = TjanstUtforandeMallar.addCustomService(prev, 'dfff');
    prev = added.state;
    const prevSnap = JSON.parse(JSON.stringify(prev));
    const next = TjanstUtforandeMallar.removeEntry(prev, added.id);
    const blocked = TAK.findBlockedDeletion(
      prevSnap,
      next,
      [{ id: 'recCustom', fields: { 'Task Name': 'dfff' } }],
      { recCustom: 1 }
    );
    assert.ok(blocked);
    assert.equal(blocked.kundCount, 1);
    assert.match(blocked.message, /Kan inte radera/);
    assert.match(blocked.message, /1 kund/);
  });

  it('tillåter radering av egen tjänst utan kundkoppling', () => {
    let prev = TjanstUtforandeMallar.emptyState();
    const added = TjanstUtforandeMallar.addCustomService(prev, 'ffff');
    prev = added.state;
    const next = TjanstUtforandeMallar.removeEntry(prev, added.id);
    assert.equal(
      TAK.findBlockedDeletion(prev, next, [{ id: 'recCustom', fields: { 'Task Name': 'ffff' } }], {}),
      null
    );
  });
