...
const STAT_LABELS = {
  ...
  APPROX: 'Kontrollerad via ungefärlig ort',
  SEU: 'Särskilt utsatt område (SEU)',
  UTSATT: 'Utsatt område',
  GEO_FAIL: 'Kunde inte geokoda'
};

/** Kategorier som kräver åtgärd / uppföljning – visas klickbara i statistik. */
const STAT_ACTIONABLE = new Set([
  STAT_LABELS.GEO_FAIL,
  STAT_LABELS.SEU,
  STAT_LABELS.UTSATT
]);
...
function utsattStatLabel(stored) {
  ...
  if (stored.geocoding && stored.geocoding.ok && stored.geocoding.precision === 'approximate') {
    return STAT_LABELS.APPROX;
  }
...
function isActionableUtsattStat(label) {
  return STAT_ACTIONABLE.has(String(label || '').trim());
}
