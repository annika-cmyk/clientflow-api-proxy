...
test('archives by age according to subcategory rules', () => {
  const lag = { Riskniva: 'Låg' };
  const normal = { Riskniva: 'Normal' };
  const hog = { Riskniva: 'Hög' };
  assert.equal(shouldArchiveByAge('kyc', '2018-01-01', '2026-01-01', { customerFields: lag }), true);
  assert.equal(shouldArchiveByAge('kyc', '2022-01-01', '2026-01-01', { customerFields: lag }), false);
  assert.equal(shouldArchiveByAge('kyc', '2022-01-01', '2026-01-01', { customerFields: normal }), true);
  assert.equal(shouldArchiveByAge('kyc', '2025-06-01', '2026-01-01', { customerFields: normal }), false);
  assert.equal(shouldArchiveByAge('kyc', '2024-06-01', '2026-01-01', { customerFields: hog }), true);
...
test('applyAgeBasedArchive archives KYC when triggers are pending', () => {
  const { applyAgeBasedArchive } = require('./dokument-risk-subkategori');
  const kycUppfoljning = require('./kyc-uppfoljning');
  const list = [
    { filename: 'kyc.pdf', category: 'riskbedomning', subcategory: 'kyc', createdDate: '2026-01-01' }
  ];
  const triggers = JSON.stringify({
    addressSnapshot: 'Storgatan 1',
    triggers: [{ type: 'ny_adress', at: '2026-01-15' }]
  });
  const after = applyAgeBasedArchive(list, '2026-01-20', {
    customerFields: { [kycUppfoljning.UPPFOLJNING_FIELD]: triggers, Riskniva: 'Låg' }
  });
  assert.equal(after[0].category, 'historik_riskbedomning');
});
