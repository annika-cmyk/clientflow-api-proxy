...
describe('kundformular', () => {
  ...
  it('status strip and remind stamp without rotating invite', () => {
    let form = Kundformular.applySave(
      Kundformular.emptyForm(),
      {},
      { actor: 'byra', action: 'mark_sent', customerId: 'recStrip1' }
    );
    const token = form.inviteToken;
    const strip = Kundformular.statusStripForUi(form);
    assert.equal(strip.find((s) => s.state === 'current')?.id, 'skickat');
    assert.equal(strip.find((s) => s.id === 'signerat')?.comingSoon, true);
    assert.equal(Kundformular.canRemind(form), true);

    form = Kundformular.applySave(form, {}, { actor: 'byra', action: 'remind' });
    assert.equal(form.inviteToken, token);
    assert.equal(form.status, 'skickat');
    assert.equal(form.reminderCount, 1);
    assert.ok(form.lastRemindedAt);
    const summary = Kundformular.summaryForUi(form, { baseUrl: 'https://example.test' });
    assert.equal(summary.canRemind, true);
    assert.equal(summary.reminderCount, 1);
    assert.ok(summary.statusStrip.length === 5);
  });

  it('shouldAutoRemind after minDays and respects maxReminders', () => {
    let form = Kundformular.applySave(
      Kundformular.emptyForm(),
      {},
      { actor: 'byra', action: 'mark_sent', customerId: 'recAuto1' }
    );
    form = {
      ...form,
    ...
    assert.equal(Kundformular.shouldAutoRemind(form, { minDays: 3 }), true);
    assert.equal(Kundformular.shouldAutoRemind(form, { minDays: 10 }), false);

    form = Kundformular.applySave(form, {}, { actor: 'byra', action: 'remind' });
    form = { ...form, reminderCount: 5, lastRemindedAt: new Date(Date.now() - 10 * 24 * 60 * 60 * 1000).toISOString() };
    assert.equal(Kundformular.shouldAutoRemind(form, { minDays: 3, maxReminders: 5 }), false);
  });

  it('set_byra_vh and kyc sync patch from kundformulär', () => {
    let form = Kundformular.applyPrefill(Kundformular.emptyForm(), {
      fields: { Namn: 'Acme AB', Orgnr: '556677-8899' },
      kyc: { verksamhet: 'Handel', pep: 'Nej', kapitalUrsprung: 'Vinst' }
    });
    form = Kundformular.applySave(form, {
      byraVhBekraftelse: 'Ja',
      byraVhNote: 'Kontrollerat mot register'
    }, { actor: 'byra', action: 'set_byra_vh' });
    assert.equal(form.byraVhBekraftelse, 'Ja');
    assert.equal(form.byraVhNote, 'Kontrollerat mot register');
    assert.ok(form.byraVhUpdatedAt);
    // answers ska inte ha skrivits över av meta-action
    assert.equal(form.answers.verksamhet, 'Handel');

    const patch = Kundformular.buildKycSyncPatchFromKundformular(form);
    assert.equal(patch.foretagsnamn, 'Acme AB');
    assert.equal(patch.orgnr, '556677-8899');
    assert.equal(patch.verksamhet, 'Handel');
    assert.equal(patch.pep, 'Nej');
    assert.equal(patch.kapitalUrsprung, 'Vinst');
  });

  it('sanitizePublicForm strips byrå/sync/reminder internals', () => {
    let form = Kundformular.applySave(
      Kundformular.emptyForm(),
      { foretagsnamn: 'Acme AB', pep: 'Nej' },
      { actor: 'byra', action: 'save' }
    );
    form = Kundformular.applySave(form, {
      byraVhBekraftelse: 'Osaker',
      byraVhNote: 'hemlig utredning'
    }, { actor: 'byra', action: 'set_byra_vh' });
    form = Kundformular.applySave(form, {}, { actor: 'byra', action: 'mark_sent', customerId: 'recX' });
    form.reminderCount = 3;
    form.lastRemindedAt = '2026-09-01T00:00:00.000Z';
    form.lastSyncedToKycAt = '2026-09-02T00:00:00.000Z';

    const pub = Kundformular.buildPublicResponse(form, {
      tjansterOptions: [{ id: 't1', namn: 'Bokföring' }],
      bolagsform: 'Aktiebolag',
      fields: { Bolagsform: 'Aktiebolag' }
    });

    assert.equal(pub.form.answers.foretagsnamn, 'Acme AB');
    assert.equal(pub.form.status, 'skickat');
    assert.equal(pub.form.inviteToken, undefined);
    assert.equal(pub.form.byraVhBekraftelse, undefined);
    assert.equal(pub.form.byraVhNote, undefined);
    assert.equal(pub.form.reminderCount, undefined);
    assert.equal(pub.form.lastSyncedToKycAt, undefined);
    assert.equal(pub.form.lastRemindedAt, undefined);
    assert.ok(!Object.prototype.hasOwnProperty.call(pub.summary, 'byraVhNote'));
    assert.ok(!Object.prototype.hasOwnProperty.call(pub.summary, 'reminderCount'));
    assert.equal(pub.meta.customerMode, true);
    assert.equal(pub.meta.vhKomplex, false);
    // Byrå riskFlags får inte påverka — skarpt bara via answers (pep Nej → false)
    assert.equal(pub.meta.skarptKapital, false);
  });

  it('buildPublicUiMeta ignores byrå riskFlags and kyc komplex flags', () => {
    const form = Kundformular.applySave(
      Kundformular.emptyForm(),
      { pep: 'Nej', huvudman: [{ namn: 'A', hemvist: 'Sverige', agarandel: '50' }] },
      { actor: 'byra', action: 'save' }
    );
    const meta = Kundformular.buildPublicUiMeta(form, {
      bolagsform: 'Aktiebolag',
      fields: { Bolagsform: 'Aktiebolag' }
    });
    assert.equal(meta.vhKomplex, false);
    assert.equal(meta.skarptKapital, false);

    // Kontroll: byrå-meta skulle visa komplex via kyc-flagga
    const byraMeta = Kundformular.buildUiMeta(form, {
      bolagsform: 'Aktiebolag',
      riskFlags: ['Komplex ägarstruktur'],
      kyc: { komplexAgarstruktur: 'Ja' },
      hogrisksland: true
    });
    assert.equal(byraMeta.vhKomplex, true);
    assert.equal(byraMeta.skarptKapital, true);
  });
