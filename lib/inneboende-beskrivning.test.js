...
describe('inneboende-beskrivning', () => {
  ...
  it('namn- och inneboende-blocket ligger på rätt flikar och flikarna scrollar', () => {
    ...
    const pageHtml = fs.readFileSync(path.join(__dirname, '../public/riskbedomning-byra.html'), 'utf8');
    ...
    assert.match(css, /\.ai-review-summary\[hidden\]\s*\{[^}]*display:\s*none/);
    ...
    assert.match(css, /\.tjanst-modal-layout\s*\{/);
  ...
  it('tjänstesidan separerar vägledning, inneboende risk och residual', () => {
    ...
    const pageHtml = fs.readFileSync(path.join(__dirname, '../public/riskbedomning-byra.html'), 'utf8');
    const html = AnalyskortMall.renderPageModals('tjanst');
    assert.match(pageHtml, /Vägledning: Riskbedömning av byråns tjänster/);
    assert.match(pageHtml, /Separation av risk och åtgärd/);
    assert.match(pageHtml, /Terrorfinansiering \(TF\)/);
