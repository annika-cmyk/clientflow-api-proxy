...
describe('inneboende-beskrivning', () => {
  it('använder tydliga etiketter utan dubbel inneboende-risk-rubrik', () => {
    assert.equal(TJANST_BESKRIVNING_LABEL, 'Tjänstebeskrivning');
  ...
  it('tjänstesidan använder bara designsystemets typsnitt', () => {
    const html = fs.readFileSync(path.join(__dirname, '../public/riskbedomning-byra.html'), 'utf8');
    const css = fs.readFileSync(path.join(__dirname, '../public/styles.css'), 'utf8');
    assert.doesNotMatch(html, /fonts\.googleapis\.com/);
    assert.doesNotMatch(html, /Inter|Merriweather/);
    ...
    assert.match(css, /\.risk-begrepp-card h4[\s\S]*text-transform:\s*none/);
