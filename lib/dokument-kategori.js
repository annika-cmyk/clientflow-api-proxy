...
function upsertDokumentKategori(kategorier, attachment, updates) {
  ...
  if (patch.subcategory !== undefined) {
    const sub = String(patch.subcategory || '').trim();
    if (sub) next.subcategory = sub;
    else delete next.subcategory;
  }
  if (patch.screeningKey !== undefined) {
    const key = String(patch.screeningKey || '').trim();
    if (key) next.screeningKey = key;
    else delete next.screeningKey;
  }
...
function applyDokumentKategoriMeta(item, kategorier) {
  ...
  if (meta.subcategory) overlay.subcategory = String(meta.subcategory).trim();
  if (meta.screeningKey) overlay.screeningKey = String(meta.screeningKey).trim();
