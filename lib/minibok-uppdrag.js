...
const YearlyUppdragRuns = require(path.join(__dirname, '..', 'public', 'js', 'yearly-uppdrag-runs.js'));
...
function startIsoForRun(periodKey, deadlineIso, typ, freq, fields) {
  ...
  if (YearlyUppdragRuns.isYearlyFreq(freq) || typ === 'Bokslut' || typ === 'Deklaration') {
    return YearlyUppdragRuns.alignYearlyStartToDeadline(fields?.['Startdatum'] || '', dl) || dl;
  }
...
function createMinibokUppdrag({
  ...
  async function listUppdragForByra(byraId, { mine = false, userName = '' } = {}) {
    ...
    const fetchBatch = async (ids) => {
      ...
      try {
        custRes = await axios.get(custUrl, {
          ...
          params: {
            ...
            maxRecords: 100,
            fields: ['Namn', 'Företagsnamn', 'Orgnr', 'Klientansvarig', 'Användare']
          }
        });
      } catch (e) {
          ...
          try {
            custRes = await axios.get(custUrl, {
              ...
              params: {
                filterByFormula: formula,
                maxRecords: 100,
                fields: ['Namn', 'Orgnr', 'Klientansvarig', 'Användare']
              }
            });
          } catch (e2) {
            const msg2 = e2.response?.data?.error?.message || e2.message || '';
            if (/Unknown field name/i.test(String(msg2))) {
              custRes = await axios.get(custUrl, {
                headers,
      ...
      (custRes.data.records || []).forEach((r) => {
        ...
        nameById[r.id] = {
          ...
          klientansvarig: String(f.Klientansvarig || '').trim(),
          anvandareIds: parseLinkedUserIds(f['Användare'])
        };
      });
    };
    ...
    const userIds = Array.from(new Set(
      Object.values(nameById).flatMap((c) => (c && c.anvandareIds) || [])
    ));
    const userNameById = {};
    if (userIds.length) {
      try {
        for (let i = 0; i < userIds.length; i += 40) {
          const batch = userIds.slice(i, i + 40);
          const parts = batch.map((id) => `RECORD_ID()="${escAirtable(id)}"`).join(',');
          // eslint-disable-next-line no-await-in-loop
          const userRes = await axios.get(
            `https://api.airtable.com/v0/${airtableBaseId()}/${encodeURIComponent('Application Users')}`,
            {
              headers,
              params: { filterByFormula: `OR(${parts})`, maxRecords: 50 }
            }
          );
          (userRes.data.records || []).forEach((u) => {
            const uf = u.fields || {};
            userNameById[u.id] = String(uf['Full Name'] || uf.Namn || uf.Email || '').trim() || u.id;
          });
        }
      } catch (e) {
        console.warn('⚠️ minibok-uppdrag: kunde inte slå upp behöriga:', e.message);
      }
    }
    ...
    records.forEach((r) => {
      ...
      if (cid && nameById[cid]) {
        const cust = nameById[cid];
        r.fields.Kundnamn = cust.name;
        r.fields.Orgnr = cust.orgNr;
        r.fields._klientansvarigKund = cust.klientansvarig || '';
        r.fields._behoriga = (cust.anvandareIds || []).map((id) => userNameById[id] || id);
      }
    });
  ...
  async function completeUppdrag({ customerId, typ, note, doneAt, periodKey: periodKeyRaw, userEmail, byraId }) {
    ...
    if (next && YearlyUppdragRuns.isYearlyFreq(freq)) {
      const advanced = YearlyUppdragRuns.advanceYearlyTemplate({
  ...
  function buildBoardRows(records, { month, typ } = {}) {
    ...
    const rows = forMonth.map((x) => mapUppdragBoardRow(x));
          ...
          tables: [
            {
              ...
              keyFields: [
                ...
                'Ansvarig', 'Klientansvarig', 'Senast utförd', 'Status', 'Historik',
                'Riskåtgärder aktiverade', 'Riskåtgärder valda', 'PTL Underlag',
                'Anteckning', 'Anteckning för denna körning', 'Rutin'
              ]
            },
            {
            ...
            },
            {
              ...
              keyFields: ['Namn', 'Företagsnamn', 'Orgnr', 'Klientansvarig', 'Användare']
            }
          ],
          typValues: [
            LONE_TYP_LEGACY,
