...
describe('ai-tjanst-analys', () => {
  ...
  it('inkluderar egna/kompletterande utförandesvar i AI-blocket', () => {
    const block = Ai.formatUtforandeBlock('Momsredovisning', Mallar.upsertEntry(Mallar.emptyState(), 'momsredovisning', {
      aktiv: true,
      answers: {
      ...
      kommentarer: {
        momsGor: 'Vi hanterar även omvänd skattskyldighet manuellt'
      }
    }));
    assert.match(block, /Ja, upprättar/);
    assert.match(block, /eget\/kompletterande svar/);
    assert.match(block, /omvänd skattskyldighet/);
  });
  ...
  it('tar bort läckt evidensetikett sist i sårbarhetsbeskrivning', () => {
    const leaked = 'Felaktig redovisning av kunddata kan påverka momsredovisningen negativt. Bekräftad.';
    assert.equal(
      Ai.stripEvidensLeakFromText(leaked),
      'Felaktig redovisning av kunddata kan påverka momsredovisningen negativt.'
    );
    assert.equal(Ai.stripEvidensLeakFromText('Text. Tjänstetypisk risk.'), 'Text.');
    assert.equal(Ai.stripEvidensLeakFromText('Text. Saknad information.'), 'Text.');
    assert.equal(Ai.stripEvidensLeakFromText('Text utan etikett.'), 'Text utan etikett.');
    assert.equal(
      Ai.cleanSarbarhetItem({ titel: 'Kunddata', beskrivning: leaked, evidens: 'bekraftad' }).beskrivning,
      'Felaktig redovisning av kunddata kan påverka momsredovisningen negativt.'
    );
  });
  ...
  it('AI-tjänstprompten förbjuder påhittade byråfakta och tillåter bekräftad tjänstebeskrivning', () => {
    ...
    assert.match(Ai.TJANST_ANALYS_AI_RULES, /INGA EVIDENSETIKETTER/);
    assert.match(Ai.TJANST_ANALYS_AI_RULES, /Bekräftad\./);
    assert.doesNotMatch(Ai.TJANST_ANALYS_AI_RULES, /Märk varje sårbarhet med evidens/);
    ...
    assert.match(Ai.HOT_MODUS_AI_RULES, /3–6 meningar/);
  ...
  it('styr pedagogisk analys för nyanställda och förbjuder svagare omskrivningar', () => {
    assert.match(Ai.TJANST_ANALYS_AI_RULES, /PEDAGOGIK FÖR NYANSTÄLLDA/);
    assert.match(Ai.TJANST_ANALYS_AI_RULES, /expandera och förtydliga/);
    assert.match(Ai.PEDAGOGISK_ANALYS_AI_RULES, /nyanställda/);
    assert.match(Ai.PEDAGOGISK_ANALYS_AI_RULES, /AML för dummies/);
    assert.match(Ai.PEDAGOGISK_ANALYS_AI_RULES, /4–8 meningar/);
    assert.match(Ai.PEDAGOGISK_ANALYS_AI_RULES, /svagare/);
    const index = fs.readFileSync(path.join(__dirname, '../index.js'), 'utf8');
    assert.match(index, /PEDAGOGISK_ANALYS_AI_RULES/);
    assert.match(index, /4-8 meningar/);
    assert.match(index, /VAR det dokumenteras/);
  });
  ...
  it('formaterar extra underlag och behåller användartillagda hot vid regenerering', () => {
    assert.equal(Ai.formatExtraUnderlagBlock(''), '');
    assert.equal(Ai.formatExtraUnderlagBlock('   '), '');
    const block = Ai.formatExtraUnderlagBlock('Falska mejl som ser ut att komma från kunden och ber om betalning.');
    assert.match(block, /EXTRA UNDERLAG FRÅN BYRÅN/);
    assert.match(block, /Falska mejl/);
    assert.match(block, /AML-relevant/);
    assert.match(block, /PRIMÄR källa för åtgärder/);
    assert.match(block, /Skriv OM dessa fält/);

    const state = Mallar.upsertEntry(Mallar.emptyState(), 'betalningsuppdrag', {
      aktiv: true,
      aiExtraUnderlag: 'Phishing-mejl om utbetalning'
    });
    assert.equal(Ai.readExtraUnderlag('Betalningsuppdrag', state), 'Phishing-mejl om utbetalning');
    assert.equal(
      Ai.readExtraUnderlag('Betalningsuppdrag', state, 'Override från request'),
      'Override från request'
    );

    assert.equal(Ai.isUserAddedItem({ titel: 'X', userAdded: true }), true);
    assert.equal(Ai.isUserAddedItem({ titel: 'X', ursprung: 'user' }), true);
    assert.equal(Ai.isUserAddedItem({ titel: 'X' }), false);

    const current = [
      { titel: 'Falskt mejl från kund', beskrivning: 'Ber om betalning.', userAdded: true },
      { titel: 'Osanna fakturor', beskrivning: 'AI-hot.' }
    ];
    const incoming = [
      { titel: 'Osanna fakturor', beskrivning: 'Uppdaterad AI-text.' },
      { titel: 'Kapitaltillskott utan underlag', beskrivning: 'Nytt AI-hot.' }
    ];
    const merged = Ai.preserveUserAddedItems(current, incoming);
    assert.equal(merged[0].titel, 'Falskt mejl från kund');
    assert.equal(merged[0].userAdded, true);
    assert.ok(merged.some((h) => h.titel === 'Osanna fakturor'));
    assert.ok(merged.some((h) => h.titel === 'Kapitaltillskott utan underlag'));
    assert.equal(merged.filter((h) => /Falskt mejl/.test(h.titel)).length, 1);

    const index = fs.readFileSync(path.join(__dirname, '../index.js'), 'utf8');
    const chunk = index.slice(index.indexOf("app.post('/api/ai-byra-tjanst'"));
    assert.match(chunk, /formatExtraUnderlagBlock/);
    assert.match(chunk, /readExtraUnderlag/);
    assert.match(chunk, /extraUnderlagBlock/);
    assert.match(chunk, /preferUnderlagRewrite/);
    assert.match(chunk, /formatTjanstExistingBlock\(befintligt,\s*\{/);
  });

  it('tjänst-AI använder prompt_cache_key, max_output_tokens och synlig modell', () => {
    const index = fs.readFileSync(path.join(__dirname, '../index.js'), 'utf8');
    const chunk = index.slice(index.indexOf("app.post('/api/ai-byra-tjanst'"));
    assert.match(chunk, /promptCacheKey:\s*reviewMode \? 'cf-byra-tjanst-rev' : 'cf-byra-tjanst-gen'/);
    ...
    assert.match(chunk, /RiskanalysTjanstKatalog\.PROMPT_RULES/);
    assert.doesNotMatch(chunk, /katalogBlock \? `\\n\$\{RiskanalysTjanstKatalog\.PROMPT_RULES\}/);
  });

  it('riskregel: Nej på klientmedelskonto förbjuder genomgångskonto-hot', () => {
    const state = Mallar.upsertEntry(Mallar.emptyState(), 'betalningsuppdrag', {
      aktiv: true,
      answers: { betKlientmedelskonto: 'Nej', betVad: ['Löner'] }
    });
    const signals = Ai.analyzeUtforandeSignals('Betalningsuppdrag', state);
    assert.equal(signals.hasKlientmedelskonto, false);
    assert.ok(signals.lines.some((l) => /inte har klientmedelskonto|klientmedelskonto/i.test(l)));
    assert.ok(signals.lines.some((l) => /genomgångskonto|ta bort/i.test(l)));
    const jaState = Mallar.upsertEntry(Mallar.emptyState(), 'betalningsuppdrag', {
      aktiv: true,
      answers: { betKlientmedelskonto: 'Ja' }
    });
    const jaSignals = Ai.analyzeUtforandeSignals('Betalningsuppdrag', jaState);
    assert.equal(jaSignals.hasKlientmedelskonto, true);
    assert.ok(jaSignals.lines.some((l) => /har klientmedelskonto/i.test(l)));
  });
});
