...
describe('mapUppdragBoardRow', () => {
  it('exponerar rutin, anteckning, klientansvarig, behöriga och riskåtgärder', () => {
    ...
    assert.equal(row.runAnsvarig, 'Lisa Bok');
    assert.equal(row.runAnsvarigInherited, true);
  });

  it('använder Historik.ansvarig som körningens utförare', () => {
    const row = mapUppdragBoardRow({
      record: {
        id: 'recUppRun',
        fields: {
          Ansvarig: 'Lisa Bok',
          Historik: JSON.stringify([
            { periodKey: '2026-Q2', ansvarig: 'Johanna Delvert', status: 'Pågående' },
          ]),
        },
      },
      typ: 'Momsredovisning',
    ...
    assert.equal(row.ansvarig, 'Lisa Bok');
    assert.equal(row.runAnsvarig, 'Johanna Delvert');
    assert.equal(row.runAnsvarigInherited, false);
  });

  it('plockar namn från länkade Airtable-objekt', () => {
    const row = mapUppdragBoardRow({
      record: {
        id: 'recUpp2',
        fields: {
          Ansvarig: [{ id: 'recA', name: 'Lisa Bok' }],
          Klientansvarig: { id: 'recK', name: 'Mirsad Rrahmani' },
        },
      },
      typ: 'Bokslut',
      periodKey: '2026',
    ...
    assert.equal(row.ansvarig, 'Lisa Bok');
    assert.equal(row.klientansvarig, 'Mirsad Rrahmani');
  });
});
