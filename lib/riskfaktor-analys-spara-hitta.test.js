...
const KUND_MODAL = AnalyskortMall.renderPageModals('kundrisker');
const OVRIGA_MODAL = AnalyskortMall.renderPageModals('ovriga');
...
describe('riskfaktor analys spara/hitta', () => {
  it('validerar obligatoriska fält med flikbyte i stället för tyst HTML5-fail', () => {
    ...
    assert.match(KUND_MODAL, /id="add-risk-form"[^>]*novalidate/);
    assert.match(KUND_MODAL, /id="edit-risk-form"[^>]*novalidate/);
    assert.match(OVRIGA_MODAL, /id="add-risk-form"[^>]*novalidate/);
    assert.match(OVRIGA_MODAL, /id="edit-risk-form"[^>]*novalidate/);
  });
  ...
  it('läser namn och beskrivning från DOM (namn ligger utanför form)', () => {
    ...
    assert.match(OVRIGA_MODAL, /id="risk-factor"[^>]*form="add-risk-form"/);
    assert.match(OVRIGA_MODAL, /id="edit-risk-factor"[^>]*form="edit-risk-form"/);
    assert.match(KUND_MODAL, /id="risk-factor"[^>]*form="add-risk-form"/);
    assert.match(KUND_MODAL, /id="edit-risk-factor"[^>]*form="edit-risk-form"/);
    assert.match(KUND, /ovriga-riskfaktorer\.js\?v=\d+/);
    assert.match(OVRIGA, /ovriga-riskfaktorer\.js\?v=\d+/);
    assert.match(KUND, /analyskort-mall\.js/);
    assert.match(OVRIGA, /analyskort-mall\.js/);
  });
  ...
  it('fyller tomma fält vid AI-reviewMode (byråprofil-prefill)', () => {
    const genStart = JS.indexOf('async generateAiSuggestion');
    const genEnd = JS.indexOf('modeFromModalId', genStart);
    assert.ok(genStart >= 0 && genEnd > genStart, 'generateAiSuggestion ska finnas');
    const gen = JS.slice(genStart, genEnd);
    assert.match(gen, /if \(reviewMode\)[\s\S]*?applyOvrigAiIfEmpty\(prefix, befintligt, data\)/);
    assert.match(gen, /focusRiskTabAfterAi\(/);
    assert.match(JS, /applyOvrigAiIfEmpty\(prefix, existing, data\)[\s\S]*?data\.hot[\s\S]*?addHotRow/);
    assert.match(JS, /applyOvrigAiIfEmpty\(prefix, existing, data\)[\s\S]*?data\.sarbarheter[\s\S]*?addSarbarhetRow/);
    assert.match(JS, /applyOvrigAiMotivering\([\s\S]*?fillSplitMotiveringToDom/);
    assert.match(JS, /focusRiskTabAfterAi\(mode, data/);
  });
  ...
  it('Från byråprofilen kan refreshas efter sparning', () => {
    ...
    assert.match(KUND, /kundrisker-enkat-sammanfattning\.js\?v=\d+/);
    assert.match(KUND, /ovriga-riskfaktorer\.js\?v=\d+/);
  });
});
