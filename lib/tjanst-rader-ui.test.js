...
describe('förenklade tjänsterader', () => {
  ...
  it('visar frågerubrikerna i redigeringsvyn', () => {
    const page = read('public/riskbedomning-byra.html');
  ...
  it('håller källa ihopfälld och valfri på hot/sårbarhet', () => {
    const js = read('public/js/riskbedomning-byra.v5.js');
    const html = AnalyskortMall.renderPageModals('tjanst');
    assert.match(js, /dyn-kalla-wrap is-collapsed/);
    assert.match(js, /Visa källa \(valfritt\)/);
    assert.match(js, /Lägg till källa \(valfritt\)/);
    assert.match(html, /Källa är valfritt/);
    assert.doesNotMatch(html, /beskrivning och källa/);
  });
