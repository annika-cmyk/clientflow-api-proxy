...
describe('byra-resa', () => {
  ...
  it('requires NRA checklist only when NRA is anvander', () => {
    const all = {};
    DEFAULT_KALLOR.forEach((k) => {
      all[k.id] = { status: 'tagit_del' };
    });
    assert.equal(nraChecklistRequired(all), false);
    assert.equal(nraChecklistComplete(all, {}), true);
    all[NRA_SOURCE_ID] = { status: 'anvander' };
    assert.equal(nraChecklistRequired(all), true);
    assert.equal(nraChecklistComplete(all, {}), false);

    const answers = {};
    NRA_SCENARIOS.forEach((s) => {
      if (s.kind === 'yesno') answers[s.id] = { answer: 'ja', why: '' };
      else if (s.kind === 'controls') {
        answers[s.id] = { answer: 'beskriven', why: 'Rimlighetsbedömning av underlag och avvikelsehantering.' };
      } else {
        answers[s.id] = { answer: 'derived', why: '' };
      }
    });
    assert.equal(nraChecklistComplete(all, answers), true);

    const yesnoId = NRA_SCENARIOS.find((s) => s.kind === 'yesno').id;
    answers[yesnoId] = { answer: 'nej', why: 'x' };
    assert.equal(nraChecklistComplete(all, answers), false);
    answers[yesnoId] = { answer: 'nej', why: 'inte i kundstock' };
    assert.equal(nraChecklistComplete(all, answers), true);

    const controlsId = NRA_SCENARIOS.find((s) => s.kind === 'controls').id;
    answers[controlsId] = { answer: 'unset', why: 'kort' };
    assert.equal(nraChecklistComplete(all, answers), false);
    answers[controlsId] = { answer: 'beskriven', why: 'Rimlighetsbedömning av underlag och avvikelsehantering.' };
    assert.equal(nraChecklistComplete(all, answers), true);
    assert.equal(step8Ready(all, [], answers), true);
  });

  it('persists nraChecklist through parse/build', () => {
    const answers = {};
    NRA_SCENARIOS.forEach((s) => {
      if (s.kind === 'yesno') answers[s.id] = { answer: 'ja', why: '' };
      else if (s.kind === 'controls') {
        answers[s.id] = { answer: 'beskriven', why: 'Kontroller via rimlighetsbedömning.' };
      } else {
        answers[s.id] = { answer: 'derived', why: '' };
      }
    });
    const built = buildByraResaState({
      ...
      nraChecklist: answers
    });
    const parsed = parseByraResaState(JSON.stringify(built));
    assert.equal(parsed.nraChecklist[NRA_SCENARIOS[0].id].answer, 'ja');
    assert.equal(mergeNraChecklist(parsed.nraChecklist).length, NRA_SCENARIOS.length);
  });

  it('exposes three NRA question kinds with readable copy', () => {
    assert.equal(NRA_SCENARIOS.length, 6);
    assert.equal(NRA_SCENARIOS.filter((s) => s.kind === 'yesno').length, 3);
    assert.equal(NRA_SCENARIOS.filter((s) => s.kind === 'controls').length, 2);
    assert.equal(NRA_SCENARIOS.filter((s) => s.kind === 'derived').length, 1);

    assert.equal(NRA_SCENARIOS[0].title, 'Terrorfinansiering');
    assert.equal(NRA_SCENARIOS[0].tag, 'NRA kap. 4');
    assert.match(NRA_SCENARIOS[0].desc, /ideella föreningar/);
    assert.equal(NRA_SCENARIOS[1].title, 'Formella intyg');
    assert.match(NRA_SCENARIOS[1].desc, /kreditintyg/);
    assert.equal(NRA_SCENARIOS[2].title, 'Slussbolag');
    assert.match(NRA_SCENARIOS[2].desc, /skenbara transaktioner/);
    assert.equal(NRA_SCENARIOS[3].title, 'Bokföring som täckmantel');
    assert.equal(NRA_SCENARIOS[3].kind, 'controls');
    assert.equal(NRA_SCENARIOS[4].title, 'Oavsiktlig möjliggörare');
    assert.equal(NRA_SCENARIOS[4].kind, 'controls');
    assert.equal(NRA_SCENARIOS[5].title, 'Klientmedel och betalningsuppdrag');
    assert.equal(NRA_SCENARIOS[5].kind, 'derived');
    assert.equal(NRA_SCENARIOS[5].tag, '');

    const merged = mergeNraChecklist({});
    assert.equal(merged[0].tag, 'NRA kap. 4');
    assert.equal(merged[5].kind, 'derived');
    assert.equal(merged[5].derived.active, false);
    assert.match(merged[5].derived.label, /ingen aktiv tjänst/i);
    assert.equal(nraRowComplete(merged[5]), true);
    assert.equal(nraChecklistProgress({}).done, 1);
    assert.doesNotMatch(merged[5].derived.label, /^Ja/i);
  });

  it('derives klientmedel from active betalningsuppdrag service', () => {
    const finder = () => ({
      active: true,
      mallId: 'betalningsuppdrag',
      name: 'Betalningsuppdrag och betalningshantering'
    });
    const merged = mergeNraChecklist({}, { findActiveBetalningsuppdrag: finder });
    const row = merged.find((r) => r.kind === 'derived');
    assert.equal(row.derived.active, true);
    assert.equal(row.answer, 'ja');
    assert.match(row.derived.label, /Betalningsuppdrag och betalningshantering/);
    assert.doesNotMatch(row.derived.label, /^Ja\s*[—\-]/);
    assert.match(row.derived.href, /riskbedomning-byra\.html\?namn=/);
    assert.equal(nraRowComplete(row), true);
  });
});
