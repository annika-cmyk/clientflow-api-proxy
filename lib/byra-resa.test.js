...
describe('byra-resa', () => {
  ...
  it('requires NRA checklist only when NRA is anvander', () => {
    ...
    NRA_SCENARIOS.forEach((s) => {
      answers[s.id] = { answer: 'ja', why: '' };
    });
    assert.equal(nraChecklistComplete(all, answers), true);
    answers[NRA_SCENARIOS[0].id] = { answer: 'nej', why: 'x' };
    assert.equal(nraChecklistComplete(all, answers), false);
    answers[NRA_SCENARIOS[0].id] = { answer: 'nej', why: 'inte i kundstock' };
    assert.equal(nraChecklistComplete(all, answers), true);
  ...
  it('exposes readable NRA scenario copy with optional source tags', () => {
    ...
    assert.equal(NRA_SCENARIOS[1].title, 'Bokföring som täckmantel');
    assert.equal(NRA_SCENARIOS[2].title, 'Skattebrott och slussbolag');
    assert.match(NRA_SCENARIOS[2].desc, /slussbolag/);
    assert.equal(NRA_SCENARIOS[3].title, 'Formella intyg');
