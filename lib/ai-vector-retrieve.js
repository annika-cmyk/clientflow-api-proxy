...
function buildOvrigRepairPrompt({
  ...
  const badBits = [];
  if (rejected.beskrivning) badBits.push('- återanvänd inte den gamla beskrivningens generiska «högt värde»-formulering');
  (Array.isArray(rejected.hot) ? rejected.hot : []).slice(0, 3).forEach((h) => {
    const t = String((h && h.titel) || '').trim();
    if (t) badBits.push(`- återanvänd inte hotet «${t}» utan att skriva om med konkret bokförings-/exportmekanism`);
    const k = String((h && h.kalla) || '').trim();
    if (k) badBits.push(`- använd inte källan «${k}»`);
  });

  return [
    'FÖRRA JSON-SVARET ÄR UNDERKÄNT OCH FÅR INTE ÅTERANVÄNDAS.',
    'Gör en ny komplett analys från riskfaktorn, EXTRA UNDERLAG och KÄLLUTDRAG — inte minsta möjliga ändring av förra utkastet.',
    badBits.length ? `Särskilt förbjudet att återanvända:\n${badBits.join('\n')}` : '',
    ...
    'Svara ENDAST med komplett JSON enligt schemat.'
  ].filter(Boolean).join('\n\n');
}
