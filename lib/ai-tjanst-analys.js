...
6. Bekräftade byråspecifika uppgifter / generella tjänstetypiska risker / saknad information (resonera i texten, ingen separat etikett).
...
Om något är en möjlig risk men inte bekräftad: formulera den som möjlig sårbarhet / bör verifieras.
Om något är bekräftat: formulera den som faktisk byråspecifik sårbarhet.

SÅRBARHETSBESKRIVNING — INGA EVIDENSETIKETTER:
- Skriv bara den löpande beskrivningen i fältet "beskrivning".
- Lägg INTE till evidensetiketter sist i texten (förbjudet: "Bekräftad.", "Tjänstetypisk risk.", "Saknad information.", "bekraftad", "tjanstetypisk", "saknas" som avslut).
- Evidensstatus visas inte i UI och ska inte finnas som separat fält eller suffix.
...
S×K-KALIBRERING (undervärdera inte):
- 3 = medel/kännbar — inte «lite förhöjt». Soft-default till S=2–3 och K=2–3 är fel när utförandesvar, statistik eller konkreta hot visar verklig exponering.
- Kalibrera S och K mot faktisk exponering: utförandesvar, kundstatistik (högriskbransch, utland, kontanter, PEP, högriskland), och de hot du listar — inte mot en lugn «Normal»-känsla.
- Inneboende risk = före kontroller. Soft-score inte inneboende bara för att residualen ska se lugn ut. Residual får vara lägre än inneboende efter åtgärder; inneboende ska spegla exponeringen.
- När byrån hanterar ansökan till myndighet OCH betalningsuppgifter: inneboende risk ska normalt vara minst Förhöjd (S×K ≥ 10), t.ex. S=3 K=4 eller S=4 K=3 — inte Normal (3×3).
- Saknad rimlighetskontroll sänker inte inneboende; den håller residualen uppe och ska synas som sårbarhet.
- Statistik som höjer: högriskbransch, internationella transaktioner, kontanthantering, PEP, högriskland, komplex ägarstruktur, hög andel förhöjd/hög riskklass bland tjänstens kunder → höj S och/eller K jämfört med en vanlig redovisningstjänst.
- Dåligt: S=3 K=3 (Normal) när hoten beskriver oriktiga myndighetsansökningar, felaktiga utbetalningar och byrån som legitimerande mellanled utan förebyggande kontroll.
- Bra: S och K som ger en risknivå som stämmer med exponeringen; motiveringen använder dimensionsorden (medel/kännbar/allvarlig …) och nämner siffrorna — inte «förhöjd sannolikhet».

RISKREGLER — exponering, åtgärder och residual:
- Om tjänsten innebär ansökan, rapportering eller inlämning till myndighet ska detta öka tjänstens exponering och ofta höja inneboende S×K.
- Om tjänsten innebär hantering av betalningsuppgifter ska detta öka exponeringen och ofta höja inneboende S×K.
- Om byrån svarar att rimlighetskontroll normalt inte görs ska detta anges som en kvarstående sårbarhet.
- Åtgärden "kunden får komplettera i efterhand" är reaktiv och får bara sänka risken begränsat, särskilt om det saknas normal kontroll som upptäcker avvikelser. Utan rimlighetskontroll upptäcks inte oklarheten. Det är inte tillräckligt för låg residualrisk.
...
- Om byrån hanterar fakturaunderlag, arbetskostnad/materialkostnad eller ansökan till Skatteverket: inneboende risk minst Normal (S×K ≥ 5); undervärdera inte till Låg.
- Om byrån hanterar ansökan till Skatteverket OCH betalningsuppgifter: inneboende risk minst Förhöjd (S×K ≥ 10). Soft-score till 3×3 (Normal) är fel.
- Om byrån även hanterar betalningsuppgifter: öka sårbarheten och ofta konsekvensen (felaktiga utbetalningar/legitimitet).
- Om byrån inte normalt kontrollerar rimlighet: residualrisk kan normalt inte bli låg; inneboende ska ändå spegla exponeringen (kontroller sänker residual, inte inneboende).
- Önskat residualresonemang när ansökan eller betalningsuppgifter finns utan rimlighetskontroll: "Residualrisken bedöms som normal, inte låg, eftersom byrån hanterar ansökan till Skatteverket och betalningsuppgifter men normalt inte kontrollerar ROT/RUT-underlagets rimlighet. Att kunden får komplettera i efterhand vid oklarheter minskar risken i vissa fall, men åtgärden är främst reaktiv och förutsätter att avvikelsen först upptäcks."
...
PEDAGOGIK FÖR NYANSTÄLLDA (Din resa / AR):
- Skriv så att en nyanställd med begränsad AML- och redovisningserfarenhet förstår.
- För varje hot, sårbarhet och åtgärd: förklara vad det betyder, varför det spelar roll för penningtvätt/terrorismfinansiering, och vad byrån ska vara uppmärksam på.
- Preferera att expandera och förtydliga framför att korta ner. Behåll konkreta exempel och operativa detaljer från byråns befintliga text när de finns.
- Föreslå aldrig en kortare/svagare omskrivning av en redan bra beskrivning. Om texten bara behöver lätt puts — behåll den och lägg hellre till pedagogik/AML-koppling utan att ta bort innehåll.`;
...
function formatUtforandeBlock(namn, state) {
  ...
  formatted.rows.forEach((row) => {
    const note = row.kommentar
      ? ` (eget/kompletterande svar: ${row.kommentar})`
      : '';
    lines.push(`- ${row.label}: ${row.svar}${note}`);
  });
...
/** Valfritt fritextunderlag som byrån skrivit till AI (sparat på utförandeposten). */
function readExtraUnderlag(namn, state, override) {
  const fromBody = String(override == null ? '' : override).trim();
  if (fromBody) return fromBody;
  const hit = TjanstUtforandeMallar.findEntryForNamn(state, namn);
  const entry = hit && hit.entry;
  return String((entry && (entry.aiExtraUnderlag || entry.extraUnderlag)) || '').trim();
}

function formatExtraUnderlagBlock(text) {
  const t = String(text == null ? '' : text).trim();
  if (!t) return '';
  return [
    'EXTRA UNDERLAG FRÅN BYRÅN (valfritt — ta med i analysen när det är AML-relevant):',
    'Byrån beskriver hot, modus eller omständigheter som AI kan ha missat (t.ex. falska mejl som ser ut att komma från kunden och ber om betalning).',
    'Detta underlag är PRIMÄR källa för åtgärder (atgarder) och motiveringResidual när det anges.',
    'Skriv OM dessa fält utifrån underlaget — återanvänd inte generisk boilerplate och strö bara in ett underlagsord.',
    'Inkludera dem som konkreta hot/modus, sårbarheter eller åtgärder när de hör till tjänsten. Hitta inte på mer än vad som står här plus övrigt underlag.',
    '',
    t
  ].join('\n');
}
...
function isUserAddedItem(item) {
  ...
  const origin = String(item.ursprung || item.source || item.kallaTyp || '').trim().toLowerCase();
  return origin === 'user' || origin === 'eget' || origin === 'egen';
}

function itemFoldKey(item) {
  return foldText((item && (item.titel || item.title || item.namn || item.beskrivning || item.description)) || '');
}

/**
 * Behåll användartillagda poster vid AI-regenerering.
 * Incoming får inte torka bort user-added; nära dubbletter (samma titel-nyckel) hoppas över.
 */
function preserveUserAddedItems(current, incoming) {
  const userItems = (Array.isArray(current) ? current : [])
    .filter(isUserAddedItem)
    .map((item) => Object.assign({}, item, { userAdded: true }));
  const have = new Set(userItems.map(itemFoldKey).filter(Boolean));
  const out = userItems.slice();
  (Array.isArray(incoming) ? incoming : []).forEach((item) => {
    if (!item || typeof item !== 'object') return;
    const key = itemFoldKey(item);
    if (key && have.has(key)) return;
    const next = Object.assign({}, item);
    if (isUserAddedItem(next)) next.userAdded = true;
    out.push(next);
    if (key) have.add(key);
  });
  return out;
...
/** AI brukade läcka evidensetiketter sist i sårbarhetsbeskrivningen när evidensfältet togs bort från JSON. */
const EVIDENS_LEAK_SUFFIX_RE =
  /(?:^|\s+)(?:Bekräftad(?:\s+byråspecifik(?:\s+faktor)?)?|Tjänstetypisk(?:\s+risk)?|Saknad(?:\s+information)?|bekraftad|tjanstetypisk|saknas)\.?$/iu;

function stripEvidensLeakFromText(value) {
  let out = String(value == null ? '' : value).trim();
  if (!out) return '';
  let prev = '';
  while (out !== prev) {
    prev = out;
    out = out.replace(EVIDENS_LEAK_SUFFIX_RE, '').trim();
  }
  return out;
}

function cleanSarbarhetItem(raw) {
  const s = raw && typeof raw === 'object' ? raw : {};
  return {
    titel: String(s.titel ?? s.title ?? '').trim(),
    beskrivning: stripEvidensLeakFromText(s.beskrivning ?? s.description),
    kalla: String(s.kalla ?? s.källa ?? s.source ?? '').trim()
  };
}
...
function analyzeUtforandeSignals(namn, state) {
  ...
  const hasKlientmedelskonto = typeof TjanstUtforandeMallar.hasKlientmedelskonto === 'function'
    ? TjanstUtforandeMallar.hasKlientmedelskonto(state)
    : null;
  ...
  if (hasKlientmedelskonto === false) {
    lines.push('- Byrån har svarat Nej på «Har byrån klientmedelskonto?». Föreslå inte hot om byråns klientmedelskonto eller genomgångskonto via byråns egna medel. Om sådana hot redan finns i befintligt innehåll: föreslå att ta bort dem (ändra inte bara källan).');
  } else if (hasKlientmedelskonto === true) {
    lines.push('- Byrån har klientmedelskonto. Bedöm hot kopplade till klientmedelskonto/genomgångskonto där det är relevant för tjänsten.');
  }
  ...
  if (isRotRut) {
    ...
    if (handlesInvoice || handlesLabor || handlesApplication) {
      ...
      lines.push('- Byrån hanterar fakturaunderlag, arbetskostnad/materialkostnad eller ansökan till Skatteverket. Inneboende risk ska vara minst Normal (S×K ≥ 5). Soft-score till 2×2 är fel.');
    }
    if (handlesPayment) {
      lines.push('- Byrån hanterar betalningsuppgifter. Det ökar sårbarheten och ofta konsekvensen.');
    }
    if (handlesApplication && handlesPayment) {
      inherentFloor = 'Förhöjd';
      lines.push('- Ansökan till Skatteverket + betalningsuppgifter: inneboende risk minst Förhöjd (S×K ≥ 10), t.ex. S=3 K=4 eller S=4 K=3. Undervärdera inte till Normal (3×3). Inneboende speglar exponeringen före kontroller.');
    }
    if (noReasonableness) {
      lines.push('- Byrån svarar att rimlighetskontroll normalt inte görs. Ange det som kvarstående sårbarhet. Det sänker inte inneboende risk men håller residualen uppe.');
    }
    if (customerSupplement) {
      lines.push('- «Kunden får komplettera i efterhand» är reaktiv och får bara sänka residualrisken begränsat, särskilt utan normal kontroll som upptäcker avvikelser.');
    }
    ...
    if (handlesAuthorityGeneric) {
      lines.push('- Tjänsten innebär ansökan, rapportering eller inlämning till myndighet. Det ökar exponeringen och ofta inneboende S×K.');
    }
    if (handlesPaymentGeneric) {
      lines.push('- Byrån hanterar betalningsuppgifter. Det ökar exponeringen och ofta inneboende S×K.');
    }
    if (handlesAuthorityGeneric && handlesPaymentGeneric) {
      inherentFloor = 'Förhöjd';
      lines.push('- Myndighetsinlämning + betalningsuppgifter: inneboende risk minst Förhöjd (S×K ≥ 10). Soft-score till Normal (3×3) är fel.');
    }
    if (noReasonableness) {
      lines.push('- Rimlighetskontroll görs normalt inte. Ange det som kvarstående sårbarhet. Det sänker inte inneboende risk men håller residualen uppe.');
    }
    if (customerSupplement) {
      lines.push('- «Kunden får komplettera i efterhand» är reaktiv och får bara sänka residualrisken begränsat.');
    }
...
Varje hot ska ha:
- kort titel
- konkret beskrivning på 3–6 meningar som täcker de fyra punkterna
...
const PEDAGOGISK_ANALYS_AI_RULES = `PEDAGOGISK ANALYS — «AML för dummies» (målgrupp nyanställda):
- Alla Din resa-fält (beskrivning/översikt, hot, sårbarheter, åtgärder, motiveringar) ska vara begripliga utan AML-bakgrund. Förklara begrepp i klarspråk första gången de används.
- LÄNGD: skriv rikare, inte telegramstil. Föredra 4–8 meningar per motivering och åtgärds-/kontrollbeskrivning; 3–6 meningar per hot/sårbarhet. En ensam vag mening är FÖRBJUDEN.
- Förklara alltid: (1) vad som kan hända, (2) hur det kopplar till penningtvätt eller terrorismfinansiering steg för steg, (3) vilken roll redovisningsbyrån har, (4) vad medarbetaren ska vara uppmärksam på i praktiken.
- Dåligt (för kort): "Branschen innebär hög risk för penningtvätt."
- Dåligt (för kort åtgärd): "Dokumentation och granskning genomförs i Capego."
- Bra: förklara mekanismen (t.ex. köp med brottsvinster → omsättning → till synes lagliga pengar), varför just denna faktor/tjänst öppnar den luckan, och vad byrån konkret tittar efter.
- Expandera framför att korta ner. Föreslå ALDRIG en kortare/svagare omskrivning som tappar kontext.`;
...
module.exports = {
  ...
  readExtraUnderlag,
  formatExtraUnderlagBlock,
  isUserAddedItem,
  preserveUserAddedItems,
  buildByraExponering,
  formatExponeringBlock,
  normalizeEvidens,
  normalizeAtgardStatus,
  evidensLabel,
  stripEvidensLeakFromText,
  cleanSarbarhetItem,
