...
const PEDAGOGISK_ANALYS_AI_RULES = `PEDAGOGISK ANALYS (målgrupp nyanställda):
- Din resa-innehåll (hot, sårbarheter, åtgärder, motiveringar, översikt) ska vara begripligt utan djup AML-bakgrund.
- Förklara mekanismen i klarspråk: vad som kan gå fel, hur det kopplar till penningtvätt eller terrorismfinansiering, och vilken roll byrån har.
- Åtgärder: behåll byråns konkreta detaljer (kontroller, konsekvenser, kundbeteende, system). Expandera med varför det minskar risk — korta inte ner till en vag sammanfattning.
- Dåligt: ersätt en rik beskrivning med en kortare, svagare mening som tappar kontext.
- Bra: behåll detaljerna och lägg till en mening om AML-relevans och vad medarbetaren ska tänka på.`;
